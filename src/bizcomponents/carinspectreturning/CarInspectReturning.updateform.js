...
import styles from './CarInspectReturning.updateform.less';
...
class CarInspectReturningUpdateForm extends Component {
 ...
 mapBackToImageValues(convertedImagesValues) {
  ...
  getSelectedRow(){
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
			returnTime: moment(item.returnTime).format('YYYY-MM-DD'),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
     validateFieldsAndScroll((error, values) => {
        ...
        const carInspectReturningId = values.id;
        const imagesValues = this.mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values,carInspectReturningId, ...imagesValues };

	    const newIndex= currentUpdateIndex+1;
        dispatch({
          type: owner.type+'/updateCarInspectReturning',
    ...
    const skipToNext = () => {
      ...
      const newIndex= currentUpdateIndex+1;
      dispatch({
          type: owner.type+'/gotoNextCarInspectReturningUpdateRow',
            payload: {
              id:owner.id,type:'carInspectReturning', 
    ...
    return (
      <PageHeaderLayout
        ...
        title={"更新汽车检查返回"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新汽车检查返回"
...
}))(Form.create()(CarInspectReturningUpdateForm));
