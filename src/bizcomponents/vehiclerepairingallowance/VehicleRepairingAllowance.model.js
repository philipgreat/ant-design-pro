    ...
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        const dashboardmatch = pathToRegexp(
          '/vehicleRepairingAllowance/:id/dashboard'
        ).exec(pathname)
        if (dashboardmatch) {
        ...
        const editDetailMatch = pathToRegexp(
          '/vehicleRepairingAllowance/:id/editDetail'
        ).exec(pathname)
        if (editDetailMatch) {
        ...
        const viewDetailMatch = pathToRegexp(
          '/vehicleRepairingAllowance/:id/viewDetail'
        ).exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp(
          '/vehicleRepairingAllowance/:id/list/:listName'
        ).exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *load({ payload }, { call, put }) {
      ...
      const data = yield call(
        VehicleRepairingAllowanceService.load,
        payload.id,
        payload.parameters
      )
    ...
    *gotoCreateForm({ payload }, { put }) {
      ...
      yield put(
        routerRedux.push(
          `/vehicleRepairingAllowance/${id}/list/${type}CreateForm`
        )
      )
    },
    *gotoUpdateForm({ payload }, { put }) {
      ...
      const location = {
        pathname: `/vehicleRepairingAllowance/${id}/list/${type}UpdateForm`,
        state,
      }
    ...
    *goback({ payload }, { put }) {
      const { id, type } = payload
      yield put(
        routerRedux.push(`/vehicleRepairingAllowance/${id}/list/${type}List`)
      )
    },
    ...
    *addRepairingQuotationItem({ payload }, { call, put }) {
      ...
      const data = yield call(
        VehicleRepairingAllowanceService.addRepairingQuotationItem,
        id,
        parameters
      )
      if (hasError(data)) {
      ...
      const location = {
        pathname: `/vehicleRepairingAllowance/${id}/list/${type}List`,
        state: data,
      }
      yield put(routerRedux.push(location))
    },
    *updateRepairingQuotationItem({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(
        VehicleRepairingAllowanceService.updateRepairingQuotationItem,
        id,
        parameters
      )
      if (hasError(data)) {
    ...
    *removeRepairingQuotationItemList({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(
        VehicleRepairingAllowanceService.removeRepairingQuotationItemList,
        id,
        parameters
      )
      if (hasError(data)) {
