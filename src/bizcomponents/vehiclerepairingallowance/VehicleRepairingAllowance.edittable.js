...
import styles from './VehicleRepairingAllowance.edittable.less'
...
class VehicleRepairingAllowanceEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const vehicleRepairingAllowanceIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {vehicleRepairingAllowanceId} = record.id
      const parameters = { ...record, vehicleRepairingAllowanceId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateVehicleRepairingAllowance`,
        payload: {
          id: owner.id,
          type: 'vehicleRepairingAllowance',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newVehicleRepairingAllowanceToAppend  = {
      	'id':`+1`, 
				'allowanceTitle':'',
				'allowanceCode':'',
				'allowanceAmount':'',
      ...
      newData.push(newVehicleRepairingAllowanceToAppend);
...
export default Form.create()(VehicleRepairingAllowanceEditTable)
