...
import styles from './VehicleRepairingAllowance.search.less'
...
class VehicleRepairingAllowanceSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const vehicleRepairingAllowanceIds = selectedRows.map((item) => { return item.id })
    console.log('vehicleRepairingAllowanceIds', vehicleRepairingAllowanceIds)
    const parameters = { vehicleRepairingAllowanceIds }
    dispatch({
      type: `${owner.type}/removeVehicleRepairingAllowanceList`,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'vehicleRepairingAllowance', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'汽车修理平台补贴列表' } })
  ...
  render() {
    ...
    const {VehicleRepairingAllowanceTable} = GlobalComponents;
    const {VehicleRepairingAllowanceConfirmationTable} = GlobalComponents;
    const {VehicleRepairingAllowanceSearchForm} = GlobalComponents;
...
<Menu.Item key="service">关联服务</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <VehicleRepairingAllowanceConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 汽车修理平台补贴列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <VehicleRepairingAllowanceSearchForm {...this.props} />
            </div>
...
export default Form.create()(VehicleRepairingAllowanceSearch)
