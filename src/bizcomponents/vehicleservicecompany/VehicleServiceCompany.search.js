...
class VehicleServiceCompanySearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const vehicleServiceCompanyIds = selectedRows.map((item) => { return item.id })
    console.log('vehicleServiceCompanyIds', vehicleServiceCompanyIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'vehicleServiceCompany', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'商户列表' } })
  ...
  render() {
    ...
    const {VehicleServiceCompanyTable} = GlobalComponents;
    const {VehicleServiceCompanyConfirmationTable} = GlobalComponents;
    const {VehicleServiceCompanySearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <VehicleServiceCompanyConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 商户列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <VehicleServiceCompanySearchForm {...this.props} />
            </div>
...
export default Form.create()(VehicleServiceCompanySearch)
