  ...
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="拒收原因">
        {serviceFileMovementM2m.rejectComments}
      </Description>
      <Description term="拒收凭证1">
        {serviceFileMovementM2m.rejectEvidence1}
      </Description>
      <Description term="拒收凭证2">
        {serviceFileMovementM2m.rejectEvidence2}
      </Description>
      <Description term="拒收凭证3">
        {serviceFileMovementM2m.rejectEvidence3}
      </Description>
      <Description term="拒收凭证4">
        {serviceFileMovementM2m.rejectEvidence4}
      </Description>
      <Description term="拒收凭证5">
        {serviceFileMovementM2m.rejectEvidence5}
      </Description>
      <Description term="开始时间">
        {moment(serviceFileMovementM2m.startTime).format('YYYY-MM-DD')}
      </Description>
      <Description term="最后的位置">
        {serviceFileMovementM2m.lastLocation}
      </Description>
...
export default class ServiceFileMovementM2mViewDetail extends Component {
  ...
  render() {
    ...
    const { serviceFileMovementM2mChecklistResultList } = serviceFileMovementM2m

    const owner = { type: '_serviceFileMovementM2m', id }

    const tabList = [
      {
        key: 'serviceFileMovementM2mChecklistResultList',
        tab: `移件服务检查结果(${serviceFileMovementM2mChecklistResultCount})`,
      },
    ]
    ...
    const contentList = {
      serviceFileMovementM2mChecklistResultList: (
        <ServiceFileMovementM2mChecklistResultViewTable
          data={serviceFileMovementM2mChecklistResultList}
          owner={owner}
          {...this.props}
        />
      ),
