...
export default {
  ...
  effects: {
    ...
    *addServiceFileMovementM2mChecklistResult({ payload }, { call, put }) {
      ...
      const data = yield call(ServiceFileMovementM2mService.addServiceFileMovementM2mChecklistResult, id, parameters)
      if (hasError(data)) {
    ...
    *updateServiceFileMovementM2mChecklistResult({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(ServiceFileMovementM2mService.updateServiceFileMovementM2mChecklistResult, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
    ...
    *gotoNextServiceFileMovementM2mChecklistResultUpdateRow({ payload }, { call, put }) {
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      const newPlayload = { ...payload, selectedRows, currentUpdateIndex }
      yield put({ type: 'updateState', payload: newPlayload })
    },
    *removeServiceFileMovementM2mChecklistResultList({ payload }, { call, put }) {
      const {ServiceFileMovementM2mService} = GlobalComponents; 
      const { id, type, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(ServiceFileMovementM2mService.removeServiceFileMovementM2mChecklistResultList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
