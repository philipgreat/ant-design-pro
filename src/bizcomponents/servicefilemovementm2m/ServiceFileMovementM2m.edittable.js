...
import styles from './ServiceFileMovementM2m.edittable.less'
...
class ServiceFileMovementM2mEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
      ...
      return {responsibleWorkerId,mainOrderId,senderId,receiverId,merchantId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const serviceFileMovementM2mIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {serviceFileMovementM2mId} = record.id
      const parameters = { ...record, serviceFileMovementM2mId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateServiceFileMovementM2m`,
        payload: {
          id: owner.id,
          type: 'serviceFileMovementM2m',
          parameters,
  ...
  { title: '服务类型', debugtype: 'string', dataIndex: 'movementPurpose', width: '26', render: (text, record) => renderStringEdit('movementPurpose',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newServiceFileMovementM2mToAppend  = {
      	'id':`+1`, 
				'serviceStatus':'',
				'responsibleWorker':'',
				'serviceSummary':'',
      ...
      newData.push(newServiceFileMovementM2mToAppend);
...
export default Form.create()(ServiceFileMovementM2mEditTable)
