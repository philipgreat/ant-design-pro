...
import styles from './CustomerAccount.search.less'
...
class CustomerAccountSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const customerAccountIds = selectedRows.map((item) => { return item.id })
    console.log('customerAccountIds', customerAccountIds)
    const parameters = { customerAccountIds }
    dispatch({
      type: `${owner.type}/removeCustomerAccountList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'customerAccount' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'customerAccount', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'客户账户列表' } })
  ...
  render() {
    ...
    const {CustomerAccountTable} = GlobalComponents;
    const {CustomerAccountConfirmationTable} = GlobalComponents;
    const {CustomerAccountSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CustomerAccountConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 客户账户列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CustomerAccountSearchForm {...this.props} />
            </div>
...
export default Form.create()(CustomerAccountSearch)
