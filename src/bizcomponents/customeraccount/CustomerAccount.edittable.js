...
import styles from './CustomerAccount.edittable.less'
...
class CustomerAccountEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
			...
			const accountManagementId = record.accountManagement.id

      //const communityId = record.community.id;
      return {customerId,accountManagementId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const customerAccountIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {customerAccountId} = record.id
      const parameters = { ...record, customerAccountId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateCustomerAccount`,
        payload: {
          id: owner.id,
          type: 'customerAccount',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newCustomerAccountToAppend  = {
      	'id':`+1`, 
				'name':'',
				'account':'',
				'customer':'',
      ...
      newData.push(newCustomerAccountToAppend);
...
export default Form.create()(CustomerAccountEditTable)
