...
import styles from './GroupPage.edittable.less'
...
class GroupPageEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {groupPageId} = record.id
      const parameters = { ...record, groupPageId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateGroupPage`,
        payload: {
          id: owner.id,
          type: 'groupPage',
          parameters,
  ...
  { title: '标题', debugtype: 'string', dataIndex: 'title', width: '7', render: (text, record) => renderStringEdit('title',text, record)  },
  { title: '目前的组名', debugtype: 'string', dataIndex: 'currentGroupName', width: '9', render: (text, record) => renderStringEdit('currentGroupName',text, record)  },
  { title: '社区', dataIndex: 'community', render: (text, record) => (record.community ? record.community.displayName : '暂无') },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newGroupPageToAppend  = {
      	'id':`+1`, 
				'title':'',
				'currentGroupName':'',
				'community':'',
      ...
      newData.push(newGroupPageToAppend);
...
export default Form.create()(GroupPageEditTable)
