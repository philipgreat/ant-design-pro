...
import styles from './GroupPage.createform.less'
...
class GroupPageCreateForm extends Component {
  state = {
  ...
  executeCandidateCommunitySearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = GroupPageService.requestCandidateCommunity("community", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addGroupPage`,
          payload: { id: owner.id, type: 'groupPage', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'groupPage',listName:'群组页面列表' },
      })
    }
        ...
        <Card title="基础信息" className={styles.card} bordered={false}>
          <Form >
            <Row gutter={16}>
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.currentGroupName} {...formItemLayout}>
                  {getFieldDecorator('currentGroupName', {
                    rules: [{ required: true, message: '请输入目前的组名' }],
                  })(
...
}))(Form.create()(GroupPageCreateForm))
