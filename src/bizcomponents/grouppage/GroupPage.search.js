...
class GroupPageSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const groupPageIds = selectedRows.map((item) => { return item.id })
    console.log('groupPageIds', groupPageIds)
    const parameters = { groupPageIds }
    dispatch({
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'groupPage' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'groupPage', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'群组页面列表' } })
  ...
  render() {
    ...
    const {GroupPageTable} = GlobalComponents;
    const {GroupPageConfirmationTable} = GlobalComponents;
    const {GroupPageSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <GroupPageConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 群组页面列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <GroupPageSearchForm {...this.props} />
            </div>
...
export default Form.create()(GroupPageSearch)
