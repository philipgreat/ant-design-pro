...
import styles from './GroupPage.updateform.less';
...
class GroupPageUpdateForm extends PureComponent {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
     validateFieldsAndScroll((error, values) => {
        ...
        const groupPageId = values.id;
        const imagesValues = this.mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values,groupPageId, ...imagesValues };
    ...
    const skipToNext = () => {
      ...
      const newIndex= currentUpdateIndex+1;
      dispatch({
          type: owner.type+'/gotoNextGroupPageUpdateRow',
            payload: {
              id:owner.id,type:'groupPage', 
    ...
    return (
      <PageHeaderLayout
        ...
        title={"更新群组页面"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新群组页面"
        ...
        <Card title="基础信息" className={styles.card} bordered={false}>
          <Form layout="vertical" hideRequiredMark>
            <Row gutter={16}>
             ...
             <Col lg={6} md={12} sm={24}>
                <Form.Item label={fieldLabels.currentGroupName}>
                  {getFieldDecorator('currentGroupName', {
                    rules: [{ required: true, message: '请输入目前的组名' }],
                  })(
                    <Input placeholder="请输入请输入目前的组名string" />
                  )}
                </Form.Item>
              </Col>			
...
}))(Form.create()(GroupPageUpdateForm));
