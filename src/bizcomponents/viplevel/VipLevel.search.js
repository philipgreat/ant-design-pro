...
import styles from './VipLevel.search.less'
...
class VipLevelSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const vipLevelIds = selectedRows.map((item) => { return item.id })
    console.log('vipLevelIds', vipLevelIds)
    const parameters = { vipLevelIds }
    dispatch({
      type: `${owner.type}/removeVipLevelList`,
      payload: { id: owner.id, type: 'vipLevel', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'vipLevel' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'vipLevel', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'Vip级别列表' } })
  ...
  render() {
    ...
    const {VipLevelTable} = GlobalComponents;
    const {VipLevelConfirmationTable} = GlobalComponents;
    const {VipLevelSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <VipLevelConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: Vip级别列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <VipLevelSearchForm {...this.props} />
            </div>
...
export default Form.create()(VipLevelSearch)
