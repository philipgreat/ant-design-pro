...
@Form.create()
export default class EmployeeAttendanceSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const employeeAttendanceIds = selectedRows.map((item) => { return item.id })
    console.log('employeeAttendanceIds', employeeAttendanceIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employeeAttendance', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {EmployeeAttendanceConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EmployeeAttendanceConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 员工考勤列表`}>
        <Card bordered={false}>
