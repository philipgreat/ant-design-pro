...
import styles from './EmployeeAttendance.edittable.less'
...
class EmployeeAttendanceEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const employeeAttendanceIds = [record.id];
      const parameters = { employeeAttendanceIds }
      dispatch({
        type: `${owner.type}/removeEmployeeAttendanceList`,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {employeeAttendanceId} = record.id
      const parameters = { ...record, employeeAttendanceId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '备注', debugtype: 'string', dataIndex: 'remark', width: '11', render: (text, record) => renderStringEdit('remark',text, record)  },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newEmployeeAttendanceToAppend  = {
      	'id':`+1`, 
				'employee':'',
				'enterTime':'',
				'leaveTime':'',
      ...
      newData.push(newEmployeeAttendanceToAppend);
...
export default Form.create()(EmployeeAttendanceEditTable)
