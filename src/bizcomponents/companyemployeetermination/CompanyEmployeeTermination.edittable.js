...
import styles from './CompanyEmployeeTermination.edittable.less'
...
class CompanyEmployeeTerminationEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const companyEmployeeTerminationIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {companyEmployeeTerminationId} = record.id
      const parameters = { ...record, companyEmployeeTerminationId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateCompanyEmployeeTermination`,
        payload: {
          id: owner.id,
          type: 'companyEmployeeTermination',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newCompanyEmployeeTerminationToAppend  = {
      	'id':`+1`, 
				'who':'',
				'eventTime':'',
				'comment':'',
      ...
      newData.push(newCompanyEmployeeTerminationToAppend);
...
export default Form.create()(CompanyEmployeeTerminationEditTable)
