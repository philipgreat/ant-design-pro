...
import styles from './CompanyEmployeeTermination.search.less'
...
class CompanyEmployeeTerminationSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const companyEmployeeTerminationIds = selectedRows.map((item) => { return item.id })
    console.log('companyEmployeeTerminationIds', companyEmployeeTerminationIds)
    const parameters = { companyEmployeeTerminationIds }
    dispatch({
      type: `${owner.type}/removeCompanyEmployeeTerminationList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'companyEmployeeTermination' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'companyEmployeeTermination', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'商户员工合同结束状态变更列表' } })
  ...
  render() {
    ...
    const {CompanyEmployeeTerminationTable} = GlobalComponents;
    const {CompanyEmployeeTerminationConfirmationTable} = GlobalComponents;
    const {CompanyEmployeeTerminationSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CompanyEmployeeTerminationConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 商户员工合同结束状态变更列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CompanyEmployeeTerminationSearchForm {...this.props} />
            </div>
...
export default Form.create()(CompanyEmployeeTerminationSearch)
