...
import styles from './WorkshopRegisterHistory.search.less'
...
class WorkshopRegisterHistorySearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const workshopRegisterHistoryIds = selectedRows.map((item) => { return item.id })
    console.log('workshopRegisterHistoryIds', workshopRegisterHistoryIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'workshopRegisterHistory' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'workshopRegisterHistory', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'车间登记历史列表' } })
  ...
  render() {
    ...
    const {WorkshopRegisterHistoryTable} = GlobalComponents;
    const {WorkshopRegisterHistoryConfirmationTable} = GlobalComponents;
    const {WorkshopRegisterHistorySearchForm} = GlobalComponents;
...
<Menu.Item key="workshop">关联车间</Menu.Item>
<Menu.Item key="registerMember">关联注册会员</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <WorkshopRegisterHistoryConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 车间登记历史列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <WorkshopRegisterHistorySearchForm {...this.props} />
            </div>
...
export default Form.create()(WorkshopRegisterHistorySearch)
