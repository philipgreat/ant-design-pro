...
class PrinterTaskSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const printerTaskIds = selectedRows.map((item) => { return item.id })
    console.log('printerTaskIds', printerTaskIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'printerTask' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'printerTask', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'打印机任务列表' } })
  ...
  render() {
    ...
    const {PrinterTaskTable} = GlobalComponents;
    const {PrinterTaskConfirmationTable} = GlobalComponents;
    const {PrinterTaskSearchForm} = GlobalComponents;
...
<Menu.Item key="printer">关联打印机</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <PrinterTaskConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 打印机任务列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <PrinterTaskSearchForm {...this.props} />
            </div>
...
export default Form.create()(PrinterTaskSearch)
