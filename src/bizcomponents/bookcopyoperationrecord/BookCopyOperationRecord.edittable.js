...
import styles from './BookCopyOperationRecord.edittable.less'
...
class BookCopyOperationRecordEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
			...
			const operateStoreId = record.operateStore.id
			const operationEmployeeId = record.operationEmployee.id

      //const communityId = record.community.id;
      return {bookCopyId,operateStoreId,operationEmployeeId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const bookCopyOperationRecordIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {bookCopyOperationRecordId} = record.id
      const parameters = { ...record, bookCopyOperationRecordId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateBookCopyOperationRecord`,
        payload: {
          id: owner.id,
          type: 'bookCopyOperationRecord',
          parameters,
  ...
  { title: '书副本操作类型', debugtype: 'string', dataIndex: 'bookCopyOperateType', width: '6', render: (text, record) => renderStringEdit('bookCopyOperateType',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newBookCopyOperationRecordToAppend  = {
      	'id':`+1`, 
				'bookCopy':'',
				'bookCopyOperateType':'',
				'operateStore':'',
      ...
      newData.push(newBookCopyOperationRecordToAppend);
...
export default Form.create()(BookCopyOperationRecordEditTable)
