...
import styles from './BookCopyOperationRecord.search.less'
...
class BookCopyOperationRecordSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookCopyOperationRecordIds = selectedRows.map((item) => { return item.id })
    console.log('bookCopyOperationRecordIds', bookCopyOperationRecordIds)
    const parameters = { bookCopyOperationRecordIds }
    dispatch({
      type: `${owner.type}/removeBookCopyOperationRecordList`,
      payload: { id: owner.id, type: 'bookCopyOperationRecord', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookCopyOperationRecord' },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'书复制操作记录列表' } })
  ...
  render() {
    ...
    const {BookCopyOperationRecordTable} = GlobalComponents;
    const {BookCopyOperationRecordConfirmationTable} = GlobalComponents;
    const {BookCopyOperationRecordSearchForm} = GlobalComponents;
...
<Menu.Item key="operateStore">关联经营商店</Menu.Item>
<Menu.Item key="operationEmployee">关联操作员工</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookCopyOperationRecordConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 书复制操作记录列表`}>
        <Card bordered={false}>
...
export default Form.create()(BookCopyOperationRecordSearch)
