...
@Form.create()
export default class SalaryGradeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const salaryGradeIds = selectedRows.map((item) => { return item.id })
    console.log('salaryGradeIds', salaryGradeIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'salaryGrade' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'salaryGrade', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {SalaryGradeConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <SalaryGradeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 工资等级列表`}>
        <Card bordered={false}>
