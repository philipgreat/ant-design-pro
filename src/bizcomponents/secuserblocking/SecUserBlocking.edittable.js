...
import styles from './SecUserBlocking.edittable.less'
...
class SecUserBlockingEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e, record) => {
      ...
      const { data } = this.state
      const secUserBlockingIds = [record.id]
    ...
    const updateRecord = (e, record) => {
      const { dispatch, owner } = this.props
      const { data } = this.state
      const { secUserBlockingId } = record.id
      const parameters = { ...record, secUserBlockingId }
      const newData = [...data]
      const row = newData.filter(item => item.id === record.id)[0]
      row.editable = !row.editable
...
export default Form.create()(SecUserBlockingEditTable)
