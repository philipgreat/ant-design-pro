...
import SecUserBlockingTable from './SecUserBlocking.table';
import SecUserBlockingConfirmationTable from './SecUserBlocking.confirmmationtable';

import SecUserBlockingSearchForm from './SecUserBlocking.searchform';
...
@Form.create()
export default class SecUserBlockingSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    const params = {
      ...
      secUserBlockingListCurrentPage: pagination.current,
      secUserBlockingListRowsPerPage: pagination.pageSize,
      ...formValues,
      ...filters,
    };
    if (sorter.field) {
  ...
  handleDelete = () => {
    ...
    const secUserBlockingIds = selectedRows.map((item) => { return item.id; });
    console.log('secUserBlockingIds', secUserBlockingIds);
    const parameters = { secUserBlockingIds };
    dispatch({
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'secUserBlocking' },
    });
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'secUserBlocking', selectedRows, currentUpdateIndex },
    });
  }
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <SecUserBlockingConfirmationTable data={selectedRows} owner={owner} />
        </Modal>);
    };
