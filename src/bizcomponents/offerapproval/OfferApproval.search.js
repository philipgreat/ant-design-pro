...
@Form.create()
export default class OfferApprovalSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const offerApprovalIds = selectedRows.map((item) => { return item.id })
    console.log('offerApprovalIds', offerApprovalIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'offerApproval' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'offerApproval', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {OfferApprovalConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <OfferApprovalConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 审批工作要约列表`}>
        <Card bordered={false}>
