...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        if (!pathname.startsWith('/offerApproval')) {
          return
        }
        ...
        const dashboardmatch = pathToRegexp('/offerApproval/:id/dashboard').exec(pathname)
        if (dashboardmatch) {
          const id = dashboardmatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const editDetailMatch = pathToRegexp('/offerApproval/:id/editDetail').exec(pathname)
        if (editDetailMatch) {
          const id = editDetailMatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const viewDetailMatch = pathToRegexp('/offerApproval/:id/viewDetail').exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp('/offerApproval/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *gotoCreateForm({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/offerApproval/${id}/list/${type}CreateForm`))
    },
    *gotoUpdateForm({ payload }, { put }) {
      const { id, type, selectedRows, currentUpdateIndex } = payload
      const state = { id, type, selectedRows, currentUpdateIndex }
      const location = { pathname: `/offerApproval/${id}/list/${type}UpdateForm`, state }
      yield put(routerRedux.push(location))
    },
    *goback({ payload }, { put }) {
      const { id, type } = payload
      yield put(routerRedux.push(`/offerApproval/${id}/list/${type}List`))
    },
    ...
    *addEmployee({ payload }, { call, put }) {
      ...
      const location = { pathname: `/offerApproval/${id}/list/${type}List`, state: data }
      yield put(routerRedux.push(location))
    },
    *updateEmployee({ payload }, { call, put }) {
      ...
      const location = { pathname: `/offerApproval/${id}/list/${type}List`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
