...
import styles from './ServicePrice.search.less'
...
class ServicePriceSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const servicePriceIds = selectedRows.map((item) => { return item.id })
    console.log('servicePriceIds', servicePriceIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'servicePrice', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'合同价格列表' } })
  ...
  render() {
    ...
    const {ServicePriceTable} = GlobalComponents;
    const {ServicePriceConfirmationTable} = GlobalComponents;
    const {ServicePriceSearchForm} = GlobalComponents;
...
<Menu.Item key="contract">关联合同</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ServicePriceConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 合同价格列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ServicePriceSearchForm {...this.props} />
            </div>
...
export default Form.create()(ServicePriceSearch)
