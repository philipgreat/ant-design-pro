...
class ServicePriceCreateForm extends Component {
  ...
  executeCandidateContractSearch = filterKey => {
    ...
    const pageNo = 1
    const future = ServicePriceService.requestCandidateContract(
      'contract',
      id,
      filterKey,
      pageNo
  ...
  executeCandidateAvailableServiceSearch = filterKey => {
    ...
    const id = '' //not used for now
    const pageNo = 1
    const future = ServicePriceService.requestCandidateAvailableService(
      'availableService',
      id,
      filterKey,
      pageNo
  ...
  executeCandidateProductSearch = filterKey => {
    ...
    const id = '' //not used for now
    const pageNo = 1
    const future = ServicePriceService.requestCandidateProduct(
      'availableProduct',
      id,
      filterKey,
      pageNo
            ...
            <Row gutter={16}>
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label={fieldLabels.serviceEnabled}>
                  ...
                  })(<Input placeholder="请输入请输入是否提供服务bool" />)}
                </Form.Item>
              </Col>
            </Row>
          </Form>
                  ...
                  {getFieldDecorator('contractId', {
                    ...
                    >
                      {candidateContractList.candidates.map(item => {
                        return (
                          <Option key={item.id}>{`${item.platform}(${
                            item.id
                          })`}</Option>
                        )
                      })}
