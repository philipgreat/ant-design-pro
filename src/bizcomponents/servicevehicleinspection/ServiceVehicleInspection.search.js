...
class ServiceVehicleInspectionSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const serviceVehicleInspectionIds = selectedRows.map((item) => { return item.id })
    console.log('serviceVehicleInspectionIds', serviceVehicleInspectionIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'serviceVehicleInspection' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'serviceVehicleInspection', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'车辆上线检测列表' } })
  ...
  render() {
    ...
    const {ServiceVehicleInspectionTable} = GlobalComponents;
    const {ServiceVehicleInspectionConfirmationTable} = GlobalComponents;
    const {ServiceVehicleInspectionSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ServiceVehicleInspectionConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 车辆上线检测列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ServiceVehicleInspectionSearchForm {...this.props} />
            </div>
...
export default Form.create()(ServiceVehicleInspectionSearch)
