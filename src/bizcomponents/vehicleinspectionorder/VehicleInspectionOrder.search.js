...
class VehicleInspectionOrderSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'年检订单列表' } })
...
<Menu.Item key="contactAddressCity">关联城市</Menu.Item>
...
<Menu.Item key="platform">关联平台</Menu.Item>
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 年检订单列表`}>
        <Card bordered={false}>
...
export default Form.create()(VehicleInspectionOrderSearch)
