...
import styles from './CarInspectionPlatform.edittable.less'
...
class CarInspectionPlatformEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const carInspectionPlatformIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {carInspectionPlatformId} = record.id
      const parameters = { ...record, carInspectionPlatformId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateCarInspectionPlatform`,
        payload: {
          id: owner.id,
          type: 'carInspectionPlatform',
          parameters,
  ...
  { title: '保险接触手机', debugtype: 'string_china_mobile_phone', dataIndex: 'insuranceContactMobile', width: '15', render: (text, record) => renderStringEdit('insuranceContactMobile',text, record)  },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newCarInspectionPlatformToAppend  = {
      	'id':`+1`, 
				'name':'',
				'description':'',
      ...
      newData.push(newCarInspectionPlatformToAppend);
...
export default Form.create()(CarInspectionPlatformEditTable)
