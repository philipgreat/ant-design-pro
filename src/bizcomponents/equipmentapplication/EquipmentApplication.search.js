...
import styles from './EquipmentApplication.search.less'
...
@Form.create()
export default class EquipmentApplicationSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const equipmentApplicationIds = selectedRows.map((item) => { return item.id })
    console.log('equipmentApplicationIds', equipmentApplicationIds)
    const parameters = { equipmentApplicationIds }
    dispatch({
      type: `${owner.type}/removeEquipmentApplicationList`,
      payload: { id: owner.id, type: 'equipmentApplication', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'equipmentApplication' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'equipmentApplication', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {EquipmentApplicationTable} = GlobalComponents;
    const {EquipmentApplicationConfirmationTable} = GlobalComponents;
    const {EquipmentApplicationSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EquipmentApplicationConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="设备应用程序列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <EquipmentApplicationSearchForm {...this.props} />
            </div>
