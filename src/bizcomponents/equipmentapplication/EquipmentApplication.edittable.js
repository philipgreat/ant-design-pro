...
import styles from './EquipmentApplication.edittable.less'
...
class EquipmentApplicationEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
      ...
      return {supplierId,equipmentId,projectId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const equipmentApplicationIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addEquipmentApplication`,
        payload: {
          id: owner.id,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {equipmentApplicationId} = record.id
      const parameters = { ...record, equipmentApplicationId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateEquipmentApplication`,
        payload: {
          id: owner.id,
          type: 'equipmentApplication',
          parameters,
  ...
  { title: '名称', debugtype: 'string', dataIndex: 'name', width: '52', render: (text, record) => renderStringEdit('name',text, record)  },
  { title: '价格', dataIndex: 'price', className:'money', render: (text, record) => ('￥'+text.toFixed(2)) },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newEquipmentApplicationToAppend  = {
      	'id':`+1`, 
				'name':'',
				'price':'',
				'supplier':'',
      ...
      newData.push(newEquipmentApplicationToAppend);
...
export default Form.create()(EquipmentApplicationEditTable)
