...
import styles from './ThreadCanceling.edittable.less'
...
class ThreadCancelingEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const threadCancelingIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addThreadCanceling`,
        payload: {
          id: owner.id,
          type: 'threadCanceling',
          parameters,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {threadCancelingId} = record.id
      const parameters = { ...record, threadCancelingId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateThreadCanceling`,
        payload: {
          id: owner.id,
          type: 'threadCanceling',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newThreadCancelingToAppend  = {
      	'id':`+1`, 
				'who':'',
				'actionTime':'',
				'comment':'',
      ...
      newData.push(newThreadCancelingToAppend);
...
export default Form.create()(ThreadCancelingEditTable)
