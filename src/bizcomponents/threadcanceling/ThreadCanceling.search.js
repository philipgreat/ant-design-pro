...
import ThreadCancelingTable from './ThreadCanceling.table';
import ThreadCancelingConfirmationTable from './ThreadCanceling.confirmmationtable';

import ThreadCancelingSearchForm from './ThreadCanceling.searchform';
...
@Form.create()
export default class ThreadCancelingSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    const params = {
      threadCancelingList: 1,
      threadCancelingListCurrentPage: pagination.current,
      threadCancelingListRowsPerPage: pagination.pageSize,
      ...formValues,
      ...filters,
    };
    if (sorter.field) {
  ...
  handleDelete = () => {
    ...
    const threadCancelingIds = selectedRows.map((item) => { return item.id; });
    console.log('threadCancelingIds', threadCancelingIds);
    const parameters = { threadCancelingIds };
    dispatch({
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'threadCanceling' },
    });
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'threadCanceling', selectedRows, currentUpdateIndex },
    });
  }
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ThreadCancelingConfirmationTable data={selectedRows} owner={owner} />
        </Modal>);
    };
        ...
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ThreadCancelingSearchForm {...this.props} />
            </div>
