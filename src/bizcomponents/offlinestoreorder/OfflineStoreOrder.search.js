...
class OfflineStoreOrderSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const offlineStoreOrderIds = selectedRows.map((item) => { return item.id })
    console.log('offlineStoreOrderIds', offlineStoreOrderIds)
    const parameters = { offlineStoreOrderIds }
    dispatch({
      type: `${owner.type}/removeOfflineStoreOrderList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'offlineStoreOrder' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'offlineStoreOrder', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'线下门店订单列表' } })
  ...
  render() {
    ...
    const {OfflineStoreOrderTable} = GlobalComponents;
    const {OfflineStoreOrderConfirmationTable} = GlobalComponents;
    const {OfflineStoreOrderSearchForm} = GlobalComponents;
...
<Menu.Item key="gameTicket">关联游戏门票</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <OfflineStoreOrderConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 线下门店订单列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <OfflineStoreOrderSearchForm {...this.props} />
            </div>
...
export default Form.create()(OfflineStoreOrderSearch)
