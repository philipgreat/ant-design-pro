...
import styles from './OnlineOrderPayment.search.less'
...
class OnlineOrderPaymentSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const onlineOrderPaymentIds = selectedRows.map((item) => { return item.id })
    console.log('onlineOrderPaymentIds', onlineOrderPaymentIds)
    const parameters = { onlineOrderPaymentIds }
    dispatch({
      type: `${owner.type}/removeOnlineOrderPaymentList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'onlineOrderPayment' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'onlineOrderPayment', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'线上订单支付列表' } })
  ...
  render() {
    ...
    const {OnlineOrderPaymentTable} = GlobalComponents;
    const {OnlineOrderPaymentConfirmationTable} = GlobalComponents;
    const {OnlineOrderPaymentSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <OnlineOrderPaymentConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 线上订单支付列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <OnlineOrderPaymentSearchForm {...this.props} />
            </div>
...
export default Form.create()(OnlineOrderPaymentSearch)
