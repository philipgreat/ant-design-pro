...
import styles from './OnlineOrderPayment.updateform.less'
...
class OnlineOrderPaymentUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const onlineOrderPaymentId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, onlineOrderPaymentId, ...imagesValues }

        // const newIndex= currentUpdateIndex + 1
        dispatch({
          type: `${owner.type}/updateOnlineOrderPayment`,
          payload: {
            id: owner.id,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextOnlineOrderPaymentUpdateRow`,
        payload: {
          id: owner.id,
          type: 'onlineOrderPayment',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新线上订单支付"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新线上订单支付"
...
}))(Form.create()(OnlineOrderPaymentUpdateForm))
