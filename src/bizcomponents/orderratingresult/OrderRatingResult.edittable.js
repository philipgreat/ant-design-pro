...
import styles from './OrderRatingResult.edittable.less'
...
class OrderRatingResultEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
			const availableRatingItemId = record.availableRatingItem.id
			const mainOrderId = record.mainOrder.id

      //const communityId = record.community.id;
      return {availableRatingItemId,mainOrderId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const orderRatingResultIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {orderRatingResultId} = record.id
      const parameters = { ...record, orderRatingResultId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateOrderRatingResult`,
        payload: {
          id: owner.id,
          type: 'orderRatingResult',
          parameters,
  ...
  { title: '评分名称', debugtype: 'string', dataIndex: 'ratingName', width: '11', render: (text, record) => renderStringEdit('ratingName',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newOrderRatingResultToAppend  = {
      	'id':`+1`, 
				'ratingName':'',
				'ratingResult':'',
				'availableRatingItem':'',
      ...
      newData.push(newOrderRatingResultToAppend);
...
export default Form.create()(OrderRatingResultEditTable)
