...
class OrderRatingResultSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const orderRatingResultIds = selectedRows.map((item) => { return item.id })
    console.log('orderRatingResultIds', orderRatingResultIds)
    const parameters = { orderRatingResultIds }
    dispatch({
      type: `${owner.type}/removeOrderRatingResultList`,
      payload: { id: owner.id, type: 'orderRatingResult', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'orderRatingResult' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'orderRatingResult', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'订单评分结果列表' } })
  ...
  render() {
    ...
    const {OrderRatingResultTable} = GlobalComponents;
    const {OrderRatingResultConfirmationTable} = GlobalComponents;
    const {OrderRatingResultSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <OrderRatingResultConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 订单评分结果列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <OrderRatingResultSearchForm {...this.props} />
            </div>
...
export default Form.create()(OrderRatingResultSearch)
