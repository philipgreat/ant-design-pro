...
class AvailableInsuranceSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const availableInsuranceIds = selectedRows.map((item) => { return item.id })
    console.log('availableInsuranceIds', availableInsuranceIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'availableInsurance', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'车辆代办保险列表' } })
  ...
  render() {
    ...
    const {AvailableInsuranceTable} = GlobalComponents;
    const {AvailableInsuranceConfirmationTable} = GlobalComponents;
    const {AvailableInsuranceSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <AvailableInsuranceConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 车辆代办保险列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <AvailableInsuranceSearchForm {...this.props} />
            </div>
...
export default Form.create()(AvailableInsuranceSearch)
