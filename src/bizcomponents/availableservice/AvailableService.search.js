...
import styles from './AvailableService.search.less'
...
class AvailableServiceSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const availableServiceIds = selectedRows.map((item) => { return item.id })
    console.log('availableServiceIds', availableServiceIds)
    const parameters = { availableServiceIds }
    dispatch({
      type: `${owner.type}/removeAvailableServiceList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'availableService' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'availableService', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'服务范围列表' } })
  ...
  render() {
    ...
    const {AvailableServiceTable} = GlobalComponents;
    const {AvailableServiceConfirmationTable} = GlobalComponents;
    const {AvailableServiceSearchForm} = GlobalComponents;
...
<Menu.Item key="availableProduct">关联产品类型</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <AvailableServiceConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 服务范围列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <AvailableServiceSearchForm {...this.props} />
            </div>
...
export default Form.create()(AvailableServiceSearch)
