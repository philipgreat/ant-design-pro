...
import styles from './AvailableService.updateform.less'
...
class AvailableServiceUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const availableServiceId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, availableServiceId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextAvailableServiceUpdateRow`,
        payload: {
          id: owner.id,
          type: 'availableService',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新服务范围"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新服务范围"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.serviceName} {...formItemLayout}>
                  {getFieldDecorator('serviceName', {
                    initialValue: selectedRow.serviceName,
...
}))(Form.create()(AvailableServiceUpdateForm))
