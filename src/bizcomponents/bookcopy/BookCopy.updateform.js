...
import styles from './BookCopy.updateform.less'
...
class BookCopyUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextBookCopyUpdateRow`,
        payload: {
          id: owner.id,
          type: 'bookCopy',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新书副本"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新书副本"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.wxaId} {...formItemLayout}>
                  {getFieldDecorator('wxaId', {
                    initialValue: selectedRow.wxaId,
                    rules: [{ required: true, message: '请输入Wxa Id' }],
                  })(
...
}))(Form.create()(BookCopyUpdateForm))
