...
import styles from './BookCopy.search.less'
...
class BookCopySearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookCopyIds = selectedRows.map((item) => { return item.id })
    console.log('bookCopyIds', bookCopyIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookCopy' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookCopy', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'书副本列表' } })
  ...
  render() {
    ...
    const {BookCopyTable} = GlobalComponents;
    const {BookCopyConfirmationTable} = GlobalComponents;
    const {BookCopySearchForm} = GlobalComponents;
...
<Menu.Item key="bookCopyVendor">关联书副本供应商</Menu.Item>
<Menu.Item key="locationStore">关联存储位置</Menu.Item>
<Menu.Item key="bookCopyStatus">关联书副本地位</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookCopyConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 书副本列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookCopySearchForm {...this.props} />
            </div>
...
export default Form.create()(BookCopySearch)
