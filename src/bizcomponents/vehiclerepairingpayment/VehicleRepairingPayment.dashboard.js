  ...
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="订单合计">
        {vehicleRepairingPayment.originalAmount}
      </Description>
      <Description term="应付金额">
        {vehicleRepairingPayment.actualAmount}
      </Description>
      <Description term="状态">{vehicleRepairingPayment.status}</Description>
      <Description term="微信订单ID">
        {vehicleRepairingPayment.wechatOrderId}
      </Description>
      <Description term="微信预付订单ID">
        {vehicleRepairingPayment.wechatPrepayId}
      </Description>
      <Description term="创建时间">
        {moment(vehicleRepairingPayment.createTime).format('YYYY-MM-DD')}
      </Description>
      <Description term="最后更新时间">
        {moment(vehicleRepairingPayment.lastUpdateTime).format('YYYY-MM-DD')}
      </Description>
    </DescriptionList>
  )
}
...
export default class VehicleRepairingPaymentDashboard extends Component {
  componentDidMount() {
    ...
    const { dispatch, location, vehicleRepairingPayment } = this.props;
    
    if(!vehicleRepairingPayment){
    	return;
    }
    const {displayName} = vehicleRepairingPayment;
    if(!displayName){
    	return;
    }
  ...
  render() {
    ...
    const { id, displayName } = this.props.vehicleRepairingPayment
    const cardsData = {
      cardsName: '支付维修订单',
      cardsFor: 'vehicleRepairingPayment',
      cardsSource: this.props.vehicleRepairingPayment,
      subItems: [],
    }
