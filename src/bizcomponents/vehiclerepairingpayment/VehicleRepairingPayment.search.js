...
import styles from './VehicleRepairingPayment.search.less'
...
class VehicleRepairingPaymentSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const vehicleRepairingPaymentIds = selectedRows.map((item) => { return item.id })
    console.log('vehicleRepairingPaymentIds', vehicleRepairingPaymentIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'vehicleRepairingPayment', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'支付维修订单列表' } })
  ...
  render() {
    ...
    const {VehicleRepairingPaymentTable} = GlobalComponents;
    const {VehicleRepairingPaymentConfirmationTable} = GlobalComponents;
    const {VehicleRepairingPaymentSearchForm} = GlobalComponents;
...
<Menu.Item key="serviceVehicleRepairing">关联维修服务</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <VehicleRepairingPaymentConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 支付维修订单列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <VehicleRepairingPaymentSearchForm {...this.props} />
            </div>
...
export default Form.create()(VehicleRepairingPaymentSearch)
