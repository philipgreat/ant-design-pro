...
import styles from './GameTicket.search.less'
...
class GameTicketSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const gameTicketIds = selectedRows.map((item) => { return item.id })
    console.log('gameTicketIds', gameTicketIds)
    const parameters = { gameTicketIds }
    dispatch({
      type: `${owner.type}/removeGameTicketList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'gameTicket' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'gameTicket', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'游戏门票列表' } })
  ...
  render() {
    ...
    const {GameTicketTable} = GlobalComponents;
    const {GameTicketConfirmationTable} = GlobalComponents;
    const {GameTicketSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <GameTicketConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 游戏门票列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <GameTicketSearchForm {...this.props} />
            </div>
...
export default Form.create()(GameTicketSearch)
