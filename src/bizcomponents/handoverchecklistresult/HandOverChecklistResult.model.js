    ...
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        const dashboardmatch = pathToRegexp(
          '/handOverChecklistResult/:id/dashboard'
        ).exec(pathname)
        if (dashboardmatch) {
        ...
        const editDetailMatch = pathToRegexp(
          '/handOverChecklistResult/:id/editDetail'
        ).exec(pathname)
        if (editDetailMatch) {
        ...
        const viewDetailMatch = pathToRegexp(
          '/handOverChecklistResult/:id/viewDetail'
        ).exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp(
          '/handOverChecklistResult/:id/list/:listName/:listDisplayName'
        ).exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *load({ payload }, { call, put }) {
      ...
      const data = yield call(
        HandOverChecklistResultService.load,
        payload.id,
        payload.parameters
      )
    ...
    *gotoCreateForm({ payload }, { put }) {
      ...
      yield put(
        routerRedux.push(
          `/handOverChecklistResult/${id}/list/${type}CreateForm`
        )
      )
    },
    *gotoUpdateForm({ payload }, { put }) {
      ...
      const location = {
        pathname: `/handOverChecklistResult/${id}/list/${type}UpdateForm`,
        state,
      }
    ...
    *goback({ payload }, { put }) {
      const { id, type } = payload
      yield put(
        routerRedux.push(`/handOverChecklistResult/${id}/list/${type}List`)
      )
    },
  },
