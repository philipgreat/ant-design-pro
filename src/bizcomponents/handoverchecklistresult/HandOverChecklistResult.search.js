...
class HandOverChecklistResultSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'交接检查结果列表' } })
...
<Menu.Item key="availableHandOverItem">关联交接检查项</Menu.Item>
<Menu.Item key="serviceTypeVehicleC2m">关联收车服务</Menu.Item>
<Menu.Item key="serviceTypeVehicleM2m">关联移车服务</Menu.Item>
<Menu.Item key="serviceTypeVehicleM2c">关联还车服务</Menu.Item>
<Menu.Item key="serviceTypeFileC2m">关联收件服务</Menu.Item>
<Menu.Item key="serviceTypeFileM2m">关联移件服务</Menu.Item>
<Menu.Item key="serviceTypeFileM2c">关联还件服务</Menu.Item>
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 交接检查结果列表`}>
        <Card bordered={false}>
...
export default Form.create()(HandOverChecklistResultSearch)
