...
import styles from './BookCopyCheckStatus.edittable.less'
...
class BookCopyCheckStatusEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const bookCopyCheckStatusIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addBookCopyCheckStatus`,
        payload: {
          id: owner.id,
          type: 'bookCopyCheckStatus',
          parameters,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {bookCopyCheckStatusId} = record.id
      const parameters = { ...record, bookCopyCheckStatusId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateBookCopyCheckStatus`,
        payload: {
          id: owner.id,
          type: 'bookCopyCheckStatus',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newBookCopyCheckStatusToAppend  = {
      	'id':`+1`, 
				'name':'',
				'bookManagement':'',
      ...
      newData.push(newBookCopyCheckStatusToAppend);
...
export default Form.create()(BookCopyCheckStatusEditTable)
