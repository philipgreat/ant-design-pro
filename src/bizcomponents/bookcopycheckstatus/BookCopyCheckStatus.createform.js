...
import styles from './BookCopyCheckStatus.createform.less'
...
class BookCopyCheckStatusCreateForm extends Component {
  state = {
  ...
  executeCandidateBookManagementSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = BookCopyCheckStatusService.requestCandidateBookManagement("bookManagement", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addBookCopyCheckStatus`,
          payload: { id: owner.id, type: 'bookCopyCheckStatus', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'bookCopyCheckStatus',listName:'书副本检查状态列表' },
      })
    }
...
}))(Form.create()(BookCopyCheckStatusCreateForm))
