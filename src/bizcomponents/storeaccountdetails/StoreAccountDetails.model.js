  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        if (!pathname.startsWith('/storeAccountDetails')) {
          return
        }
        ...
        const dashboardmatch = pathToRegexp('/storeAccountDetails/:id/dashboard').exec(pathname)
        if (dashboardmatch) {
          const id = dashboardmatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const editDetailMatch = pathToRegexp('/storeAccountDetails/:id/editDetail').exec(pathname)
        if (editDetailMatch) {
          const id = editDetailMatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const viewDetailMatch = pathToRegexp('/storeAccountDetails/:id/viewDetail').exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp('/storeAccountDetails/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
