...
import styles from './StoreAccountDetails.edittable.less'
...
class StoreAccountDetailsEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
			const storeAccountId = record.storeAccount.id
			const relatedMainOrderId = record.relatedMainOrder.id

      //const communityId = record.community.id;
      return {storeAccountId,relatedMainOrderId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const storeAccountDetailsIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {storeAccountDetailsId} = record.id
      const parameters = { ...record, storeAccountDetailsId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateStoreAccountDetails`,
        payload: {
          id: owner.id,
          type: 'storeAccountDetails',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newStoreAccountDetailsToAppend  = {
      	'id':`+1`, 
				'summary':'',
				'changeAmount':'',
				'changeType':'',
      ...
      newData.push(newStoreAccountDetailsToAppend);
...
export default Form.create()(StoreAccountDetailsEditTable)
