...
import styles from './StoreAccountDetails.search.less'
...
class StoreAccountDetailsSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'storeAccountDetails' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'storeAccountDetails', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'存储帐户详细信息列表' } })
  ...
  render() {
    ...
    const {StoreAccountDetailsTable} = GlobalComponents;
    const {StoreAccountDetailsConfirmationTable} = GlobalComponents;
    const {StoreAccountDetailsSearchForm} = GlobalComponents;
...
<Menu.Item key="storeAccount">关联存储账户</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <StoreAccountDetailsConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 存储帐户详细信息列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <StoreAccountDetailsSearchForm {...this.props} />
            </div>
...
export default Form.create()(StoreAccountDetailsSearch)
