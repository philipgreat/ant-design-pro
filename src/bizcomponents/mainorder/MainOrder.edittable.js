...
import styles from './MainOrder.edittable.less'
...
class MainOrderEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {mainOrderId} = record.id
      const parameters = { ...record, mainOrderId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateMainOrder`,
        payload: {
          id: owner.id,
          type: 'mainOrder',
          parameters,
  ...
  { title: '主要订单状态', debugtype: 'string', dataIndex: 'mainOrderStatus', width: '7', render: (text, record) => renderStringEdit('mainOrderStatus',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newMainOrderToAppend);
...
export default Form.create()(MainOrderEditTable)
