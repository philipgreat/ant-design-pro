...
import styles from './MainOrder.search.less'
...
class MainOrderSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const mainOrderIds = selectedRows.map((item) => { return item.id })
    console.log('mainOrderIds', mainOrderIds)
    const parameters = { mainOrderIds }
    dispatch({
      type: `${owner.type}/removeMainOrderList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'mainOrder' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'mainOrder', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'主订单列表' } })
  ...
  render() {
    ...
    const {MainOrderTable} = GlobalComponents;
    const {MainOrderConfirmationTable} = GlobalComponents;
    const {MainOrderSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <MainOrderConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 主订单列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <MainOrderSearchForm {...this.props} />
            </div>
...
export default Form.create()(MainOrderSearch)
