...
@Form.create()
export default class TerminationReasonSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const terminationReasonIds = selectedRows.map((item) => { return item.id })
    console.log('terminationReasonIds', terminationReasonIds)
    const parameters = { terminationReasonIds }
    dispatch({
      type: `${owner.type}/removeTerminationReasonList`,
      payload: { id: owner.id, type: 'terminationReason', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'terminationReason' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'terminationReason', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {TerminationReasonConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TerminationReasonConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 雇佣终止的原因列表`}>
        <Card bordered={false}>
