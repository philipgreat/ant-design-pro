...
import styles from './TerminationReason.edittable.less'
...
class TerminationReasonEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const terminationReasonIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {terminationReasonId} = record.id
      const parameters = { ...record, terminationReasonId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newTerminationReasonToAppend  = {
      	'id':`+1`, 
				'code':'',
				'company':'',
				'description':'',
      ...
      newData.push(newTerminationReasonToAppend);
...
export default Form.create()(TerminationReasonEditTable)
