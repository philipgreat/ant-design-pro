...
const summaryOf = patientInfo => {
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="佩戴设备类型">
        {patientInfo.wearDeviceType}
      </Description>
      <Description term="磨损的开始时间">
        {moment(patientInfo.wearStartTime).format('YYYY-MM-DD')}
      </Description>
      <Description term="康复计划">{patientInfo.recoverPlan}</Description>
      <Description term="复苏开始时间">
        {moment(patientInfo.recoverStartTime).format('YYYY-MM-DD')}
      </Description>
    </DescriptionList>
  )
}
...
export default class PatientInfoViewDetail extends Component {
  onTabChange = key => {
