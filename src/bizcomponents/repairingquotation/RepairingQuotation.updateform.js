...
import styles from './RepairingQuotation.updateform.less'
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const repairingQuotationId = values.id
        const imagesValues = this.mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, repairingQuotationId, ...imagesValues }
    ...
    return (
      <PageHeaderLayout
        title={"更新车辆维修报价"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新车辆维修报价"
