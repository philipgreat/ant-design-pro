...
import styles from './RepairingQuotation.search.less'
...
@Form.create()
export default class RepairingQuotationSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    const params = {
      repairingQuotationList: 1,
      repairingQuotationListCurrentPage: pagination.current,
      repairingQuotationListRowsPerPage: pagination.pageSize,
      ...formValues,
      ...filters,
    }
    if (sorter.field) {
  ...
  handleDelete = () => {
    ...
    const repairingQuotationIds = selectedRows.map((item) => { return item.id })
    console.log('repairingQuotationIds', repairingQuotationIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'repairingQuotation', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {RepairingQuotationTable} = GlobalComponents;
    const {RepairingQuotationConfirmationTable} = GlobalComponents;
    const {RepairingQuotationSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <RepairingQuotationConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="维修报价列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <RepairingQuotationSearchForm {...this.props} />
            </div>
