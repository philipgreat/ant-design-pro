...
import styles from './TaskHiding.edittable.less'
...
class TaskHidingEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {taskHidingId} = record.id
      const parameters = { ...record, taskHidingId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateTaskHiding`,
        payload: {
          id: owner.id,
          type: 'taskHiding',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newTaskHidingToAppend);
...
export default Form.create()(TaskHidingEditTable)
