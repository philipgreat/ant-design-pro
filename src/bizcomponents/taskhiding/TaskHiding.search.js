...
class TaskHidingSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const taskHidingIds = selectedRows.map((item) => { return item.id })
    console.log('taskHidingIds', taskHidingIds)
    const parameters = { taskHidingIds }
    dispatch({
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'taskHiding' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'taskHiding', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'任务屏蔽列表' } })
  ...
  render() {
    ...
    const {TaskHidingTable} = GlobalComponents;
    const {TaskHidingConfirmationTable} = GlobalComponents;
    const {TaskHidingSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TaskHidingConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 任务屏蔽列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TaskHidingSearchForm {...this.props} />
            </div>
...
export default Form.create()(TaskHidingSearch)
