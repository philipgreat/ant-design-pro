...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        const match = pathToRegexp('/taskHiding/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *goback({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/taskHiding/${id}/list/${type}List/${listName}`))
    },
    ...
    *addTask({ payload }, { call, put }) {
      ...
      const data = yield call(TaskHidingService.addTask, id, parameters)
      if (hasError(data)) {
      ...
      const location = { pathname: `/taskHiding/${id}/list/${type}List/任务列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateTask({ payload }, { call, put }) {
      ...
      const location = { pathname: `/taskHiding/${id}/list/${type}List/任务列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
