...
import styles from './WorkshopLike.search.less'
...
class WorkshopLikeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const workshopLikeIds = selectedRows.map((item) => { return item.id })
    console.log('workshopLikeIds', workshopLikeIds)
    const parameters = { workshopLikeIds }
    dispatch({
      type: `${owner.type}/removeWorkshopLikeList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'workshopLike' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'workshopLike', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'车间等列表' } })
  ...
  render() {
    ...
    const {WorkshopLikeTable} = GlobalComponents;
    const {WorkshopLikeConfirmationTable} = GlobalComponents;
    const {WorkshopLikeSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <WorkshopLikeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 车间等列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <WorkshopLikeSearchForm {...this.props} />
            </div>
...
export default Form.create()(WorkshopLikeSearch)
