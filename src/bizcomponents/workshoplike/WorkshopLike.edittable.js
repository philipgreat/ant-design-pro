...
import styles from './WorkshopLike.edittable.less'
...
class WorkshopLikeEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
      ...
      return {workshopId,replierId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const workshopLikeIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {workshopLikeId} = record.id
      const parameters = { ...record, workshopLikeId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateWorkshopLike`,
        payload: {
          id: owner.id,
          type: 'workshopLike',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newWorkshopLikeToAppend);
...
export default Form.create()(WorkshopLikeEditTable)
