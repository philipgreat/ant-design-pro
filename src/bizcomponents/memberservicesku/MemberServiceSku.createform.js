...
import styles from './MemberServiceSku.createform.less'
...
class MemberServiceSkuCreateForm extends Component {
  state = {
  ...
  executeCandidateMemberServicePeriodSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = MemberServiceSkuService.requestCandidateMemberServicePeriod("memberServicePeriod", id, filterKey, pageNo);
    console.log(future);
  ...
  executeCandidateMemberProductSearch = (filterKey) =>{
    ...
    const id = "";//not used for now
    const pageNo = 1;
    const future = MemberServiceSkuService.requestCandidateMemberProduct("memberServiceProduct", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addMemberServiceSku`,
          payload: { id: owner.id, type: 'memberServiceSku', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'memberServiceSku',listName:'会员服务Sku列表' },
      })
    }
...
}))(Form.create()(MemberServiceSkuCreateForm))
