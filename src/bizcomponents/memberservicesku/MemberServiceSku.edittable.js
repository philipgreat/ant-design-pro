...
import styles from './MemberServiceSku.edittable.less'
...
class MemberServiceSkuEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
			...
			const memberProductId = record.memberProduct.id

      //const communityId = record.community.id;
      return {memberServicePeriodId,memberProductId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const memberServiceSkuIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addMemberServiceSku`,
        payload: {
          id: owner.id,
          type: 'memberServiceSku',
          parameters,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {memberServiceSkuId} = record.id
      const parameters = { ...record, memberServiceSkuId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateMemberServiceSku`,
        payload: {
          id: owner.id,
          type: 'memberServiceSku',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newMemberServiceSkuToAppend  = {
      	'id':`+1`, 
				'name':'',
				'description':'',
				'listPrice':'',
      ...
      newData.push(newMemberServiceSkuToAppend);
...
export default Form.create()(MemberServiceSkuEditTable)
