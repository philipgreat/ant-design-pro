...
@Form.create()
export default class CompanyTrainingSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const companyTrainingIds = selectedRows.map((item) => { return item.id })
    console.log('companyTrainingIds', companyTrainingIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'companyTraining', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {CompanyTrainingConfirmationTable} = GlobalComponents;
...
<Menu.Item key="instructor">关联讲师</Menu.Item>
<Menu.Item key="trainingCourseType">关联培训课程类型</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CompanyTrainingConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 公司培训列表`}>
        <Card bordered={false}>
