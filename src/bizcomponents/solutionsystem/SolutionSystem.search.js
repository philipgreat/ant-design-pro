...
import styles from './SolutionSystem.search.less'
...
@Form.create()
export default class SolutionSystemSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const solutionSystemIds = selectedRows.map((item) => { return item.id })
    console.log('solutionSystemIds', solutionSystemIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'solutionSystem' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'solutionSystem', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {SolutionSystemTable} = GlobalComponents;
    const {SolutionSystemConfirmationTable} = GlobalComponents;
    const {SolutionSystemSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <SolutionSystemConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="解决方案系统列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <SolutionSystemSearchForm {...this.props} />
            </div>
