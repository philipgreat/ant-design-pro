...
import styles from './Account.search.less'
...
class AccountSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const accountIds = selectedRows.map((item) => { return item.id })
    console.log('accountIds', accountIds)
    const parameters = { accountIds }
    dispatch({
      type: `${owner.type}/removeAccountList`,
      payload: { id: owner.id, type: 'account', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'account' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'account', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'对账单列表' } })
  ...
  render() {
    ...
    const {AccountTable} = GlobalComponents;
    const {AccountConfirmationTable} = GlobalComponents;
    const {AccountSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <AccountConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 对账单列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <AccountSearchForm {...this.props} />
            </div>
