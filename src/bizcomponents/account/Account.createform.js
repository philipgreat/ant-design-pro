...
import styles from './Account.createform.less'
...
class AccountCreateForm extends Component {
  ...
  executeCandidatePlatformSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = AccountService.requestCandidatePlatform("carInspectionPlatform", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addAccount`,
          payload: { id: owner.id, type: 'account', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'account',listName:'对账单列表' },
      })
    }
