...
import styles from './Account.edittable.less'
...
class AccountEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      ...
      const parameters = { ...record, accountId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateAccount`,
        payload: {
          id: owner.id,
          type: 'account',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newAccountToAppend);
...
export default Form.create()(AccountEditTable)
