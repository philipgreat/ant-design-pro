...
class ServiceCompanyAccountSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const serviceCompanyAccountIds = selectedRows.map((item) => { return item.id })
    console.log('serviceCompanyAccountIds', serviceCompanyAccountIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'serviceCompanyAccount', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'服务商户对账单列表' } })
  ...
  render() {
    ...
    const {ServiceCompanyAccountTable} = GlobalComponents;
    const {ServiceCompanyAccountConfirmationTable} = GlobalComponents;
    const {ServiceCompanyAccountSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ServiceCompanyAccountConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 服务商户对账单列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ServiceCompanyAccountSearchForm {...this.props} />
            </div>
...
export default Form.create()(ServiceCompanyAccountSearch)
