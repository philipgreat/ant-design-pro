...
const imageListOf = (orderReviewResult) =>{
...
const settingListOf = (orderReviewResult) =>{
...
const largeTextOf = (orderReviewResult) =>{
...
class OrderReviewResultDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, orderReviewResult } = this.props;
    
    if(!orderReviewResult){
    	return;
    }
    const {displayName} = orderReviewResult;
    if(!displayName){
    	return;
    }
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.orderReviewResult
    const cardsData = {cardsName:"订单评论结果",cardsFor: "orderReviewResult",cardsSource: this.props.orderReviewResult,
  		subItems: [
