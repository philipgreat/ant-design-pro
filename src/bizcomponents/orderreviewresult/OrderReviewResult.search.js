...
import styles from './OrderReviewResult.search.less'
...
class OrderReviewResultSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const orderReviewResultIds = selectedRows.map((item) => { return item.id })
    console.log('orderReviewResultIds', orderReviewResultIds)
    const parameters = { orderReviewResultIds }
    dispatch({
      type: `${owner.type}/removeOrderReviewResultList`,
      payload: { id: owner.id, type: 'orderReviewResult', parameters },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'orderReviewResult', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'订单评论结果列表' } })
  ...
  render() {
    ...
    const {OrderReviewResultTable} = GlobalComponents;
    const {OrderReviewResultConfirmationTable} = GlobalComponents;
    const {OrderReviewResultSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <OrderReviewResultConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 订单评论结果列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <OrderReviewResultSearchForm {...this.props} />
            </div>
...
export default Form.create()(OrderReviewResultSearch)
