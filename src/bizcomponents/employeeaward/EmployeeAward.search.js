...
@Form.create()
export default class EmployeeAwardSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const employeeAwardIds = selectedRows.map((item) => { return item.id })
    console.log('employeeAwardIds', employeeAwardIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employeeAward' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employeeAward', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {EmployeeAwardConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EmployeeAwardConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 员工嘉奖列表`}>
        <Card bordered={false}>
