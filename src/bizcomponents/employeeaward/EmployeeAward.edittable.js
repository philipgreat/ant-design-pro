...
import styles from './EmployeeAward.edittable.less'
...
class EmployeeAwardEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const employeeAwardIds = [record.id];
      const parameters = { employeeAwardIds }
      dispatch({
        type: `${owner.type}/removeEmployeeAwardList`,
        payload: { id: owner.id, type: 'employeeAward', parameters },
      })
    }
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {employeeAwardId} = record.id
      const parameters = { ...record, employeeAwardId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newEmployeeAwardToAppend  = {
      	'id':`+1`, 
				'employee':'',
				'completeTime':'',
				'type':'',
      ...
      newData.push(newEmployeeAwardToAppend);
...
export default Form.create()(EmployeeAwardEditTable)
