...
import styles from './ProductPart.search.less'
...
@Form.create()
export default class ProductPartSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const productPartIds = selectedRows.map((item) => { return item.id })
    console.log('productPartIds', productPartIds)
    const parameters = { productPartIds }
    dispatch({
      type: `${owner.type}/removeProductPartList`,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'productPart', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {ProductPartTable} = GlobalComponents;
    const {ProductPartConfirmationTable} = GlobalComponents;
    const {ProductPartSearchForm} = GlobalComponents;
...
<Menu.Item key="productComponent">关联产品组件</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ProductPartConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 产品零件列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ProductPartSearchForm {...this.props} />
            </div>
