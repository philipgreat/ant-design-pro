...
import styles from './BonusPoint.edittable.less'
...
class BonusPointEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {bonusPointId} = record.id
      const parameters = { ...record, bonusPointId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '点', debugtype: 'int', dataIndex: 'points', width: '8', render: (text, record) => renderStringEdit('points',text, record)  },
  { title: '用户', dataIndex: 'user', render: (text, record) => (record.user ? record.user.displayName : '暂无') },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newBonusPointToAppend  = {
      	'id':`+1`, 
				'name':'',
				'obtainTime':'',
				'points':'',
      ...
      newData.push(newBonusPointToAppend);
...
export default Form.create()(BonusPointEditTable)
