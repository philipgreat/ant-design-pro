...
class BonusPointSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bonusPointIds = selectedRows.map((item) => { return item.id })
    console.log('bonusPointIds', bonusPointIds)
    const parameters = { bonusPointIds }
    dispatch({
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bonusPoint', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'积分列表' } })
  ...
  render() {
    ...
    const {BonusPointTable} = GlobalComponents;
    const {BonusPointConfirmationTable} = GlobalComponents;
    const {BonusPointSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BonusPointConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 积分列表`}>
        <Card bordered={false}>
...
export default Form.create()(BonusPointSearch)
