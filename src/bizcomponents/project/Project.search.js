...
import styles from './Project.search.less'
...
export default class ProjectSearch extends PureComponent {
  ...
  handleDelete = () => {
    ...
    const projectIds = selectedRows.map((item) => { return item.id })
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'project' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'project', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {ProjectTable} = GlobalComponents;
    const {ProjectConfirmationTable} = GlobalComponents;
    const {ProjectSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ProjectConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
