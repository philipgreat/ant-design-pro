...
@Form.create()
export default class ViewSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const viewIds = selectedRows.map((item) => { return item.id })
    console.log('viewIds', viewIds)
    const parameters = { viewIds }
    dispatch({
      type: `${owner.type}/removeViewList`,
      payload: { id: owner.id, type: 'view', parameters },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'view', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {ViewConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ViewConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 观列表`}>
        <Card bordered={false}>
