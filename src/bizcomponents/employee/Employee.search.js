...
@Form.create()
export default class EmployeeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const employeeIds = selectedRows.map((item) => { return item.id })
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employee', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {EmployeeConfirmationTable} = GlobalComponents;
...
<Menu.Item key="department">关联部门</Menu.Item>
<Menu.Item key="occupation">关联职业</Menu.Item>
<Menu.Item key="responsibleFor">关联负责</Menu.Item>
<Menu.Item key="currentSalaryGrade">关联目前工资等级</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EmployeeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 员工列表`}>
        <Card bordered={false}>
