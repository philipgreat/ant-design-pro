...
class TaskReplyDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, taskReply } = this.props;
    
    if(!taskReply){
    	return;
    }
    const {displayName} = taskReply;
    if(!displayName){
    	return;
    }
  ...
  render() {
    ...
    const { id,displayName, taskReplyLikeCount } = this.props.taskReply
    const cardsData = {cardsName:"回复任务",cardsFor: "taskReply",cardsSource: this.props.taskReply,
  		subItems: [
