...
import styles from './TaskReply.edittable.less'
...
class TaskReplyEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
      ...
      return {replierId,taskId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const taskReplyIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {taskReplyId} = record.id
      const parameters = { ...record, taskReplyId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '内容', debugtype: 'string', dataIndex: 'content', width: '22', render: (text, record) => renderStringEdit('content',text, record)  },
  ...
  { title: '当前状态', debugtype: 'string', dataIndex: 'currentStatus', width: '19', render: (text, record) => renderStringEdit('currentStatus',text, record)  },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newTaskReplyToAppend  = {
      	'id':`+1`, 
				'replyTime':'',
				'content':'',
				'replier':'',
      ...
      newData.push(newTaskReplyToAppend);
...
export default Form.create()(TaskReplyEditTable)
