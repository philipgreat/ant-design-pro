...
class TaskReplyUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const taskReplyId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, taskReplyId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextTaskReplyUpdateRow`,
        payload: {
          id: owner.id,
          type: 'taskReply',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={
          '更新回复任务' + (currentUpdateIndex + 1) + '/' + selectedRows.length
        }
        content="更新回复任务"
