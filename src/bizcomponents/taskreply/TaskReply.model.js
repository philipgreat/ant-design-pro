...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        const editDetailMatch = pathToRegexp('/taskReply/:id/editDetail').exec(pathname)
        if (editDetailMatch) {
          const id = editDetailMatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const viewDetailMatch = pathToRegexp('/taskReply/:id/viewDetail').exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp('/taskReply/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *gotoUpdateForm({ payload }, { put }) {
      ...
      const location = { pathname: `/taskReply/${id}/list/${type}UpdateForm`, state }
      yield put(routerRedux.push(location))
    },
    *goback({ payload }, { put }) {
      const { id, type,listName } = payload
      yield put(routerRedux.push(`/taskReply/${id}/list/${type}List/${listName}`))
    },
    ...
    *addTaskReplyLike({ payload }, { call, put }) {
      ...
      const location = { pathname: `/taskReply/${id}/list/${type}List/任务回复点赞列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateTaskReplyLike({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(TaskReplyService.updateTaskReplyLike, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/taskReply/${id}/list/${type}List/任务回复点赞列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *removeTaskReplyLikeList({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(TaskReplyService.removeTaskReplyLikeList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
