...
class TaskReplySearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const taskReplyIds = selectedRows.map((item) => { return item.id })
    console.log('taskReplyIds', taskReplyIds)
    const parameters = { taskReplyIds }
    dispatch({
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'taskReply', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'回复任务列表' } })
  ...
  render() {
    ...
    const {TaskReplyTable} = GlobalComponents;
    const {TaskReplyConfirmationTable} = GlobalComponents;
    const {TaskReplySearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TaskReplyConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 回复任务列表`}>
        <Card bordered={false}>
...
export default Form.create()(TaskReplySearch)
