...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        const match = pathToRegexp('/vehicleServiceCompanyEmployee/:id/list/:listName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *goback({ payload }, { put }) {
      const { id, type } = payload
      yield put(routerRedux.push(`/vehicleServiceCompanyEmployee/${id}/list/${type}List`))
    },
    ...
    *updateEmployeeDrivingLicense({ payload }, { call, put }) {
      ...
      const location = { pathname: `/vehicleServiceCompanyEmployee/${id}/list/${type}List`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *addCompanyEmployeeMessage({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(VehicleServiceCompanyEmployeeService.addCompanyEmployeeMessage, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
    ...
    *updateCompanyEmployeeMessage({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      console.log('get form parameters', parameters)
      const data = yield call(VehicleServiceCompanyEmployeeService.updateCompanyEmployeeMessage, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
    ...
    *removeCompanyEmployeeMessageList({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(VehicleServiceCompanyEmployeeService.removeCompanyEmployeeMessageList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
