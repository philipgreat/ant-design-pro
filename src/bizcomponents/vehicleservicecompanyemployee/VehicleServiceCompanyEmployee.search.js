...
class VehicleServiceCompanyEmployeeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const vehicleServiceCompanyEmployeeIds = selectedRows.map((item) => { return item.id })
    console.log('vehicleServiceCompanyEmployeeIds', vehicleServiceCompanyEmployeeIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'vehicleServiceCompanyEmployee', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'商户员工列表' } })
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <VehicleServiceCompanyEmployeeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 商户员工列表`}>
        <Card bordered={false}>
...
export default Form.create()(VehicleServiceCompanyEmployeeSearch)
