...
import styles from './TaskResolving.search.less'
...
class TaskResolvingSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const taskResolvingIds = selectedRows.map((item) => { return item.id })
    console.log('taskResolvingIds', taskResolvingIds)
    const parameters = { taskResolvingIds }
    dispatch({
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'taskResolving' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'taskResolving', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'任务分解列表' } })
  ...
  render() {
    ...
    const {TaskResolvingTable} = GlobalComponents;
    const {TaskResolvingConfirmationTable} = GlobalComponents;
    const {TaskResolvingSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TaskResolvingConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 任务分解列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TaskResolvingSearchForm {...this.props} />
            </div>
...
export default Form.create()(TaskResolvingSearch)
