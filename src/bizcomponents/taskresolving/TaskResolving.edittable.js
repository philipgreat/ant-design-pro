...
import styles from './TaskResolving.edittable.less'
...
class TaskResolvingEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const taskResolvingIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addTaskResolving`,
        payload: {
          id: owner.id,
          type: 'taskResolving',
          parameters,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {taskResolvingId} = record.id
      const parameters = { ...record, taskResolvingId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateTaskResolving`,
        payload: {
          id: owner.id,
          type: 'taskResolving',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newTaskResolvingToAppend  = {
      	'id':`+1`, 
				'who':'',
				'actionTime':'',
				'comment':'',
      ...
      newData.push(newTaskResolvingToAppend);
...
export default Form.create()(TaskResolvingEditTable)
