...
import styles from './EmployeeSkill.edittable.less'
...
class EmployeeSkillEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
			...
			const skillTypeId = record.skillType.id

      //const communityId = record.community.id;
      return {employeeId,skillTypeId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const employeeSkillIds = [record.id];
      const parameters = { employeeSkillIds }
      dispatch({
        type: `${owner.type}/removeEmployeeSkillList`,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {employeeSkillId} = record.id
      const parameters = { ...record, employeeSkillId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '技能类型', dataIndex: 'skillType', render: (text, record) => (record.skillType ? record.skillType.displayName : '暂无') },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newEmployeeSkillToAppend  = {
      	'id':`+1`, 
				'employee':'',
				'skillType':'',
				'description':'',
      ...
      newData.push(newEmployeeSkillToAppend);
...
export default Form.create()(EmployeeSkillEditTable)
