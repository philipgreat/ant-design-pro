...
@Form.create()
export default class EmployeeSkillSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const employeeSkillIds = selectedRows.map((item) => { return item.id })
    console.log('employeeSkillIds', employeeSkillIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employeeSkill', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {EmployeeSkillConfirmationTable} = GlobalComponents;
...
<Menu.Item key="skillType">关联技能类型</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EmployeeSkillConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 员工技能列表`}>
        <Card bordered={false}>
