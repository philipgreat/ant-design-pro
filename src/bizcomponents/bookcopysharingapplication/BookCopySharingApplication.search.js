...
class BookCopySharingApplicationSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'书副本共享应用程序列表' } })
  ...
  render() {
    ...
    const {BookCopySharingApplicationTable} = GlobalComponents;
    const {BookCopySharingApplicationConfirmationTable} = GlobalComponents;
    const {BookCopySharingApplicationSearchForm} = GlobalComponents;
...
<Menu.Item key="destinationStore">关联目的地存储</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookCopySharingApplicationConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 书副本共享应用程序列表`}>
        <Card bordered={false}>
...
export default Form.create()(BookCopySharingApplicationSearch)
