...
@Form.create()
export default class EmployeeEducationSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const employeeEducationIds = selectedRows.map((item) => { return item.id })
    console.log('employeeEducationIds', employeeEducationIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employeeEducation' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employeeEducation', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {EmployeeEducationConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EmployeeEducationConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 员工教育列表`}>
        <Card bordered={false}>
