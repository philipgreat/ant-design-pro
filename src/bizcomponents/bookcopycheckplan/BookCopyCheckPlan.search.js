...
import styles from './BookCopyCheckPlan.search.less'
...
class BookCopyCheckPlanSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookCopyCheckPlanIds = selectedRows.map((item) => { return item.id })
    console.log('bookCopyCheckPlanIds', bookCopyCheckPlanIds)
    const parameters = { bookCopyCheckPlanIds }
    dispatch({
      type: `${owner.type}/removeBookCopyCheckPlanList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookCopyCheckPlan' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookCopyCheckPlan', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'书副本检查计划列表' } })
  ...
  render() {
    ...
    const {BookCopyCheckPlanTable} = GlobalComponents;
    const {BookCopyCheckPlanConfirmationTable} = GlobalComponents;
    const {BookCopyCheckPlanSearchForm} = GlobalComponents;
...
<Menu.Item key="checkStore">关联检查存储</Menu.Item>
<Menu.Item key="planCreator">关联计划的创造者</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookCopyCheckPlanConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 书副本检查计划列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookCopyCheckPlanSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookCopyCheckPlanSearch)
