...
import styles from './BookCopyCheckPlan.edittable.less'
...
class BookCopyCheckPlanEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
			const checkStoreId = record.checkStore.id
			const planCreatorId = record.planCreator.id
			const bookManagementId = record.bookManagement.id

      //const communityId = record.community.id;
      return {checkStoreId,planCreatorId,bookManagementId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const bookCopyCheckPlanIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {bookCopyCheckPlanId} = record.id
      const parameters = { ...record, bookCopyCheckPlanId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateBookCopyCheckPlan`,
        payload: {
          id: owner.id,
          type: 'bookCopyCheckPlan',
          parameters,
  ...
  { title: '检查计划的名字', debugtype: 'string', dataIndex: 'checkPlanName', width: '10', render: (text, record) => renderStringEdit('checkPlanName',text, record)  },
  ...
  { title: '检查计划状态', debugtype: 'string', dataIndex: 'checkPlanStatus', width: '6', render: (text, record) => renderStringEdit('checkPlanStatus',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newBookCopyCheckPlanToAppend  = {
      	'id':`+1`, 
				'checkPlanName':'',
				'checkStore':'',
				'planDatetime':'',
      ...
      newData.push(newBookCopyCheckPlanToAppend);
...
export default Form.create()(BookCopyCheckPlanEditTable)
