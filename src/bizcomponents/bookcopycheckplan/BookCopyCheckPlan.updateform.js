...
import styles from './BookCopyCheckPlan.updateform.less'
...
class BookCopyCheckPlanUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        planDatetime: moment(item.planDatetime),
        planUpdateDatetime: moment(item.planUpdateDatetime),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const bookCopyCheckPlanId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, bookCopyCheckPlanId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextBookCopyCheckPlanUpdateRow`,
        payload: {
          id: owner.id,
          type: 'bookCopyCheckPlan',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新书副本检查计划"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新书副本检查计划"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.checkPlanName} {...formItemLayout}>
                  {getFieldDecorator('checkPlanName', {
                    initialValue: selectedRow.checkPlanName,
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.planDatetime} {...formItemLayout}>
                  {getFieldDecorator('planDatetime', {
                    initialValue: selectedRow.planDatetime,
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.checkPlanStatus} {...formItemLayout}>
                  {getFieldDecorator('checkPlanStatus', {
                    initialValue: selectedRow.checkPlanStatus,
...
}))(Form.create()(BookCopyCheckPlanUpdateForm))
