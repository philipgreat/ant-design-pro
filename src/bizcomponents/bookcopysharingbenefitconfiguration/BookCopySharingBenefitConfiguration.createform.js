...
import styles from './BookCopySharingBenefitConfiguration.createform.less'
...
class BookCopySharingBenefitConfigurationCreateForm extends Component {
  state = {
  ...
  executeCandidateBookManagementSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = BookCopySharingBenefitConfigurationService.requestCandidateBookManagement("bookManagement", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addBookCopySharingBenefitConfiguration`,
          payload: { id: owner.id, type: 'bookCopySharingBenefitConfiguration', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'bookCopySharingBenefitConfiguration',listName:'图书拷贝共享利益配置。列表' },
      })
    }
    ...
    return (
      <PageHeaderLayout
        title="新建一个图书拷贝共享利益配置。"
        content="新建一个图书拷贝共享利益配置。"
...
}))(Form.create()(BookCopySharingBenefitConfigurationCreateForm))
