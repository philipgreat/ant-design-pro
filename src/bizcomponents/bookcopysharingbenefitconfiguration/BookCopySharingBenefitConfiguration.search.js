...
import styles from './BookCopySharingBenefitConfiguration.search.less'
...
class BookCopySharingBenefitConfigurationSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookCopySharingBenefitConfigurationIds = selectedRows.map((item) => { return item.id })
    console.log('bookCopySharingBenefitConfigurationIds', bookCopySharingBenefitConfigurationIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookCopySharingBenefitConfiguration' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookCopySharingBenefitConfiguration', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'图书拷贝共享利益配置。列表' } })
  ...
  render() {
    ...
    const {BookCopySharingBenefitConfigurationTable} = GlobalComponents;
    const {BookCopySharingBenefitConfigurationConfirmationTable} = GlobalComponents;
    const {BookCopySharingBenefitConfigurationSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookCopySharingBenefitConfigurationConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 图书拷贝共享利益配置。列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookCopySharingBenefitConfigurationSearchForm {...this.props} />
            </div>
            ...
            <BookCopySharingBenefitConfigurationTable
...
export default Form.create()(BookCopySharingBenefitConfigurationSearch)
