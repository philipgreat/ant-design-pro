...
import styles from './MemberAccountRechargeSku.edittable.less'
...
class MemberAccountRechargeSkuEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
			const memberAccountRechargeProductId = record.memberAccountRechargeProduct.id

      //const communityId = record.community.id;
      return {memberAccountRechargeProductId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const memberAccountRechargeSkuIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {memberAccountRechargeSkuId} = record.id
      const parameters = { ...record, memberAccountRechargeSkuId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateMemberAccountRechargeSku`,
        payload: {
          id: owner.id,
          type: 'memberAccountRechargeSku',
          parameters,
  ...
  { title: '描述', debugtype: 'string', dataIndex: 'description', width: '7', render: (text, record) => renderStringEdit('description',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newMemberAccountRechargeSkuToAppend  = {
      	'id':`+1`, 
				'name':'',
				'description':'',
				'listPrice':'',
				'salePrice':'',
				'memberAccountRechargeProduct':'',


      };
      const newData = data ? [...data]:[];
      newData.push(newMemberAccountRechargeSkuToAppend);
...
export default Form.create()(MemberAccountRechargeSkuEditTable)
