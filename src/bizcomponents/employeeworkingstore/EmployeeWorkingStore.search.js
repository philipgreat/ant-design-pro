...
import styles from './EmployeeWorkingStore.search.less'
...
class EmployeeWorkingStoreSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const employeeWorkingStoreIds = selectedRows.map((item) => { return item.id })
    console.log('employeeWorkingStoreIds', employeeWorkingStoreIds)
    const parameters = { employeeWorkingStoreIds }
    dispatch({
      type: `${owner.type}/removeEmployeeWorkingStoreList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employeeWorkingStore' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employeeWorkingStore', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'员工工作的商店列表' } })
  ...
  render() {
    ...
    const {EmployeeWorkingStoreTable} = GlobalComponents;
    const {EmployeeWorkingStoreConfirmationTable} = GlobalComponents;
    const {EmployeeWorkingStoreSearchForm} = GlobalComponents;
...
<Menu.Item key="store">关联商店</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EmployeeWorkingStoreConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 员工工作的商店列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <EmployeeWorkingStoreSearchForm {...this.props} />
            </div>
...
export default Form.create()(EmployeeWorkingStoreSearch)
