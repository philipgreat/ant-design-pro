...
import styles from './EmployeeWorkingStore.edittable.less'
...
class EmployeeWorkingStoreEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
      ...
      return {employeeId,storeId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const employeeWorkingStoreIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {employeeWorkingStoreId} = record.id
      const parameters = { ...record, employeeWorkingStoreId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateEmployeeWorkingStore`,
        payload: {
          id: owner.id,
          type: 'employeeWorkingStore',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newEmployeeWorkingStoreToAppend  = {
      	'id':`+1`, 
				'description':'',
				'employee':'',
				'store':'',
      ...
      newData.push(newEmployeeWorkingStoreToAppend);
...
export default Form.create()(EmployeeWorkingStoreEditTable)
