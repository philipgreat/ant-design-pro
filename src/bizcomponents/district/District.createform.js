...
import styles from './District.createform.less'
...
class DistrictCreateForm extends Component {
  ...
  executeCandidateCitySearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = DistrictService.requestCandidateCity("city", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addDistrict`,
          payload: { id: owner.id, type: 'district', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'district',listName:'区列表' },
      })
    }
...
}))(Form.create()(DistrictCreateForm))
