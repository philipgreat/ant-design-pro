...
import styles from './District.edittable.less'
...
class DistrictEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      ...
      const parameters = { ...record, districtId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateDistrict`,
        payload: {
          id: owner.id,
          type: 'district',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newDistrictToAppend);
...
export default Form.create()(DistrictEditTable)
