...
class DistrictSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const districtIds = selectedRows.map((item) => { return item.id })
    console.log('districtIds', districtIds)
    const parameters = { districtIds }
    dispatch({
      type: `${owner.type}/removeDistrictList`,
      payload: { id: owner.id, type: 'district', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'district' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'district', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'区列表' } })
  ...
  render() {
    ...
    const {DistrictTable} = GlobalComponents;
    const {DistrictConfirmationTable} = GlobalComponents;
    const {DistrictSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <DistrictConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 区列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <DistrictSearchForm {...this.props} />
            </div>
