...
import styles from './GameSession.search.less'
...
class GameSessionSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const gameSessionIds = selectedRows.map((item) => { return item.id })
    console.log('gameSessionIds', gameSessionIds)
    const parameters = { gameSessionIds }
    dispatch({
      type: `${owner.type}/removeGameSessionList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'gameSession' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'gameSession', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'游戏场次列表' } })
  ...
  render() {
    ...
    const {GameSessionTable} = GlobalComponents;
    const {GameSessionConfirmationTable} = GlobalComponents;
    const {GameSessionSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <GameSessionConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 游戏场次列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <GameSessionSearchForm {...this.props} />
            </div>
...
export default Form.create()(GameSessionSearch)
