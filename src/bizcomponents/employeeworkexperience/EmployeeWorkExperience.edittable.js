...
import styles from './EmployeeWorkExperience.edittable.less'
...
class EmployeeWorkExperienceEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const employeeWorkExperienceIds = [record.id];
      const parameters = { employeeWorkExperienceIds }
      dispatch({
        type: `${owner.type}/removeEmployeeWorkExperienceList`,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {employeeWorkExperienceId} = record.id
      const parameters = { ...record, employeeWorkExperienceId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '公司', debugtype: 'string', dataIndex: 'company', width: '12', render: (text, record) => renderStringEdit('company',text, record)  },
  { title: '描述', debugtype: 'string', dataIndex: 'description', width: '25', render: (text, record) => renderStringEdit('description',text, record)  },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newEmployeeWorkExperienceToAppend  = {
      	'id':`+1`, 
				'employee':'',
				'start':'',
				'end':'',
      ...
      newData.push(newEmployeeWorkExperienceToAppend);
...
export default Form.create()(EmployeeWorkExperienceEditTable)
