...
const columns = [
  {
    ...
    width: '20',
    render: (text, record) => (
      <Link to={`/secUser/${text}/dashboard`}>{text}</Link>
    ),
  },
  ...
  },
  {
    ...
    render: (text, record) => moment(record.lastLoginTime).format('YYYY-MM-DD'),
  },
  {
    title: '域',
    dataIndex: 'domain',
    render: (text, record) =>
      record.domain ? (
        <Link to={`/userDomain/${record.domain.id}/dashboard`}>
          {record.domain.id}
        </Link>
      ) : (
        '暂无'
  ...
  },
  {
    ...
    render: (text, record) =>
      record.blocking ? (
        <Link to={`/secUserBlocking/${record.blocking.id}/dashboard`}>
          {record.blocking.id}
        </Link>
      ) : (
        '暂无'
