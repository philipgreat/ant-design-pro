...
import SecUserTable from './SecUser.table';
import SecUserConfirmationTable from './SecUser.confirmmationtable';

import SecUserSearchForm from './SecUser.searchform';
...
@Form.create()
export default class SecUserSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    const params = {
      secUserList: 1,
      secUserListCurrentPage: pagination.current,
      secUserListRowsPerPage: pagination.pageSize,
      ...formValues,
      ...filters,
    };
    if (sorter.field) {
  ...
  handleDelete = () => {
    ...
    const secUserIds = selectedRows.map((item) => { return item.id; });
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'secUser', selectedRows, currentUpdateIndex },
    });
  }
