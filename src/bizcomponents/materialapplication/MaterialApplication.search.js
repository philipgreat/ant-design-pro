...
import styles from './MaterialApplication.search.less'
...
@Form.create()
export default class MaterialApplicationSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const materialApplicationIds = selectedRows.map((item) => { return item.id })
    console.log('materialApplicationIds', materialApplicationIds)
    const parameters = { materialApplicationIds }
    dispatch({
      type: `${owner.type}/removeMaterialApplicationList`,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'materialApplication', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {MaterialApplicationTable} = GlobalComponents;
    const {MaterialApplicationConfirmationTable} = GlobalComponents;
    const {MaterialApplicationSearchForm} = GlobalComponents;
...
<Menu.Item key="material">关联材料</Menu.Item>
<Menu.Item key="productPart">关联产品零件</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <MaterialApplicationConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 材料的应用列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <MaterialApplicationSearchForm {...this.props} />
            </div>
