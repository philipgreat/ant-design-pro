...
import styles from './MaterialApplication.edittable.less'
...
class MaterialApplicationEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
			...
			const productPartId = record.productPart.id

      //const communityId = record.community.id;
      return {materialId,productPartId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const materialApplicationIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {materialApplicationId} = record.id
      const parameters = { ...record, materialApplicationId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateMaterialApplication`,
        payload: {
          id: owner.id,
          type: 'materialApplication',
          parameters,
  ...
  { title: '数量', debugtype: 'int', dataIndex: 'quantity', width: '5', render: (text, record) => renderStringEdit('quantity',text, record)  },
  { title: '产品零件', dataIndex: 'productPart', render: (text, record) => (record.productPart ? record.productPart.displayName : '暂无') },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newMaterialApplicationToAppend  = {
      	'id':`+1`, 
				'name':'',
				'material':'',
				'quantity':'',
      ...
      newData.push(newMaterialApplicationToAppend);
...
export default Form.create()(MaterialApplicationEditTable)
