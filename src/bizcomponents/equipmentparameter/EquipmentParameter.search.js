...
import styles from './EquipmentParameter.search.less'
...
@Form.create()
export default class EquipmentParameterSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const equipmentParameterIds = selectedRows.map((item) => { return item.id })
    console.log('equipmentParameterIds', equipmentParameterIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'equipmentParameter' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'equipmentParameter', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {EquipmentParameterTable} = GlobalComponents;
    const {EquipmentParameterConfirmationTable} = GlobalComponents;
    const {EquipmentParameterSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EquipmentParameterConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="设备参数列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <EquipmentParameterSearchForm {...this.props} />
            </div>
