...
import styles from './VehicleInspectionOrderPayment.edittable.less'
...
class VehicleInspectionOrderPaymentEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const vehicleInspectionOrderPaymentIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {vehicleInspectionOrderPaymentId} = record.id
      const parameters = { ...record, vehicleInspectionOrderPaymentId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '付款状态', debugtype: 'string', dataIndex: 'paymentStatus', width: '8', render: (text, record) => renderStringEdit('paymentStatus',text, record)  },
  { title: '主订单', dataIndex: 'mainOrder', render: (text, record) => (record.mainOrder ? record.mainOrder.id : '暂无') },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newVehicleInspectionOrderPaymentToAppend  = {
      	'id':`+1`, 
				'paymentMethod':'',
				'paymentAmount':'',
				'paymentStatus':'',
      ...
      newData.push(newVehicleInspectionOrderPaymentToAppend);
...
export default Form.create()(VehicleInspectionOrderPaymentEditTable)
