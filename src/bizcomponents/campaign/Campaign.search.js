...
import styles from './Campaign.search.less'
...
class CampaignSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const campaignIds = selectedRows.map((item) => { return item.id })
    console.log('campaignIds', campaignIds)
    const parameters = { campaignIds }
    dispatch({
      type: `${owner.type}/removeCampaignList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'campaign' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'campaign', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'运动列表' } })
  ...
  render() {
    ...
    const {CampaignTable} = GlobalComponents;
    const {CampaignConfirmationTable} = GlobalComponents;
    const {CampaignSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CampaignConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 运动列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CampaignSearchForm {...this.props} />
            </div>
...
export default Form.create()(CampaignSearch)
