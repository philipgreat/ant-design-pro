...
import styles from './Campaign.dashboard.less'
...
const imageListOf = (campaign) =>{

  const imageList = [
	   {"title":'活动封面图片',"imageLocation":campaign.campaignCoverImage},
]
...
const summaryOf = (campaign) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{campaign.id}</Description> 
<Description term="活动名称">{campaign.campaignName}</Description> 
<Description term="活动开始日期时间">{ moment(campaign.campaignStartDatetime).format('YYYY-MM-DD')}</Description> 
<Description term="活动结束日期时间">{ moment(campaign.campaignEndDatetime).format('YYYY-MM-DD')}</Description> 
...
class CampaignDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, campaign } = this.props;
    
    if(!campaign){
    	return;
    }
    const {displayName} = campaign;
    if(!displayName){
    	return;
    }
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.campaign
    const cardsData = {cardsName:"运动",cardsFor: "campaign",cardsSource: this.props.campaign,
  		subItems: [
