...
import styles from './AvailableVehicleUseCharacter.edittable.less'
...
class AvailableVehicleUseCharacterEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const availableVehicleUseCharacterIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {availableVehicleUseCharacterId} = record.id
      const parameters = { ...record, availableVehicleUseCharacterId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '别名', debugtype: 'string', dataIndex: 'aliasName', width: '38', render: (text, record) => renderStringEdit('aliasName',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newAvailableVehicleUseCharacterToAppend  = {
      	'id':`+1`, 
				'name':'',
				'aliasName':'',
				'canDoExempt':'',
      ...
      newData.push(newAvailableVehicleUseCharacterToAppend);
...
export default Form.create()(AvailableVehicleUseCharacterEditTable)
