...
import styles from './Player.dashboard.less'
...
const summaryOf = (player) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{player.id}</Description> 
<Description term="昵称">{player.nickName}</Description> 
<Description term="手机号码">{player.mobile}</Description> 
<Description term="密码">{player.password}</Description> 
<Description term="经验值">{player.experienceValue}</Description> 
...
class PlayerDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, player } = this.props;
  ...
  render() {
    ...
    const { id,displayName, gamePlayerRecordCount, gameTokenCount } = this.props.player
    const cardsData = {cardsName:"玩家",cardsFor: "player",cardsSource: this.props.player,
  		subItems: [
