...
import styles from './Player.search.less'
...
class PlayerSearch extends PureComponent {
  ...
  handleDelete = () => {
    ...
    const playerIds = selectedRows.map((item) => { return item.id })
    console.log('playerIds', playerIds)
    const parameters = { playerIds }
    dispatch({
      type: `${owner.type}/removePlayerList`,
      payload: { id: owner.id, type: 'player', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'player' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'player', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'玩家列表' } })
  ...
  render() {
    ...
    const {PlayerTable} = GlobalComponents;
    const {PlayerConfirmationTable} = GlobalComponents;
    const {PlayerSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <PlayerConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 玩家列表`}>
        <Card bordered={false}>
...
export default Form.create()(PlayerSearch)
