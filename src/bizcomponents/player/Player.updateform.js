...
import styles from './Player.updateform.less'
...
class PlayerUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const playerId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, playerId, ...imagesValues }

        // const newIndex= currentUpdateIndex + 1
        dispatch({
          type: `${owner.type}/updatePlayer`,
          payload: {
            id: owner.id,
            type: 'player',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextPlayerUpdateRow`,
        payload: {
          id: owner.id,
          type: 'player',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新玩家"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新玩家"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.password} {...formItemLayout}>
                  {getFieldDecorator('password', {
                    initialValue: selectedRow.password,
