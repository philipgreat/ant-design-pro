...
@Form.create()
export default class TerminationTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const terminationTypeIds = selectedRows.map((item) => { return item.id })
    console.log('terminationTypeIds', terminationTypeIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'terminationType' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'terminationType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {TerminationTypeConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TerminationTypeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 雇佣终止类型列表`}>
        <Card bordered={false}>
