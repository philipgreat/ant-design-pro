...
@Form.create()
export default class PayingOffSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const payingOffIds = selectedRows.map((item) => { return item.id })
    console.log('payingOffIds', payingOffIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'payingOff', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {PayingOffConfirmationTable} = GlobalComponents;
...
<Menu.Item key="paidFor">关联支付</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <PayingOffConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 工资支付列表`}>
        <Card bordered={false}>
