...
export default class PayingOffDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, payingOff } = this.props;
    
    if(!payingOff){
    	return;
    }
    const {displayName} = payingOff;
    if(!displayName){
    	return;
    }
        ...
        <div>
          <Row gutter={24}>
            ...
            <Card title={`工资单(${numeral(employeeSalarySheetCount).format('工资单0,0')})`}  style={{ width: 180 }}>
              ...
              <p><Link to={`/payingOff/${id}/list/employeeSalarySheetList/工资单列表`}><FontAwesome name="gear"  />&nbsp;管理</Link></p>
              <p><Link to={`/payingOff/${id}/list/employeeSalarySheetCreateForm`}><FontAwesome name="plus"  />&nbsp;新增</Link></p>
