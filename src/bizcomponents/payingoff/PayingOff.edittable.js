...
import styles from './PayingOff.edittable.less'
...
class PayingOffEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
			const paidForId = record.paidFor.id
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const payingOffIds = [record.id];
      const parameters = { payingOffIds }
      dispatch({
        type: `${owner.type}/removePayingOffList`,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {payingOffId} = record.id
      const parameters = { ...record, payingOffId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '谁', debugtype: 'string', dataIndex: 'who', width: '6', render: (text, record) => renderStringEdit('who',text, record)  },
  { title: '支付', dataIndex: 'paidFor', render: (text, record) => (record.paidFor ? record.paidFor.displayName : '暂无') },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newPayingOffToAppend);
...
export default Form.create()(PayingOffEditTable)
