...
@Form.create()
export default class LeaveTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const leaveTypeIds = selectedRows.map((item) => { return item.id })
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'leaveType' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'leaveType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {LeaveTypeConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <LeaveTypeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 请假类型列表`}>
        <Card bordered={false}>
