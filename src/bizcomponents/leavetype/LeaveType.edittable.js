...
import styles from './LeaveType.edittable.less'
...
class LeaveTypeEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const leaveTypeIds = [record.id];
      const parameters = { leaveTypeIds }
      dispatch({
        type: `${owner.type}/removeLeaveTypeList`,
        payload: { id: owner.id, type: 'leaveType', parameters },
      })
    }
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {leaveTypeId} = record.id
      const parameters = { ...record, leaveTypeId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newLeaveTypeToAppend);
...
export default Form.create()(LeaveTypeEditTable)
