...
class ServiceInsuranceForInspectionSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const serviceInsuranceForInspectionIds = selectedRows.map((item) => { return item.id })
    console.log('serviceInsuranceForInspectionIds', serviceInsuranceForInspectionIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'serviceInsuranceForInspection', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'保险服务列表' } })
  ...
  render() {
    ...
    const {ServiceInsuranceForInspectionTable} = GlobalComponents;
    const {ServiceInsuranceForInspectionConfirmationTable} = GlobalComponents;
    const {ServiceInsuranceForInspectionSearchForm} = GlobalComponents;
...
<Menu.Item key="orderedInsurance">关联选定保险</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ServiceInsuranceForInspectionConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 保险服务列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ServiceInsuranceForInspectionSearchForm {...this.props} />
            </div>
...
export default Form.create()(ServiceInsuranceForInspectionSearch)
