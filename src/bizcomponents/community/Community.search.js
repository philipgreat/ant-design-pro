...
class CommunitySearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'社区列表' } })
  ...
  render() {
    ...
    const {CommunityTable} = GlobalComponents;
    const {CommunityConfirmationTable} = GlobalComponents;
    const {CommunitySearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 社区列表`}>
        <Card bordered={false}>
...
export default Form.create()(CommunitySearch)
