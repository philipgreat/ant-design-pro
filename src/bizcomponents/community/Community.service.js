...
const updateCommunityUser = (targetObjectId, parameters) => {
  const url = `${PREFIX}communityManager/updateCommunityUserProperties/communityId/id/mobile/nickName/gender/userType/avatar/birthday/experiencePoint/bonusPoint/city/status/hideInfo/administrator/experiencePointLimit/experiencePointRemain/experiencePointLastDate/tokensExpr/`
  const communityId = targetObjectId
  const requestParameters = { ...parameters, communityId, tokensExpr: 'none' }
  const headers = { 'Content-Type': 'application/x-www-form-urlencoded' }
  return post({
