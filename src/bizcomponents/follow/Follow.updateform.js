...
import styles from './Follow.updateform.less';
...
class FollowUpdateForm extends PureComponent {
 ...
 mapBackToImageValues(convertedImagesValues) {
  ...
  getSelectedRow(){
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
			addingTime: moment(item.addingTime).format('YYYY-MM-DD'),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
     validateFieldsAndScroll((error, values) => {
        ...
        const followId = values.id;
        const imagesValues = this.mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values,followId, ...imagesValues };

	    const newIndex= currentUpdateIndex+1;
        dispatch({
          type: owner.type+'/updateFollow',
    ...
    const skipToNext = () => {
      ...
      const newIndex= currentUpdateIndex+1;
      dispatch({
          type: owner.type+'/gotoNextFollowUpdateRow',
            payload: {
              id:owner.id,type:'follow', 
    ...
    return (
      <PageHeaderLayout
        ...
        title={"更新关注"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新关注"
        ...
        <Card title="基础信息" className={styles.card} bordered={false}>
          <Form layout="vertical" hideRequiredMark>
            <Row gutter={16}>
             ...
             <Col lg={6} md={12} sm={24}>
                <Form.Item label={fieldLabels.followId}>
                  {getFieldDecorator('followId', {
                    rules: [{ required: true, message: '请输入关注的社区用户' }],
                  })(
                    <Input placeholder="请输入请输入关注的社区用户string" />
                  )}
                </Form.Item>
              </Col>			
