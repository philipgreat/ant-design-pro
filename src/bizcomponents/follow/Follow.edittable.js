...
import styles from './Follow.edittable.less'
...
class FollowEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {followId} = record.id
      const parameters = { ...record, followId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '关注的社区用户', debugtype: 'string', dataIndex: 'followId', width: '12', render: (text, record) => renderStringEdit('followId',text, record)  },
  { title: '添加时间', dataIndex: 'addingTime', render: (text, record) => moment(record.addingTime).format('YYYY-MM-DD HH:mm:ss') },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newFollowToAppend  = {
      	'id':`+1`, 
				'user':'',
				'followId':'',
				'addingTime':'',
      ...
      newData.push(newFollowToAppend);
...
export default Form.create()(FollowEditTable)
