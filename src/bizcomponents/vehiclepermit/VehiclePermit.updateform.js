...
import styles from './VehiclePermit.updateform.less'
...
class VehiclePermitUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const vehiclePermitId = values.id
        const imagesValues = this.mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, vehiclePermitId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextVehiclePermitUpdateRow`,
        payload: {
          id: owner.id,
          type: 'vehiclePermit',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新行驶证"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新行驶证"
...
}))(Form.create()(VehiclePermitUpdateForm))
