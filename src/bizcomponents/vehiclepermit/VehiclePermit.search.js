...
import styles from './VehiclePermit.search.less'
...
@Form.create()
export default class VehiclePermitSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    const params = {
      vehiclePermitList: 1,
      vehiclePermitListCurrentPage: pagination.current,
      vehiclePermitListRowsPerPage: pagination.pageSize,
      ...formValues,
      ...filters,
    }
    if (sorter.field) {
  ...
  handleDelete = () => {
    ...
    const vehiclePermitIds = selectedRows.map((item) => { return item.id })
    console.log('vehiclePermitIds', vehiclePermitIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'vehiclePermit', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {VehiclePermitTable} = GlobalComponents;
    const {VehiclePermitConfirmationTable} = GlobalComponents;
    const {VehiclePermitSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <VehiclePermitConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="行驶证列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <VehiclePermitSearchForm {...this.props} />
            </div>
