...
class MainOrderAccountSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const mainOrderAccountIds = selectedRows.map((item) => { return item.id })
    console.log('mainOrderAccountIds', mainOrderAccountIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'mainOrderAccount', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'年检订单对账单列表' } })
  ...
  render() {
    ...
    const {MainOrderAccountTable} = GlobalComponents;
    const {MainOrderAccountConfirmationTable} = GlobalComponents;
    const {MainOrderAccountSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <MainOrderAccountConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 年检订单对账单列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <MainOrderAccountSearchForm {...this.props} />
            </div>
...
export default Form.create()(MainOrderAccountSearch)
