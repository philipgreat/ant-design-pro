...
const columns = [
  ...
  },
  {
    title: '商户优惠',
    debugtype: 'string',
    dataIndex: 'orderMerchantDiscount',
    width: '9',
  },
  {
    title: '客户付款总金额',
  ...
  },
  {
    ...
    render: (text, record) =>
      moment(record.orderPlacedDatetime).format('YYYY-MM-DD'),
  },
  {
    title: '付款时间',
    dataIndex: 'orderPaymentDatetime',
    render: (text, record) =>
      moment(record.orderPaymentDatetime).format('YYYY-MM-DD'),
  },
  {
    title: '订单完成时间',
    dataIndex: 'orderFinishedDatetime',
    render: (text, record) =>
      moment(record.orderFinishedDatetime).format('YYYY-MM-DD'),
  },
  {
    title: '年检订单ID',
    debugtype: 'string',
