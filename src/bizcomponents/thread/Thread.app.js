  ...
  getNavMenuItems = objectId => {
    return (
      ...
      >
        ...
        <Menu.Item>
          <Link to={`/thread/${objectId}/list/threadRegistrationList`}>
            活动注册
          </Link>
        </Menu.Item>
        <Menu.Item>
  ...
  getThreadRegistrationSearch = () => {
    ...
    return connect(state => ({
      ...
      searchFormParameters:
        state._thread.threadRegistrationSearchFormParameters,
      loading: state._thread.loading,
      owner: {
        type: '_thread',
        id: state._thread.id,
    ...
    const layout = (
      <Layout>
        ...
        >
          ...
          >
            <Menu.Item>
              <Link to={`/thread/${this.props.thread.id}/dashboard`}>
                <Icon type="dashboard" />
                <span>仪表板</span>
              </Link>
            </Menu.Item>
        ...
        <Layout>
          <Content style={{ margin: '24px 24px 0', height: '100%' }}>
            <Switch>
              ...
              <Route
                path="/thread/:id/editDetail"
                component={ThreadEditDetail}
              />
              <Route
                path="/thread/:id/viewDetail"
                component={ThreadViewDetail}
              />

              <Route
                path="/thread/:id/list/threadReplyList"
                component={this.getThreadReplySearch()}
              />
              <Route
                path="/thread/:id/list/threadReplyCreateForm"
                component={this.getThreadReplyCreateForm()}
              />
              <Route
                path="/thread/:id/list/threadReplyUpdateForm"
                component={this.getThreadReplyUpdateForm()}
              />

              <Route
                path="/thread/:id/list/threadRegistrationList"
                component={this.getThreadRegistrationSearch()}
              />
              <Route
                path="/thread/:id/list/threadRegistrationCreateForm"
                component={this.getThreadRegistrationCreateForm()}
              />
              <Route
                path="/thread/:id/list/threadRegistrationUpdateForm"
                component={this.getThreadRegistrationUpdateForm()}
              />

              <Route
                path="/thread/:id/list/threadLikeList"
                component={this.getThreadLikeSearch()}
              />
              <Route
                path="/thread/:id/list/threadLikeCreateForm"
                component={this.getThreadLikeCreateForm()}
              />
              <Route
                path="/thread/:id/list/threadLikeUpdateForm"
