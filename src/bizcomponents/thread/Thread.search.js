...
class ThreadSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const threadIds = selectedRows.map((item) => { return item.id })
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'thread', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'主贴列表' } })
  ...
  render() {
    ...
    const {ThreadTable} = GlobalComponents;
    const {ThreadConfirmationTable} = GlobalComponents;
    const {ThreadSearchForm} = GlobalComponents;
...
<Menu.Item key="creator">关联创建者</Menu.Item>
<Menu.Item key="homePage">关联主页</Menu.Item>
<Menu.Item key="groupPage">关联群组页面</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ThreadConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 主贴列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ThreadSearchForm {...this.props} />
            </div>
...
export default Form.create()(ThreadSearch)
