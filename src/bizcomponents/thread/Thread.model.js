...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        const match = pathToRegexp('/thread/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *goback({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/thread/${id}/list/${type}List/${listName}`))
    },
    ...
    *addThreadReply({ payload }, { call, put }) {
      ...
      const location = { pathname: `/thread/${id}/list/${type}List/跟帖回复列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateThreadReply({ payload }, { call, put }) {
      ...
      const location = { pathname: `/thread/${id}/list/${type}List/跟帖回复列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *addThreadRegistration({ payload }, { call, put }) {
      ...
      const partialList = true
      const newState = {...data, partialList}
      const location = { pathname: `/thread/${id}/list/${type}List/活动注册列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateThreadRegistration({ payload }, { call, put }) {
      ...
      const location = { pathname: `/thread/${id}/list/${type}List/活动注册列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *addThreadLike({ payload }, { call, put }) {
      ...
      const partialList = true
      const newState = {...data, partialList}
      const location = { pathname: `/thread/${id}/list/${type}List/主贴点赞列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateThreadLike({ payload }, { call, put }) {
      ...
      const location = { pathname: `/thread/${id}/list/${type}List/主贴点赞列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
