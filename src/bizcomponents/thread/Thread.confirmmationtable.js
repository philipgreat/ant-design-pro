...
const columns = [
  ...
  },
  {
    ...
    render: (text, record) =>
      moment(record.registrationStopTime).format('YYYY-MM-DD'),
  },
  {
    title: '事件的位置',
    debugtype: 'string',
  ...
  },
  {
    title: '创建者',
    dataIndex: 'creator',
    render: (text, record) => (record.creator ? record.creator.id : '暂无'),
  },
  {
    title: '主页',
    dataIndex: 'homePage',
  ...
  },
  {
    ...
    render: (text, record) => (
      <ImagePreview imageLocation={record.封面图像路径1} />
    ),
  },
  {
    title: '封面图像路径2',
    ...
    render: (text, record) => (
      <ImagePreview imageLocation={record.封面图像路径2} />
    ),
  },
  {
    title: '封面图像路径3',
    ...
    render: (text, record) => (
      <ImagePreview imageLocation={record.封面图像路径3} />
    ),
  },
  {
    title: '图1',
  ...
  { title: '内容', debugtype: 'string', dataIndex: 'content', width: '107' },
  {
    title: '验收',
    dataIndex: 'approval',
    render: (text, record) => (record.approval ? record.approval.id : '暂无'),
  },
  {
    title: '取消',
    dataIndex: 'canceling',
    render: (text, record) => (record.canceling ? record.canceling.id : '暂无'),
  },
  {
    title: '完成',
    dataIndex: 'completion',
    render: (text, record) =>
      record.completion ? record.completion.id : '暂无',
  },
  {
    title: '躲藏',
    dataIndex: 'hiding',
    render: (text, record) => (record.hiding ? record.hiding.id : '暂无'),
  },
  {
    title: '当前用户已点赞',
