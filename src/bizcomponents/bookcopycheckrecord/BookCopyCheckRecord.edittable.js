...
import styles from './BookCopyCheckRecord.edittable.less'
...
class BookCopyCheckRecordEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
			...
			const bookCopyCheckStatusId = record.bookCopyCheckStatus.id
			const checkProcessEmployeeId = record.checkProcessEmployee.id
			const bookCopyCheckPlanId = record.bookCopyCheckPlan.id

      //const communityId = record.community.id;
      return {bookCopyId,bookCopyCheckStatusId,checkProcessEmployeeId,bookCopyCheckPlanId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const bookCopyCheckRecordIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {bookCopyCheckRecordId} = record.id
      const parameters = { ...record, bookCopyCheckRecordId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateBookCopyCheckRecord`,
        payload: {
          id: owner.id,
          type: 'bookCopyCheckRecord',
          parameters,
  ...
  { title: '检查过程的结果', debugtype: 'string', dataIndex: 'checkProcessResults', width: '6', render: (text, record) => renderStringEdit('checkProcessResults',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newBookCopyCheckRecordToAppend  = {
      	'id':`+1`, 
				'bookCopy':'',
				'bookCopyCheckStatus':'',
				'bookCopyCheckDatetime':'',
				'checkProcessEmployee':'',
				'checkProcessResults':'',
				'checkProcessDatetime':'',
				'bookCopyCheckPlan':'',


      };
      const newData = data ? [...data]:[];
      newData.push(newBookCopyCheckRecordToAppend);
...
export default Form.create()(BookCopyCheckRecordEditTable)
