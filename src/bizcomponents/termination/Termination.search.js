...
@Form.create()
export default class TerminationSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const terminationIds = selectedRows.map((item) => { return item.id })
    console.log('terminationIds', terminationIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'termination', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {TerminationConfirmationTable} = GlobalComponents;
...
<Menu.Item key="reason">关联原因</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TerminationConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 雇佣终止列表`}>
        <Card bordered={false}>
