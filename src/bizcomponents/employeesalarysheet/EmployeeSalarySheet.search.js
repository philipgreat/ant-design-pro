...
@Form.create()
export default class EmployeeSalarySheetSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const employeeSalarySheetIds = selectedRows.map((item) => { return item.id })
    console.log('employeeSalarySheetIds', employeeSalarySheetIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employeeSalarySheet', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {EmployeeSalarySheetConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EmployeeSalarySheetConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 工资单列表`}>
        <Card bordered={false}>
