...
class StoreSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const storeIds = selectedRows.map((item) => { return item.id })
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'store' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'store', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'商店列表' } })
  ...
  render() {
    ...
    const {StoreTable} = GlobalComponents;
    const {StoreConfirmationTable} = GlobalComponents;
    const {StoreSearchForm} = GlobalComponents;
...
<Menu.Item key="district">关联区</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <StoreConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 商店列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <StoreSearchForm {...this.props} />
            </div>
