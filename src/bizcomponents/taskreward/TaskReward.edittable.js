...
import styles from './TaskReward.edittable.less'
...
class TaskRewardEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const taskRewardIds = [record.id];
      const parameters = { taskRewardIds }
      dispatch({
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {taskRewardId} = record.id
      const parameters = { ...record, taskRewardId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateTaskReward`,
        payload: {
          id: owner.id,
          type: 'taskReward',
          parameters,
  ...
  { title: '改写点', debugtype: 'int', dataIndex: 'rewordPoint', width: '7', render: (text, record) => renderStringEdit('rewordPoint',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newTaskRewardToAppend);
...
export default Form.create()(TaskRewardEditTable)
