...
import TaskRewardTable from './TaskReward.table';
import TaskRewardConfirmationTable from './TaskReward.confirmmationtable';

import TaskRewardSearchForm from './TaskReward.searchform';

import PageHeaderLayout from '../../layouts/PageHeaderLayout';

import styles from './TaskReward.search.less';
...
@Form.create()
export default class TaskRewardSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    const params = {
      taskRewardList: 1,
      taskRewardListCurrentPage: pagination.current,
      taskRewardListRowsPerPage: pagination.pageSize,
      ...formValues,
      ...filters,
    };
    if (sorter.field) {
  ...
  handleDelete = () => {
    ...
    const taskRewardIds = selectedRows.map((item) => { return item.id; });
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'taskReward', selectedRows, currentUpdateIndex },
    });
  }
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TaskRewardConfirmationTable data={selectedRows} owner={owner} />
        </Modal>);
    };
    
    return (
      <PageHeaderLayout title="任务悬赏列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TaskRewardSearchForm {...this.props} />
            </div>
