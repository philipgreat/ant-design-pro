...
class TaskRewardSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'任务悬赏列表' } })
  ...
  render() {
    ...
    const {TaskRewardTable} = GlobalComponents;
    const {TaskRewardConfirmationTable} = GlobalComponents;
    const {TaskRewardSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 任务悬赏列表`}>
        <Card bordered={false}>
...
export default Form.create()(TaskRewardSearch)
