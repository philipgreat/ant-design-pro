...
import styles from './InputInterface.search.less'
...
@Form.create()
export default class InputInterfaceSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const inputInterfaceIds = selectedRows.map((item) => { return item.id })
    console.log('inputInterfaceIds', inputInterfaceIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'inputInterface' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'inputInterface', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {InputInterfaceTable} = GlobalComponents;
    const {InputInterfaceConfirmationTable} = GlobalComponents;
    const {InputInterfaceSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <InputInterfaceConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="输入接口列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <InputInterfaceSearchForm {...this.props} />
            </div>
