...
import styles from './ThreadHiding.edittable.less'
...
class ThreadHidingEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const threadHidingIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addThreadHiding`,
        payload: {
          id: owner.id,
          type: 'threadHiding',
          parameters,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {threadHidingId} = record.id
      const parameters = { ...record, threadHidingId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newThreadHidingToAppend);
...
export default Form.create()(ThreadHidingEditTable)
