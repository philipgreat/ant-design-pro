...
import styles from './ThreadHiding.search.less'
...
class ThreadHidingSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const threadHidingIds = selectedRows.map((item) => { return item.id })
    console.log('threadHidingIds', threadHidingIds)
    const parameters = { threadHidingIds }
    dispatch({
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'threadHiding', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'线程隐藏列表' } })
  ...
  render() {
    ...
    const {ThreadHidingTable} = GlobalComponents;
    const {ThreadHidingConfirmationTable} = GlobalComponents;
    const {ThreadHidingSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ThreadHidingConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 线程隐藏列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ThreadHidingSearchForm {...this.props} />
            </div>
...
export default Form.create()(ThreadHidingSearch)
