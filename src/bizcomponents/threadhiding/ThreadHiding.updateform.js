...
class ThreadHidingUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const threadHidingId = values.id
        const imagesValues = this.mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, threadHidingId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextThreadHidingUpdateRow`,
        payload: {
          id: owner.id,
          type: 'threadHiding',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={
          '更新线程隐藏' + (currentUpdateIndex + 1) + '/' + selectedRows.length
        }
        content="更新线程隐藏"
