...
export default class ResponsibilityTypeDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, responsibilityType } = this.props;
    
    if(!responsibilityType){
    	return;
    }
    const {displayName} = responsibilityType;
    if(!displayName){
    	return;
    }
        ...
        <div>
          <Row gutter={24}>
            ...
            <Card title={`员工(${numeral(employeeCount).format('员工0,0')})`}  style={{ width: 180 }}>
              ...
              <p><Link to={`/responsibilityType/${id}/list/employeeList/员工列表`}><FontAwesome name="gear"  />&nbsp;管理</Link></p>
              <p><Link to={`/responsibilityType/${id}/list/employeeCreateForm`}><FontAwesome name="plus"  />&nbsp;新增</Link></p>
