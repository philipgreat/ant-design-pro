...
@Form.create()
export default class ResponsibilityTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const responsibilityTypeIds = selectedRows.map((item) => { return item.id })
    console.log('responsibilityTypeIds', responsibilityTypeIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'responsibilityType' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'responsibilityType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {ResponsibilityTypeConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ResponsibilityTypeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 责任类型列表`}>
        <Card bordered={false}>
