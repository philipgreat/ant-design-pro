...
class GroupFilterSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'群组过滤器列表' } })
  ...
  render() {
    ...
    const {GroupFilterTable} = GlobalComponents;
...
<Menu.Item key="groupPage">关联群组页面</Menu.Item>
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 群组过滤器列表`}>
        <Card bordered={false}>
...
export default Form.create()(GroupFilterSearch)
