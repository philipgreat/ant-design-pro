...
import styles from './GroupFilter.edittable.less'
...
class GroupFilterEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {groupFilterId} = record.id
      const parameters = { ...record, groupFilterId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '滤波环节', debugtype: 'string', dataIndex: 'filterLink', width: '32', render: (text, record) => renderStringEdit('filterLink',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newGroupFilterToAppend);
...
export default Form.create()(GroupFilterEditTable)
