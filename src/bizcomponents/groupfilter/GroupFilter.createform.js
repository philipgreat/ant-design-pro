...
import styles from './GroupFilter.createform.less'
...
class GroupFilterCreateForm extends Component {
  state = {
  ...
  executeCandidateGroupPageSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = GroupFilterService.requestCandidateGroupPage("groupPage", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addGroupFilter`,
          payload: { id: owner.id, type: 'groupFilter', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'groupFilter',listName:'群组过滤器列表' },
      })
    }
