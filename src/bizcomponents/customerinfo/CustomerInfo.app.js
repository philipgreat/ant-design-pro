...
import styles from './CustomerInfo.app.less';
import CustomerInfoDashboard from './CustomerInfo.dashboard';
...
class CustomerInfoBizApp extends React.PureComponent {
 ...
 constructor(props) {
  ...
  getNavMenuItems(objectId){
    ...
    return (
      ...
      <Menu.Item >   
        <Link to={"/customerInfo/"+objectId+"/list/carInfoList"}>汽车信息</Link>
      </Menu.Item>
  

      <Menu.Item >   
        <Link to={"/customerInfo/"+objectId+"/list/carInspectionOrderList"}>审车订单</Link>
      </Menu.Item>
  ...
  getCarInfoSearch() {
    ...
    return connect(state => ({
      rule: state.rule,
      data: state._customerInfo.carInfoList,
      count: state._customerInfo.carInfoCount,
      currentPage: state._customerInfo.carInfoCurrentPageNumber,
      searchFormParameters: state._customerInfo.carInfoSearchFormParameters,
      loading: state._customerInfo.loading,
  ...
  getCarInspectionOrderSearch() {
    ...
    return connect(state => ({
      rule: state.rule,
      data: state._customerInfo.carInspectionOrderList,
      count: state._customerInfo.carInspectionOrderCount,
      currentPage: state._customerInfo.carInspectionOrderCurrentPageNumber,
      searchFormParameters: state._customerInfo.carInspectionOrderSearchFormParameters,
      loading: state._customerInfo.loading,
      owner: {type:'_customerInfo',id:state._customerInfo.id}//this is for model namespace and 
    ...
    const layout = (
      <Layout>
        ...
        >
          ...
          >
            {this.getNavMenuItems(this.props.customerInfo.id)}
          </Menu>
        </Sider>
        <Layout>
          ...
          <Content style={{ margin: '24px 24px 0', height: '100%' }}>
            <Switch>
            <Route path="/customerInfo/:id/dashboard" component={CustomerInfoDashboard} />
