...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        if (!pathname.startsWith("/customerInfo")) {
          return;
        }
        ...
        const dashboardmatch = pathToRegexp('/customerInfo/:id/dashboard').exec(pathname);
        if (dashboardmatch) {
        ...
        const match = pathToRegexp('/customerInfo/:id/list/:listName').exec(pathname);
        if (!match) {
          return;
  ...
  effects: {
    *view({ payload }, { call, put }) { 
      ...
      const data = yield call(CustomerInfoService.view,payload.id);
      console.log("this is the data id: ", data.id)
      yield put({type:"updateState",payload:data});
    },
    *load({ payload }, { call, put }) { 
        yield put({type:"showLoading",payload:payload});
        const data = yield call(CustomerInfoService.load,payload.id,payload.parameters);
    ...
    *gotoCreateForm({ payload }, { call, put }) {
     ...
     *goback({ payload }, { call, put }) {
    	const {id,type}=payload;
    	yield put(routerRedux.push('/customerInfo/'+id+'/list/'+type+'List'));
     },
