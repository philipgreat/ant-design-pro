...
import styles from './LineItem.edittable.less'
...
class LineItemEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const lineItemIds = [record.id];
      const parameters = { lineItemIds }
      dispatch({
        type: `${owner.type}/removeLineItemList`,
    ...
    const updateRecord = (e,record) =>{
      ...
      const parameters = { ...record, lineItemId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '总结', debugtype: 'string', dataIndex: 'summary', width: '20', render: (text, record) => renderStringEdit('summary',text, record)  },
  ...
  { title: 'Sku类型', debugtype: 'string', dataIndex: 'skuType', width: '13', render: (text, record) => renderStringEdit('skuType',text, record)  },
  { title: 'Sku Id', debugtype: 'string', dataIndex: 'skuId', width: '12', render: (text, record) => renderStringEdit('skuId',text, record)  },
  { title: 'Sku链接', debugtype: 'string_url', dataIndex: 'skuLink', width: '40', render: (text, record) => renderStringEdit('skuLink',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newLineItemToAppend);
...
export default Form.create()(LineItemEditTable)
