...
import styles from './BookCopySku.edittable.less'
...
class BookCopySkuEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
      ...
      return {bookId,bookCopyId,bookManagementId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const bookCopySkuIds = [record.id];
      const parameters = { bookCopySkuIds }
      dispatch({
        type: `${owner.type}/removeBookCopySkuList`,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {bookCopySkuId} = record.id
      const parameters = { ...record, bookCopySkuId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '书的名字', debugtype: 'string', dataIndex: 'bookName', width: '24', render: (text, record) => renderStringEdit('bookName',text, record)  },
  { title: '书的封面', dataIndex: 'bookCover', render: (text, record) => <ImagePreview imageLocation={record.bookCover} /> },
  { title: '书的作者', debugtype: 'string', dataIndex: 'bookAuthor', width: '7', render: (text, record) => renderStringEdit('bookAuthor',text, record)  },
  { title: '图书出版者', debugtype: 'string', dataIndex: 'bookPublisher', width: '9', render: (text, record) => renderStringEdit('bookPublisher',text, record)  },
  ...
  { title: '书Isbn13', debugtype: 'string', dataIndex: 'bookIsbn13', width: '17', render: (text, record) => renderStringEdit('bookIsbn13',text, record)  },
  { title: '书Isbn10', debugtype: 'int', dataIndex: 'bookIsbn10', width: '14', render: (text, record) => renderStringEdit('bookIsbn10',text, record)  },
  { title: '书的地位', debugtype: 'string', dataIndex: 'bookStatus', width: '47', render: (text, record) => renderStringEdit('bookStatus',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newBookCopySkuToAppend);
...
export default Form.create()(BookCopySkuEditTable)
