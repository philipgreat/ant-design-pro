...
import styles from './OnlineOrder.search.less'
...
class OnlineOrderSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const onlineOrderIds = selectedRows.map((item) => { return item.id })
    console.log('onlineOrderIds', onlineOrderIds)
    const parameters = { onlineOrderIds }
    dispatch({
      type: `${owner.type}/removeOnlineOrderList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'onlineOrder' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'onlineOrder', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'线上订单号列表' } })
  ...
  render() {
    ...
    const {OnlineOrderTable} = GlobalComponents;
    const {OnlineOrderConfirmationTable} = GlobalComponents;
    const {OnlineOrderSearchForm} = GlobalComponents;
...
<Menu.Item key="session">关联场次</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <OnlineOrderConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 线上订单号列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <OnlineOrderSearchForm {...this.props} />
            </div>
...
export default Form.create()(OnlineOrderSearch)
