...
@Form.create()
export default class OfferAcceptanceSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const offerAcceptanceIds = selectedRows.map((item) => { return item.id })
    console.log('offerAcceptanceIds', offerAcceptanceIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'offerAcceptance' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'offerAcceptance', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {OfferAcceptanceConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <OfferAcceptanceConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 接受工作要约列表`}>
        <Card bordered={false}>
