...
import styles from './CompanyQrcodePromotionRecord.search.less'
...
class CompanyQrcodePromotionRecordSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const companyQrcodePromotionRecordIds = selectedRows.map((item) => { return item.id })
    console.log('companyQrcodePromotionRecordIds', companyQrcodePromotionRecordIds)
    const parameters = { companyQrcodePromotionRecordIds }
    dispatch({
      type: `${owner.type}/removeCompanyQrcodePromotionRecordList`,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'companyQrcodePromotionRecord', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'公司二维码推广记录列表' } })
  ...
  render() {
    ...
    const {CompanyQrcodePromotionRecordTable} = GlobalComponents;
    const {CompanyQrcodePromotionRecordConfirmationTable} = GlobalComponents;
    const {CompanyQrcodePromotionRecordSearchForm} = GlobalComponents;
...
<Menu.Item key="customer">关联客户</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CompanyQrcodePromotionRecordConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 公司二维码推广记录列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CompanyQrcodePromotionRecordSearchForm {...this.props} />
            </div>
...
export default Form.create()(CompanyQrcodePromotionRecordSearch)
