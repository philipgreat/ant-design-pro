...
class ServiceVehicleMovementM2cSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const serviceVehicleMovementM2cIds = selectedRows.map((item) => { return item.id })
    console.log('serviceVehicleMovementM2cIds', serviceVehicleMovementM2cIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'serviceVehicleMovementM2c', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'还车服务列表' } })
  ...
  render() {
    ...
    const {ServiceVehicleMovementM2cTable} = GlobalComponents;
    const {ServiceVehicleMovementM2cConfirmationTable} = GlobalComponents;
    const {ServiceVehicleMovementM2cSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ServiceVehicleMovementM2cConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 还车服务列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ServiceVehicleMovementM2cSearchForm {...this.props} />
            </div>
...
export default Form.create()(ServiceVehicleMovementM2cSearch)
