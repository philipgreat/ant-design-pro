...
import styles from './ServiceVehicleMovementM2c.edittable.less'
...
class ServiceVehicleMovementM2cEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const serviceVehicleMovementM2cIds = [record.id];
      const parameters = { serviceVehicleMovementM2cIds }
      dispatch({
        type: `${owner.type}/removeServiceVehicleMovementM2cList`,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {serviceVehicleMovementM2cId} = record.id
      const parameters = { ...record, serviceVehicleMovementM2cId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateServiceVehicleMovementM2c`,
        payload: {
          id: owner.id,
          type: 'serviceVehicleMovementM2c',
          parameters,
  ...
  { title: '服务类型', debugtype: 'string', dataIndex: 'movementPurpose', width: '35', render: (text, record) => renderStringEdit('movementPurpose',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newServiceVehicleMovementM2cToAppend  = {
      	'id':`+1`, 
				'serviceStatus':'',
				'responsibleWorker':'',
				'serviceSummary':'',
      ...
      newData.push(newServiceVehicleMovementM2cToAppend);
...
export default Form.create()(ServiceVehicleMovementM2cEditTable)
