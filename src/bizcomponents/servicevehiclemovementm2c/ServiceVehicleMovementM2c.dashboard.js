...
export default class ServiceVehicleMovementM2cDashboard extends Component {
  render() {
    ...
    const { id, serviceVehicleMovementM2cChecklistResultCount } = this.props.serviceVehicleMovementM2c
          ...
          <Row gutter={24}>
            ...
            <Col {...topColResponsiveProps}>
              <ChartCard
                bordered={false}
                title="还车服务检查结果"
                action={<Tooltip title="还车服务检查结果"><Icon type="info-circle-o" /></Tooltip>}
                total={numeral(serviceVehicleMovementM2cChecklistResultCount).format('0,0')}
                footer={<Field label="状态" value="良好" />}
                contentHeight={46}
              >
                <Link to={`/serviceVehicleMovementM2c/${id}/list/serviceVehicleMovementM2cChecklistResultList`}><Icon type="profile" style={{ fontSize: 20, color: '#08c' }} /></Link>
                &nbsp;
                <Link to={`/serviceVehicleMovementM2c/${id}/list/serviceVehicleMovementM2cChecklistResultCreateForm`}><Icon type="plus-circle-o" style={{ fontSize: 20, color: '#08c' }} /></Link>
                &nbsp;
                <Link to={`/serviceVehicleMovementM2c/${id}/list/serviceVehicleMovementM2cChecklistResultList`}><Icon type="line-chart" style={{ fontSize: 20, color: '#08c' }} /></Link>
              </ChartCard>
            </Col>
