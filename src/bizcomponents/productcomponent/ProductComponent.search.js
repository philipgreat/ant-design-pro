...
import styles from './ProductComponent.search.less'
...
@Form.create()
export default class ProductComponentSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const productComponentIds = selectedRows.map((item) => { return item.id })
    console.log('productComponentIds', productComponentIds)
    const parameters = { productComponentIds }
    dispatch({
      type: `${owner.type}/removeProductComponentList`,
      payload: { id: owner.id, type: 'productComponent', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'productComponent' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'productComponent', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {ProductComponentTable} = GlobalComponents;
    const {ProductComponentConfirmationTable} = GlobalComponents;
    const {ProductComponentSearchForm} = GlobalComponents;
...
<Menu.Item key="product">关联产品</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ProductComponentConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 产品组件列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ProductComponentSearchForm {...this.props} />
            </div>
