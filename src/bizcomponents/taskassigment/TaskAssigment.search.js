...
class TaskAssigmentSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const taskAssigmentIds = selectedRows.map((item) => { return item.id })
    console.log('taskAssigmentIds', taskAssigmentIds)
    const parameters = { taskAssigmentIds }
    dispatch({
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'任务分配列表' } })
  ...
  render() {
    ...
    const {TaskAssigmentTable} = GlobalComponents;
    const {TaskAssigmentConfirmationTable} = GlobalComponents;
    const {TaskAssigmentSearchForm} = GlobalComponents;
...
<Menu.Item key="assignee">关联受让人</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TaskAssigmentConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 任务分配列表`}>
        <Card bordered={false}>
...
export default Form.create()(TaskAssigmentSearch)
