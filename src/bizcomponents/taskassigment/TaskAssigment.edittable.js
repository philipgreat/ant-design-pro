...
import styles from './TaskAssigment.edittable.less'
...
class TaskAssigmentEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = record => {
      ...
      const assigneeId = record.assignee.id
    ...
    const deleteRecord = (e, record) => {
      ...
      const { data } = this.state
      const taskAssigmentIds = [record.id]
    ...
    const updateRecord = (e, record) => {
      const { dispatch, owner } = this.props
      const { data } = this.state
      const { taskAssigmentId } = record.id
      const parameters = { ...record, taskAssigmentId }
      const newData = [...data]
      const row = newData.filter(item => item.id === record.id)[0]
      row.editable = !row.editable

      dispatch({
        type: `${owner.type}/updateTaskAssigment`,
        payload: {
          id: owner.id,
          type: 'taskAssigment',
          parameters,
    ...
    const columns = [
      { title: '序号', debugtype: 'string', dataIndex: 'id', width: '20' },
      {
        ...
        dataIndex: 'task',
        render: (text, record) =>
          record.task ? record.task.displayName : '暂无',
      },
      {
        title: '受让人',
        dataIndex: 'assignee',
        render: (text, record) =>
          record.assignee ? record.assignee.displayName : '暂无',
      },
      {
        title: '分配时间',
    ...
    const newRecord = () => {
      const newTaskAssigmentToAppend = {
        id: `+1`,
        task: '',
        assignee: '',
        assignTime: '',
      ...
      newData.push(newTaskAssigmentToAppend)
...
export default Form.create()(TaskAssigmentEditTable)
