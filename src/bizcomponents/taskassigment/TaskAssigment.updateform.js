...
import styles from './TaskAssigment.updateform.less';
...
class TaskAssigmentUpdateForm extends PureComponent {
 ...
 mapBackToImageValues(convertedImagesValues) {
  ...
  getSelectedRow(){
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
			assignTime: moment(item.assignTime).format('YYYY-MM-DD'),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
     validateFieldsAndScroll((error, values) => {
        ...
        const taskAssigmentId = values.id;
        const imagesValues = this.mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values,taskAssigmentId, ...imagesValues };
    ...
    const skipToNext = () => {
      ...
      const newIndex= currentUpdateIndex+1;
      dispatch({
          type: owner.type+'/gotoNextTaskAssigmentUpdateRow',
            payload: {
              id:owner.id,type:'taskAssigment', 
    ...
    return (
      <PageHeaderLayout
        ...
        title={"更新任务分配"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新任务分配"
...
}))(Form.create()(TaskAssigmentUpdateForm));
