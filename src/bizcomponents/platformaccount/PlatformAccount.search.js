...
import styles from './PlatformAccount.search.less'
...
class PlatformAccountSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const platformAccountIds = selectedRows.map((item) => { return item.id })
    console.log('platformAccountIds', platformAccountIds)
    const parameters = { platformAccountIds }
    dispatch({
      type: `${owner.type}/removePlatformAccountList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'platformAccount' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'platformAccount', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'平台账户列表' } })
  ...
  render() {
    ...
    const {PlatformAccountTable} = GlobalComponents;
    const {PlatformAccountConfirmationTable} = GlobalComponents;
    const {PlatformAccountSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <PlatformAccountConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 平台账户列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <PlatformAccountSearchForm {...this.props} />
            </div>
...
export default Form.create()(PlatformAccountSearch)
