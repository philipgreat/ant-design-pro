...
import styles from './PlatformAccount.edittable.less'
...
class PlatformAccountEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
      ...
      return {platformId,accountManagementId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const platformAccountIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {platformAccountId} = record.id
      const parameters = { ...record, platformAccountId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updatePlatformAccount`,
        payload: {
          id: owner.id,
          type: 'platformAccount',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newPlatformAccountToAppend  = {
      	'id':`+1`, 
				'name':'',
				'amount':'',
				'platform':'',
      ...
      newData.push(newPlatformAccountToAppend);
...
export default Form.create()(PlatformAccountEditTable)
