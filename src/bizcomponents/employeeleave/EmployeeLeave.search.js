...
@Form.create()
export default class EmployeeLeaveSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const employeeLeaveIds = selectedRows.map((item) => { return item.id })
    console.log('employeeLeaveIds', employeeLeaveIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employeeLeave', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {EmployeeLeaveConfirmationTable} = GlobalComponents;
...
<Menu.Item key="who">关联谁</Menu.Item>
<Menu.Item key="type">关联类型</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EmployeeLeaveConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 请假记录列表`}>
        <Card bordered={false}>
