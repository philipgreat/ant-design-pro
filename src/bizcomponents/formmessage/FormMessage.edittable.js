...
import styles from './FormMessage.edittable.less'
...
class FormMessageEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const formMessageIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {formMessageId} = record.id
      const parameters = { ...record, formMessageId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '头衔', debugtype: 'string', dataIndex: 'title', width: '10', render: (text, record) => renderStringEdit('title',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newFormMessageToAppend);
...
export default Form.create()(FormMessageEditTable)
