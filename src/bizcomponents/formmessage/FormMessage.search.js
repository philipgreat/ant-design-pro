...
@Form.create()
export default class FormMessageSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const formMessageIds = selectedRows.map((item) => { return item.id })
    console.log('formMessageIds', formMessageIds)
    const parameters = { formMessageIds }
    dispatch({
      type: `${owner.type}/removeFormMessageList`,
      payload: { id: owner.id, type: 'formMessage', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'formMessage' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'formMessage', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'表单信息列表' } })
  ...
  render() {
    ...
    const {FormMessageConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <FormMessageConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 表单信息列表`}>
        <Card bordered={false}>
