...
class InsuranceServiceAccountSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const insuranceServiceAccountIds = selectedRows.map((item) => { return item.id })
    console.log('insuranceServiceAccountIds', insuranceServiceAccountIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'insuranceServiceAccount', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'保险服务对账单列表' } })
  ...
  render() {
    ...
    const {InsuranceServiceAccountTable} = GlobalComponents;
    const {InsuranceServiceAccountConfirmationTable} = GlobalComponents;
    const {InsuranceServiceAccountSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <InsuranceServiceAccountConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 保险服务对账单列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <InsuranceServiceAccountSearchForm {...this.props} />
            </div>
...
export default Form.create()(InsuranceServiceAccountSearch)
