...
const summaryOf = insuranceServiceAccount => {
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="保险服务单号">
        {insuranceServiceAccount.insuranceOrderNumber}
      </Description>
      <Description term="员工姓名">
        {insuranceServiceAccount.employeeName}
      </Description>
      <Description term="保险名称">
        {insuranceServiceAccount.insuranceName}
      </Description>
      <Description term="承保方">
        {insuranceServiceAccount.insuranceVendor}
      </Description>
      <Description term="保费">
        {insuranceServiceAccount.insurancePrice}
      </Description>
      <Description term="保单号码">
        {insuranceServiceAccount.insuranceNumber}
      </Description>
      <Description term="保险购买日期">
        {moment(insuranceServiceAccount.insuranceOrderDatetime).format(
          'YYYY-MM-DD'
        )}
      </Description>
      <Description term="年检订单ID">
        {insuranceServiceAccount.mainOrderId}
      </Description>
    </DescriptionList>
  )
}
...
export default class InsuranceServiceAccountDashboard extends Component {
  ...
  render() {
    ...
    const cardsData = {
      cardsName: '保险服务对账单',
      cardsFor: 'insuranceServiceAccount',
      cardsSource: this.props.insuranceServiceAccount,
      subItems: [],
    }
