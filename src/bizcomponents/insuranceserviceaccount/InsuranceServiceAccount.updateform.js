...
class InsuranceServiceAccountUpdateForm extends Component {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
        insuranceOrderDatetime: moment(item.insuranceOrderDatetime),
  ...
  render() {
    ...
    return (
      <PageHeaderLayout
        title={"更新保险服务对账单"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新保险服务对账单"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.insuranceOrderNumber} {...formItemLayout}>
                  {getFieldDecorator('insuranceOrderNumber', {
                    initialValue: selectedRow.insuranceOrderNumber,
