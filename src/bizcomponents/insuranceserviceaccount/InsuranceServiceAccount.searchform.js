...
@Form.create()
export default class InsuranceServiceAccountSearchForm extends PureComponent {
  state = {
    // addInputValue: '',
    // modalVisible: false,
    expandForm: false,
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      dispatch({
        ...
        payload: {
          id: owner.id,
          parameters: params,
          insuranceServiceAccountSearchFormParameters: fieldsValue,
        },
      })
    })
  }
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={8} sm={24}>
            <FormItem label="保险订单号">
              {getFieldDecorator('insuranceOrderNumber')(
                <Input placeholder="请输入保险订单号" />
              )}
            </FormItem>
          </Col>
