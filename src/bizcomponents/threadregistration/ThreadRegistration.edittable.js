...
import styles from './ThreadRegistration.edittable.less'
...
class ThreadRegistrationEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const threadRegistrationIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {threadRegistrationId} = record.id
      const parameters = { ...record, threadRegistrationId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateThreadRegistration`,
        payload: {
          id: owner.id,
          type: 'threadRegistration',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newThreadRegistrationToAppend  = {
      	'id':`+1`, 
				'thread':'',
				'participant':'',
				'registerTime':'',
      ...
      newData.push(newThreadRegistrationToAppend);
...
export default Form.create()(ThreadRegistrationEditTable)
