...
import styles from './ThreadRegistration.search.less'
...
class ThreadRegistrationSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const threadRegistrationIds = selectedRows.map((item) => { return item.id })
    console.log('threadRegistrationIds', threadRegistrationIds)
    const parameters = { threadRegistrationIds }
    dispatch({
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'threadRegistration', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'活动注册列表' } })
  ...
  render() {
    ...
    const {ThreadRegistrationTable} = GlobalComponents;
    const {ThreadRegistrationConfirmationTable} = GlobalComponents;
    const {ThreadRegistrationSearchForm} = GlobalComponents;
...
<Menu.Item key="thread">关联主贴</Menu.Item>
<Menu.Item key="participant">关联参与者</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ThreadRegistrationConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 活动注册列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ThreadRegistrationSearchForm {...this.props} />
            </div>
...
export default Form.create()(ThreadRegistrationSearch)
