...
import styles from './ThreadRegistration.updateform.less';
...
class ThreadRegistrationUpdateForm extends PureComponent {
 ...
 mapBackToImageValues(convertedImagesValues) {
  ...
  getSelectedRow(){
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
			registerTime: moment(item.registerTime).format('YYYY-MM-DD'),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
     validateFieldsAndScroll((error, values) => {
        ...
        const threadRegistrationId = values.id;
        const imagesValues = this.mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values,threadRegistrationId, ...imagesValues };
    ...
    const skipToNext = () => {
      ...
      const newIndex= currentUpdateIndex+1;
      dispatch({
          type: owner.type+'/gotoNextThreadRegistrationUpdateRow',
            payload: {
              id:owner.id,type:'threadRegistration', 
    ...
    return (
      <PageHeaderLayout
        ...
        title={"更新活动注册"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新活动注册"
...
}))(Form.create()(ThreadRegistrationUpdateForm));
