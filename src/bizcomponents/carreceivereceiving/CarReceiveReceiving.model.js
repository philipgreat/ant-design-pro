  ...
  effects: {
    ...
    *gotoCreateForm({ payload }, { call, put }) {
     ...
     *gotoUpdateForm({ payload }, { call, put }) {
        ...
        const location = {pathname:'/carReceiveReceiving/'+id+'/list/'+type+'UpdateForm',state};
	...
	*updateCarReceivingServiceOrder({ payload }, { call, put }) {
		...
		const data = yield call(CarReceiveReceivingService.updateCarReceivingServiceOrder, id, parameters);
		if(hasError(data)){
			handleServerError(data);
			return;
		}
		...
		const location = {pathname:'/carReceiveReceiving/' + id + '/list/' + type + 'List',state:newPlayload};
		yield put(routerRedux.push(location));
	},
