...
import styles from './MemberServiceBoundleSku.search.less'
...
class MemberServiceBoundleSkuSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const memberServiceBoundleSkuIds = selectedRows.map((item) => { return item.id })
    console.log('memberServiceBoundleSkuIds', memberServiceBoundleSkuIds)
    const parameters = { memberServiceBoundleSkuIds }
    dispatch({
      type: `${owner.type}/removeMemberServiceBoundleSkuList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberServiceBoundleSku' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberServiceBoundleSku', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'会员服务Boundle Sku列表' } })
  ...
  render() {
    ...
    const {MemberServiceBoundleSkuTable} = GlobalComponents;
    const {MemberServiceBoundleSkuConfirmationTable} = GlobalComponents;
    const {MemberServiceBoundleSkuSearchForm} = GlobalComponents;
...
<Menu.Item key="memberProduct">关联成员的产品</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <MemberServiceBoundleSkuConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 会员服务Boundle Sku列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <MemberServiceBoundleSkuSearchForm {...this.props} />
            </div>
...
export default Form.create()(MemberServiceBoundleSkuSearch)
