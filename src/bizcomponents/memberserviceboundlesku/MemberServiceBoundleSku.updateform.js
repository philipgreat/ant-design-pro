...
import styles from './MemberServiceBoundleSku.updateform.less'
...
class MemberServiceBoundleSkuUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const memberServiceBoundleSkuId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, memberServiceBoundleSkuId, ...imagesValues }

        // const newIndex= currentUpdateIndex + 1
        dispatch({
          type: `${owner.type}/updateMemberServiceBoundleSku`,
          payload: {
            id: owner.id,
            type: 'memberServiceBoundleSku',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextMemberServiceBoundleSkuUpdateRow`,
        payload: {
          id: owner.id,
          type: 'memberServiceBoundleSku',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新会员服务Boundle Sku"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新会员服务Boundle Sku"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.memberServiceName} {...formItemLayout}>
                  {getFieldDecorator('memberServiceName', {
                    initialValue: selectedRow.memberServiceName,
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.servicePeriodMonths} {...formItemLayout}>
                  {getFieldDecorator('servicePeriodMonths', {
                    initialValue: selectedRow.servicePeriodMonths,
...
}))(Form.create()(MemberServiceBoundleSkuUpdateForm))
