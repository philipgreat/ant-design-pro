...
import styles from './TaskReplyLike.search.less'
...
class TaskReplyLikeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const taskReplyLikeIds = selectedRows.map((item) => { return item.id })
    console.log('taskReplyLikeIds', taskReplyLikeIds)
    const parameters = { taskReplyLikeIds }
    dispatch({
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'taskReplyLike', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'任务回复点赞列表' } })
  ...
  render() {
    ...
    const {TaskReplyLikeTable} = GlobalComponents;
    const {TaskReplyLikeConfirmationTable} = GlobalComponents;
    const {TaskReplyLikeSearchForm} = GlobalComponents;
...
<Menu.Item key="taskReply">关联回复任务</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TaskReplyLikeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 任务回复点赞列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TaskReplyLikeSearchForm {...this.props} />
            </div>
...
export default Form.create()(TaskReplyLikeSearch)
