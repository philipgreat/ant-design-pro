...
import styles from './TaskReplyLike.edittable.less'
...
class TaskReplyLikeEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = record => {
      ...
      const taskReplyId = record.taskReply.id

      //const communityId = record.community.id;
      return { replierId, taskReplyId }
    }
    const deleteRecord = (e, record) => {
      const { dispatch, owner } = this.props
      const { data } = this.state
      const taskReplyLikeIds = [record.id]
    ...
    const updateRecord = (e, record) => {
      const { dispatch, owner } = this.props
      const { data } = this.state
      const { taskReplyLikeId } = record.id
      const parameters = { ...record, taskReplyLikeId }
      const newData = [...data]
      const row = newData.filter(item => item.id === record.id)[0]
      row.editable = !row.editable

      dispatch({
        type: `${owner.type}/updateTaskReplyLike`,
        payload: {
          id: owner.id,
          type: 'taskReplyLike',
          parameters,
    ...
    const columns = [
      ...
      },
      {
        ...
        dataIndex: 'taskReply',
        render: (text, record) =>
          record.taskReply ? record.taskReply.displayName : '暂无',
      },
      {
        title: '操作',
    ...
    const newRecord = () => {
      const newTaskReplyLikeToAppend = {
        id: `+1`,
        likeTime: '',
        replier: '',
        taskReply: '',
      ...
      newData.push(newTaskReplyLikeToAppend)
...
export default Form.create()(TaskReplyLikeEditTable)
