...
class EmployeeDrivingLicenseSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'员工驾驶证列表' } })
...
<Menu.Item key="employee">关联员工</Menu.Item>
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 员工驾驶证列表`}>
        <Card bordered={false}>
...
export default Form.create()(EmployeeDrivingLicenseSearch)
