...
import styles from './UserMessage.search.less'
...
class UserMessageSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const userMessageIds = selectedRows.map((item) => { return item.id })
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'userMessage', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'用户消息列表' } })
  ...
  render() {
    ...
    const {UserMessageTable} = GlobalComponents;
    const {UserMessageConfirmationTable} = GlobalComponents;
    const {UserMessageSearchForm} = GlobalComponents;
...
<Menu.Item key="receiver">关联接收者</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <UserMessageConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 用户消息列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <UserMessageSearchForm {...this.props} />
            </div>
...
export default Form.create()(UserMessageSearch)
