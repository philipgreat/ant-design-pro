...
import styles from './LoginHistory.edittable.less'
...
class LoginHistoryEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {loginHistoryId} = record.id
      const parameters = { ...record, loginHistoryId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '从IP', debugtype: 'string', dataIndex: 'fromIp', width: '15', render: (text, record) => renderStringEdit('fromIp',text, record)  },
...
export default Form.create()(LoginHistoryEditTable)
