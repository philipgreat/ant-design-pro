...
export default class LoginHistorySearch extends PureComponent {
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'登录历史列表' } })
...
<Menu.Item key="secUser">关联SEC的用户</Menu.Item>
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 登录历史列表`}>
        <Card bordered={false}>
