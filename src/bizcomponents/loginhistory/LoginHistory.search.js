...
import LoginHistoryConfirmationTable from './LoginHistory.confirmmationtable';

import LoginHistorySearchForm from './LoginHistory.searchform';
...
@Form.create()
export default class LoginHistorySearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    const params = {
      loginHistoryList: 1,
      loginHistoryListCurrentPage: pagination.current,
      loginHistoryListRowsPerPage: pagination.pageSize,
      ...formValues,
      ...filters,
    };
    if (sorter.field) {
  ...
  handleDelete = () => {
    ...
    const loginHistoryIds = selectedRows.map((item) => { return item.id; });
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'loginHistory', selectedRows, currentUpdateIndex },
    });
  }
