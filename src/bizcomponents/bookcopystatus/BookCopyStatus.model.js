...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        if (!pathname.startsWith('/bookCopyStatus')) {
          return
        }
        ...
        const dashboardmatch = pathToRegexp('/bookCopyStatus/:id/dashboard').exec(pathname)
        if (dashboardmatch) {
          const id = dashboardmatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const editDetailMatch = pathToRegexp('/bookCopyStatus/:id/editDetail').exec(pathname)
        if (editDetailMatch) {
          const id = editDetailMatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const viewDetailMatch = pathToRegexp('/bookCopyStatus/:id/viewDetail').exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp('/bookCopyStatus/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *addBookCopy({ payload }, { call, put }) {
      ...
      const location = { pathname: `/bookCopyStatus/${id}/list/${type}List/书副本列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateBookCopy({ payload }, { call, put }) {
      ...
      const location = { pathname: `/bookCopyStatus/${id}/list/${type}List/书副本列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
