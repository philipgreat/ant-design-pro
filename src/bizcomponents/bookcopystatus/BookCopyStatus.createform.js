...
import styles from './BookCopyStatus.createform.less'
...
class BookCopyStatusCreateForm extends Component {
  state = {
  ...
  executeCandidateBookManagementSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = BookCopyStatusService.requestCandidateBookManagement("bookManagement", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addBookCopyStatus`,
          payload: { id: owner.id, type: 'bookCopyStatus', parameters },
        })
      })
    }
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addBookCopyStatus`,
          payload: { id: owner.id, type: 'bookCopyStatus', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'bookCopyStatus',listName:'书副本地位列表' },
      })
    }
...
}))(Form.create()(BookCopyStatusCreateForm))
