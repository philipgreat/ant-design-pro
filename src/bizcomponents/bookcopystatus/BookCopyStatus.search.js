...
import styles from './BookCopyStatus.search.less'
...
class BookCopyStatusSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookCopyStatusIds = selectedRows.map((item) => { return item.id })
    console.log('bookCopyStatusIds', bookCopyStatusIds)
    const parameters = { bookCopyStatusIds }
    dispatch({
      type: `${owner.type}/removeBookCopyStatusList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookCopyStatus' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookCopyStatus', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'书副本地位列表' } })
  ...
  render() {
    ...
    const {BookCopyStatusTable} = GlobalComponents;
    const {BookCopyStatusConfirmationTable} = GlobalComponents;
    const {BookCopyStatusSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookCopyStatusConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 书副本地位列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookCopyStatusSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookCopyStatusSearch)
