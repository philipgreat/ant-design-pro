...
import styles from './ServiceVehicleMovementM2cChecklistResult.edittable.less'
...
class ServiceVehicleMovementM2cChecklistResultEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const serviceVehicleMovementM2cChecklistResultIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addServiceVehicleMovementM2cChecklistResult`,
        payload: {
          id: owner.id,
          type: 'serviceVehicleMovementM2cChecklistResult',
          parameters,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {serviceVehicleMovementM2cChecklistResultId} = record.id
      const parameters = { ...record, serviceVehicleMovementM2cChecklistResultId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateServiceVehicleMovementM2cChecklistResult`,
        payload: {
          id: owner.id,
          type: 'serviceVehicleMovementM2cChecklistResult',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newServiceVehicleMovementM2cChecklistResultToAppend  = {
      	'id':`+1`, 
				'service':'',
				'checkResult':'',
				'checkResultComments':'',
      ...
      newData.push(newServiceVehicleMovementM2cChecklistResultToAppend);
...
export default Form.create()(ServiceVehicleMovementM2cChecklistResultEditTable)
