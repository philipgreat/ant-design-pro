...
import styles from './GamePlatform.dashboard.less'
...
const imageListOf = (gamePlatform) =>{
...
const settingListOf = (gamePlatform) =>{
...
const largeTextOf = (gamePlatform) =>{
...
const summaryOf = (gamePlatform) =>{

	return (
	<DescriptionList className={styles.headerList} size="small" col="4">
<Description term="ID">{gamePlatform.id}</Description> 
<Description term="名称">{gamePlatform.name}</Description> 
...
class GamePlatformDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, gamePlatform } = this.props;
    
    if(!gamePlatform){
    	return;
    }
    const {displayName} = gamePlatform;
    if(!displayName){
    	return;
    }
  ...
  render() {
    ...
    const { id,displayName, cityCount, onlineOrderCount, customerCount, couponCount, adCount } = this.props.gamePlatform
    const cardsData = {cardsName:"游戏平台",cardsFor: "gamePlatform",cardsSource: this.props.gamePlatform,
  		subItems: [
