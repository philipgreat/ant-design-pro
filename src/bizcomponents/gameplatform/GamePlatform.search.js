...
import styles from './GamePlatform.search.less'
...
class GamePlatformSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const gamePlatformIds = selectedRows.map((item) => { return item.id })
    console.log('gamePlatformIds', gamePlatformIds)
    const parameters = { gamePlatformIds }
    dispatch({
      type: `${owner.type}/removeGamePlatformList`,
      payload: { id: owner.id, type: 'gamePlatform', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'gamePlatform' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'gamePlatform', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'游戏平台列表' } })
  ...
  render() {
    ...
    const {GamePlatformTable} = GlobalComponents;
    const {GamePlatformConfirmationTable} = GlobalComponents;
    const {GamePlatformSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <GamePlatformConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 游戏平台列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <GamePlatformSearchForm {...this.props} />
            </div>
...
export default Form.create()(GamePlatformSearch)
