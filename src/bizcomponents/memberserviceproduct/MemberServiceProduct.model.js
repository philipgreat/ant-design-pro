...
export default {
  ...
  effects: {
    ...
    *addTokenInMemberServiceProduct({ payload }, { call, put }) {
      ...
      const location = { pathname: `/memberServiceProduct/${id}/list/${type}List/成员服务产品中的令牌。列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateTokenInMemberServiceProduct({ payload }, { call, put }) {
      ...
      const location = { pathname: `/memberServiceProduct/${id}/list/${type}List/成员服务产品中的令牌。列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *addMemberServiceBoundleSku({ payload }, { call, put }) {
      const {MemberServiceProductService} = GlobalComponents;

      const { id, type, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(MemberServiceProductService.addMemberServiceBoundleSku, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const partialList = true
      const newState = {...data, partialList}
      const location = { pathname: `/memberServiceProduct/${id}/list/${type}List/会员服务Boundle Sku列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateMemberServiceBoundleSku({ payload }, { call, put }) {
      const {MemberServiceProductService} = GlobalComponents;      
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      console.log('get form parameters', parameters)
      const data = yield call(MemberServiceProductService.updateMemberServiceBoundleSku, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/memberServiceProduct/${id}/list/${type}List/会员服务Boundle Sku列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    *gotoNextMemberServiceBoundleSkuUpdateRow({ payload }, { call, put }) {
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      const newPlayload = { ...payload, selectedRows, currentUpdateIndex }
      yield put({ type: 'updateState', payload: newPlayload })
    },
    *removeMemberServiceBoundleSkuList({ payload }, { call, put }) {
      const {MemberServiceProductService} = GlobalComponents; 
      const { id, type, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(MemberServiceProductService.removeMemberServiceBoundleSkuList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
