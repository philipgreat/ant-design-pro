...
import styles from './MemberServiceProduct.edittable.less'
...
class MemberServiceProductEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const memberServiceProductIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {memberServiceProductId} = record.id
      const parameters = { ...record, memberServiceProductId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newMemberServiceProductToAppend  = {
      	'id':`+1`, 
				'productName':'',
				'productDescription':'',
				'joinWorkshop':'',
      ...
      newData.push(newMemberServiceProductToAppend);
...
export default Form.create()(MemberServiceProductEditTable)
