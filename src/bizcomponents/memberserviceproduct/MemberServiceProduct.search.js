...
import styles from './MemberServiceProduct.search.less'
...
class MemberServiceProductSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const memberServiceProductIds = selectedRows.map((item) => { return item.id })
    console.log('memberServiceProductIds', memberServiceProductIds)
    const parameters = { memberServiceProductIds }
    dispatch({
      type: `${owner.type}/removeMemberServiceProductList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberServiceProduct' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberServiceProduct', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'会员服务产品列表' } })
  ...
  render() {
    ...
    const {MemberServiceProductTable} = GlobalComponents;
    const {MemberServiceProductConfirmationTable} = GlobalComponents;
    const {MemberServiceProductSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <MemberServiceProductConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 会员服务产品列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <MemberServiceProductSearchForm {...this.props} />
            </div>
...
export default Form.create()(MemberServiceProductSearch)
