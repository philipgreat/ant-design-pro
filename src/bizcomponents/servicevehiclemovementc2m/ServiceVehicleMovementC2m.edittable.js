...
import styles from './ServiceVehicleMovementC2m.edittable.less'
...
class ServiceVehicleMovementC2mEditTable extends PureComponent {
  constructor(props) {
    super(props)
    console.log(props)
    this.state = {
  ...
  render() {
    ...
    const deleteRecord = (e, record) => {
      ...
      const { data } = this.state
      const serviceVehicleMovementC2mIds = [record.id]
    ...
    const updateRecord = (e, record) => {
      const { dispatch, owner } = this.props
      const { data } = this.state
      const { serviceVehicleMovementC2mId } = record.id
      const parameters = { ...record, serviceVehicleMovementC2mId }
      const newData = [...data]
      const row = newData.filter(item => item.id === record.id)[0]
      row.editable = !row.editable

      dispatch({
        type: `${owner.type}/updateServiceVehicleMovementC2m`,
        payload: {
          id: owner.id,
          type: 'serviceVehicleMovementC2m',
          parameters,
    ...
    const newRecord = () => {
      const newServiceVehicleMovementC2mToAppend = {
        id: `+1`,
        serviceStatus: '',
        responsibleWorker: '',
        rejectComments: '',
      ...
      newData.push(newServiceVehicleMovementC2mToAppend)
...
export default Form.create()(ServiceVehicleMovementC2mEditTable)
