  ...
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="通知日期时间">
        {moment(serviceVehicleMovementC2m.notifyDatetime).format('YYYY-MM-DD')}
      </Description>
      <Description term="通知地址">
        {serviceVehicleMovementC2m.notifyAddress}
      </Description>
      <Description term="备注">
        {serviceVehicleMovementC2m.notifyComment}
      </Description>
      <Description term="交接检查结果">
        {serviceVehicleMovementC2m.handoverResult}
      </Description>
      <Description term="交接检查备注">
        {serviceVehicleMovementC2m.handoverResultComment}
      </Description>
    </DescriptionList>
  )
}
...
export default class ServiceVehicleMovementC2mDashboard extends Component {
  render() {
    ...
    } = this.props.serviceVehicleMovementC2m
        ...
        <div>
          <Row gutter={24}>
            <Col {...topColResponsiveProps}>
              ...
              >
                <Link
                  to={`/serviceVehicleMovementC2m/${id}/list/handOverChecklistResultList`}
                >
                  <Icon
                    type="profile"
                ...
                &nbsp;
                <Link
                  to={`/serviceVehicleMovementC2m/${id}/list/handOverChecklistResultCreateForm`}
                >
                  <Icon
                    type="plus-circle-o"
                    style={{ fontSize: 20, color: '#08c' }}
