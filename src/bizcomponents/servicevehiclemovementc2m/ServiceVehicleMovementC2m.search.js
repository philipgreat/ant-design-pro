...
class ServiceVehicleMovementC2mSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const serviceVehicleMovementC2mIds = selectedRows.map((item) => { return item.id })
    console.log('serviceVehicleMovementC2mIds', serviceVehicleMovementC2mIds)
    const parameters = { serviceVehicleMovementC2mIds }
    dispatch({
      type: `${owner.type}/removeServiceVehicleMovementC2mList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'serviceVehicleMovementC2m' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'serviceVehicleMovementC2m', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'收车服务列表' } })
  ...
  render() {
    ...
    const {ServiceVehicleMovementC2mTable} = GlobalComponents;
    const {ServiceVehicleMovementC2mConfirmationTable} = GlobalComponents;
    const {ServiceVehicleMovementC2mSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ServiceVehicleMovementC2mConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 收车服务列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ServiceVehicleMovementC2mSearchForm {...this.props} />
            </div>
...
export default Form.create()(ServiceVehicleMovementC2mSearch)
