...
import styles from './ReportHandover.search.less'
...
@Form.create()
export default class ReportHandoverSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const reportHandoverIds = selectedRows.map((item) => { return item.id })
    console.log('reportHandoverIds', reportHandoverIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'reportHandover', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {ReportHandoverTable} = GlobalComponents;
    const {ReportHandoverConfirmationTable} = GlobalComponents;
    const {ReportHandoverSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ReportHandoverConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="交接报告列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ReportHandoverSearchForm {...this.props} />
            </div>
