...
import styles from './ReportHandover.updateform.less'
...
class ReportHandoverUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const reportHandoverId = values.id
        const imagesValues = this.mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, reportHandoverId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextReportHandoverUpdateRow`,
        payload: {
          id: owner.id,
          type: 'reportHandover',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新交接报告"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新交接报告"
...
}))(Form.create()(ReportHandoverUpdateForm))
