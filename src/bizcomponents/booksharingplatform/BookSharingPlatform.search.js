...
import styles from './BookSharingPlatform.search.less'
...
class BookSharingPlatformSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookSharingPlatformIds = selectedRows.map((item) => { return item.id })
    console.log('bookSharingPlatformIds', bookSharingPlatformIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookSharingPlatform' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookSharingPlatform', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'书共享平台列表' } })
  ...
  render() {
    ...
    const {BookSharingPlatformTable} = GlobalComponents;
    const {BookSharingPlatformConfirmationTable} = GlobalComponents;
    const {BookSharingPlatformSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookSharingPlatformConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 书共享平台列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookSharingPlatformSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookSharingPlatformSearch)
