...
export default {
  ...
  effects: {
    ...
    *addAvailableToken({ payload }, { call, put }) {
      ...
      const location = { pathname: `/bookSharingPlatform/${id}/list/${type}List/使用令牌列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateAvailableToken({ payload }, { call, put }) {
      ...
      const location = { pathname: `/bookSharingPlatform/${id}/list/${type}List/使用令牌列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *addPrinter({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(BookSharingPlatformService.addPrinter, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const partialList = true
      const newState = {...data, partialList}
      const location = { pathname: `/bookSharingPlatform/${id}/list/${type}List/打印机列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updatePrinter({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      console.log('get form parameters', parameters)
      const data = yield call(BookSharingPlatformService.updatePrinter, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/bookSharingPlatform/${id}/list/${type}List/打印机列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *removePrinterList({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(BookSharingPlatformService.removePrinterList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
