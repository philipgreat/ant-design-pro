...
import styles from './ServiceOrderFilter.search.less'
...
class ServiceOrderFilterSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const serviceOrderFilterIds = selectedRows.map((item) => { return item.id })
    console.log('serviceOrderFilterIds', serviceOrderFilterIds)
    const parameters = { serviceOrderFilterIds }
    dispatch({
      type: `${owner.type}/removeServiceOrderFilterList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'serviceOrderFilter' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'serviceOrderFilter', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'服务单状态类型列表' } })
  ...
  render() {
    ...
    const {ServiceOrderFilterTable} = GlobalComponents;
    const {ServiceOrderFilterConfirmationTable} = GlobalComponents;
    const {ServiceOrderFilterSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ServiceOrderFilterConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 服务单状态类型列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ServiceOrderFilterSearchForm {...this.props} />
            </div>
...
export default Form.create()(ServiceOrderFilterSearch)
