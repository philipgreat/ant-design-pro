...
import styles from './ServiceOrderFilter.edittable.less'
...
class ServiceOrderFilterEditTable extends PureComponent {
  constructor(props) {
    super(props)
    console.log(props)
    this.state = {
  ...
  render() {
    ...
    const deleteRecord = (e, record) => {
      ...
      const { data } = this.state
      const serviceOrderFilterIds = [record.id]
    ...
    const updateRecord = (e, record) => {
      const { dispatch, owner } = this.props
      const { data } = this.state
      const { serviceOrderFilterId } = record.id
      const parameters = { ...record, serviceOrderFilterId }
      const newData = [...data]
      const row = newData.filter(item => item.id === record.id)[0]
      row.editable = !row.editable
    ...
    const columns = [
      { title: 'ID', debugtype: 'string', dataIndex: 'id', width: '20' },
      {
        ...
        width: '8',
        render: (text, record) => renderStringEdit('filterName', text, record),
      },
      {
        title: '订单数量',
        debugtype: 'int',
        ...
        width: '6',
        render: (text, record) => renderStringEdit('orderCount', text, record),
      },
      {
        title: '选择',
        dataIndex: 'selected',
    ...
    const newRecord = () => {
      ...
      newData.push(newServiceOrderFilterToAppend)
...
export default Form.create()(ServiceOrderFilterEditTable)
