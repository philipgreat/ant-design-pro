...
import styles from './ServiceOrderFilter.updateform.less'
...
class ServiceOrderFilterUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const serviceOrderFilterId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, serviceOrderFilterId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextServiceOrderFilterUpdateRow`,
        payload: {
          id: owner.id,
          type: 'serviceOrderFilter',
          selectedRows,
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label={fieldLabels.filterName}>
                  {getFieldDecorator('filterName', {
                    rules: [
                      { required: true, message: '请输入服务单状态名称' },
                    ],
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label={fieldLabels.linkUrl}>
                  {getFieldDecorator('linkUrl', {
                    rules: [
                      { required: true, message: '请输入服务单状态接口' },
                    ],
...
}))(Form.create()(ServiceOrderFilterUpdateForm))
