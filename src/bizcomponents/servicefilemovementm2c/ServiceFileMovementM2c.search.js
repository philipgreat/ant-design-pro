...
class ServiceFileMovementM2cSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'还件服务列表' } })
...
<Menu.Item key="merchant">关联商户</Menu.Item>
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 还件服务列表`}>
        <Card bordered={false}>
...
export default Form.create()(ServiceFileMovementM2cSearch)
