...
import styles from './ServiceFileMovementM2c.search.less'
...
@Form.create()
export default class ServiceFileMovementM2cSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    const params = {
      serviceFileMovementM2cList: 1,
      serviceFileMovementM2cListCurrentPage: pagination.current,
      serviceFileMovementM2cListRowsPerPage: pagination.pageSize,
      ...formValues,
      ...filters,
    }
    if (sorter.field) {
  ...
  handleDelete = () => {
    ...
    const serviceFileMovementM2cIds = selectedRows.map((item) => { return item.id })
    console.log('serviceFileMovementM2cIds', serviceFileMovementM2cIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'serviceFileMovementM2c', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {ServiceFileMovementM2cTable} = GlobalComponents;
    const {ServiceFileMovementM2cConfirmationTable} = GlobalComponents;
    const {ServiceFileMovementM2cSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ServiceFileMovementM2cConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="还件服务列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ServiceFileMovementM2cSearchForm {...this.props} />
            </div>
