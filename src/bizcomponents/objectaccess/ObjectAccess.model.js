...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        const dashboardmatch = pathToRegexp('/objectAccess/:id/dashboard').exec(
          pathname
        )
        if (dashboardmatch) {
        ...
        const match = pathToRegexp('/objectAccess/:id/list/:listName').exec(
          pathname
        )
        if (!match) {
          return
  ...
  effects: {
    ...
    *gotoUpdateForm({ payload }, { put }) {
      ...
      const location = {
        pathname: `/objectAccess/${id}/list/${type}UpdateForm`,
        state,
      }
