...
import styles from './CompanyDiscount.createform.less'
...
class CompanyDiscountCreateForm extends Component {
  state = {
  ...
  executeCandidateCompanySearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = CompanyDiscountService.requestCandidateCompany("vehicleServiceCompany", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addCompanyDiscount`,
          payload: { id: owner.id, type: 'companyDiscount', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'companyDiscount',listName:'公司折扣列表' },
      })
    }
              ...
              <Col lg={8} md={12} sm={24}>
                <Form.Item label={fieldLabels.allFreeDiscount}  {...switchFormItemLayout}>
                  {getFieldDecorator('allFreeDiscount', {
                    ...
                    <Switch checkedChildren="是" unCheckedChildren="否"  placeholder="请输入代理费全免bool" />
                  )}
                </Form.Item>
              </Col>
...
}))(Form.create()(CompanyDiscountCreateForm))
