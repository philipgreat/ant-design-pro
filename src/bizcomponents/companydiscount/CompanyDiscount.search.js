...
import styles from './CompanyDiscount.search.less'
...
class CompanyDiscountSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const companyDiscountIds = selectedRows.map((item) => { return item.id })
    console.log('companyDiscountIds', companyDiscountIds)
    const parameters = { companyDiscountIds }
    dispatch({
      type: `${owner.type}/removeCompanyDiscountList`,
      payload: { id: owner.id, type: 'companyDiscount', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'companyDiscount' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'companyDiscount', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'公司折扣列表' } })
  ...
  render() {
    ...
    const {CompanyDiscountTable} = GlobalComponents;
    const {CompanyDiscountConfirmationTable} = GlobalComponents;
    const {CompanyDiscountSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CompanyDiscountConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 公司折扣列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CompanyDiscountSearchForm {...this.props} />
            </div>
...
export default Form.create()(CompanyDiscountSearch)
