...
import styles from './CompanyDiscount.edittable.less'
...
class CompanyDiscountEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const companyDiscountIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addCompanyDiscount`,
        payload: {
          id: owner.id,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {companyDiscountId} = record.id
      const parameters = { ...record, companyDiscountId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '优惠金额', debugtype: 'string', dataIndex: 'directDiscountValue', width: '9', render: (text, record) => renderStringEdit('directDiscountValue',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newCompanyDiscountToAppend  = {
      	'id':`+1`, 
				'allFreeDiscount':'',
				'directDiscountValue':'',
				'company':'',
      ...
      newData.push(newCompanyDiscountToAppend);
...
export default Form.create()(CompanyDiscountEditTable)
