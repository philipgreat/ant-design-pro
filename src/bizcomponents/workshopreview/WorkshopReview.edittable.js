...
import styles from './WorkshopReview.edittable.less'
...
class WorkshopReviewEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
      ...
      return {workshopId,reviewerId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const workshopReviewIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {workshopReviewId} = record.id
      const parameters = { ...record, workshopReviewId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '审查内容', debugtype: 'string', dataIndex: 'reviewContent', width: '16', render: (text, record) => renderStringEdit('reviewContent',text, record)  },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newWorkshopReviewToAppend  = {
      	'id':`+1`, 
				'workshop':'',
				'reviewer':'',
      ...
      newData.push(newWorkshopReviewToAppend);
...
export default Form.create()(WorkshopReviewEditTable)
