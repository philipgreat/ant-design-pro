...
import styles from './WorkshopReview.search.less'
...
class WorkshopReviewSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const workshopReviewIds = selectedRows.map((item) => { return item.id })
    console.log('workshopReviewIds', workshopReviewIds)
    const parameters = { workshopReviewIds }
    dispatch({
      type: `${owner.type}/removeWorkshopReviewList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'workshopReview' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'workshopReview', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'车间检查列表' } })
  ...
  render() {
    ...
    const {WorkshopReviewTable} = GlobalComponents;
    const {WorkshopReviewConfirmationTable} = GlobalComponents;
    const {WorkshopReviewSearchForm} = GlobalComponents;
...
<Menu.Item key="reviewer">关联评论家</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <WorkshopReviewConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 车间检查列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <WorkshopReviewSearchForm {...this.props} />
            </div>
...
export default Form.create()(WorkshopReviewSearch)
