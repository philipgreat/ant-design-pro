...
@Form.create()
export default class EmployeeCompanyTrainingSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const employeeCompanyTrainingIds = selectedRows.map((item) => { return item.id })
    console.log('employeeCompanyTrainingIds', employeeCompanyTrainingIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employeeCompanyTraining', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {EmployeeCompanyTrainingConfirmationTable} = GlobalComponents;
...
<Menu.Item key="training">关联训练</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EmployeeCompanyTrainingConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 员工参与的公司培训列表`}>
        <Card bordered={false}>
