...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        const dashboardmatch = pathToRegexp('/userApp/:id/dashboard').exec(
          pathname
        )
        if (dashboardmatch) {
        ...
        const editDetailMatch = pathToRegexp('/userApp/:id/editDetail').exec(
          pathname
        )
        if (editDetailMatch) {
        ...
        const viewDetailMatch = pathToRegexp('/userApp/:id/viewDetail').exec(
          pathname
        )
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp(
          '/userApp/:id/list/:listName/:listDisplayName'
        ).exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *gotoUpdateForm({ payload }, { put }) {
      ...
      const location = {
        pathname: `/userApp/${id}/list/${type}UpdateForm`,
        state,
      }
    ...
    *addObjectAccess({ payload }, { call, put }) {
      ...
      const location = {
        pathname: `/userApp/${id}/list/${type}List`,
        state: data,
      }
      yield put(routerRedux.push(location))
    },
    ...
    *removeObjectAccessList({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(
        UserAppService.removeObjectAccessList,
        id,
        parameters
      )
      if (hasError(data)) {
