...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        const editDetailMatch = pathToRegexp('/userApp/:id/editDetail').exec(pathname)
        if (editDetailMatch) {
          const id = editDetailMatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const viewDetailMatch = pathToRegexp('/userApp/:id/viewDetail').exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp('/userApp/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *updateObjectAccess({ payload }, { call, put }) {
      ...
      const location = { pathname: `/userApp/${id}/list/${type}List`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
