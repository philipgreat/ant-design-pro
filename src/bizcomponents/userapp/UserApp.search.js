...
import UserAppTable from './UserApp.table';
import UserAppConfirmationTable from './UserApp.confirmmationtable';

import UserAppSearchForm from './UserApp.searchform';
...
@Form.create()
export default class UserAppSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    const params = {
      userAppList: 1,
      userAppListCurrentPage: pagination.current,
      userAppListRowsPerPage: pagination.pageSize,
      ...formValues,
      ...filters,
    };
    if (sorter.field) {
  ...
  handleDelete = () => {
    ...
    const userAppIds = selectedRows.map((item) => { return item.id; });
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'userApp', selectedRows, currentUpdateIndex },
    });
  }
