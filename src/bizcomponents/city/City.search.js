...
import styles from './City.search.less'
...
@Form.create()
export default class CitySearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const cityIds = selectedRows.map((item) => { return item.id })
    console.log('cityIds', cityIds)
    const parameters = { cityIds }
    dispatch({
      type: `${owner.type}/removeCityList`,
      payload: { id: owner.id, type: 'city', parameters },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'city', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'城市列表' } })
  ...
  render() {
    ...
    const {CityTable} = GlobalComponents;
    const {CityConfirmationTable} = GlobalComponents;
    const {CitySearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CityConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 城市列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CitySearchForm {...this.props} />
            </div>
