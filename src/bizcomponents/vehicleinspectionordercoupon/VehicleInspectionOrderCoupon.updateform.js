...
import styles from './VehicleInspectionOrderCoupon.updateform.less'
...
class VehicleInspectionOrderCouponUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...
        appliedDate: moment(item.appliedDate),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const vehicleInspectionOrderCouponId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, vehicleInspectionOrderCouponId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextVehicleInspectionOrderCouponUpdateRow`,
        payload: {
          id: owner.id,
          type: 'vehicleInspectionOrderCoupon',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新优惠券使用记录"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新优惠券使用记录"
...
}))(Form.create()(VehicleInspectionOrderCouponUpdateForm))
