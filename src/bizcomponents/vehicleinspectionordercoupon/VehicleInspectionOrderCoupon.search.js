...
import styles from './VehicleInspectionOrderCoupon.search.less'
...
class VehicleInspectionOrderCouponSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const vehicleInspectionOrderCouponIds = selectedRows.map((item) => { return item.id })
    console.log('vehicleInspectionOrderCouponIds', vehicleInspectionOrderCouponIds)
    const parameters = { vehicleInspectionOrderCouponIds }
    dispatch({
      type: `${owner.type}/removeVehicleInspectionOrderCouponList`,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'vehicleInspectionOrderCoupon', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'优惠券使用记录列表' } })
  ...
  render() {
    ...
    const {VehicleInspectionOrderCouponTable} = GlobalComponents;
    const {VehicleInspectionOrderCouponConfirmationTable} = GlobalComponents;
    const {VehicleInspectionOrderCouponSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <VehicleInspectionOrderCouponConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 优惠券使用记录列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <VehicleInspectionOrderCouponSearchForm {...this.props} />
            </div>
...
export default Form.create()(VehicleInspectionOrderCouponSearch)
