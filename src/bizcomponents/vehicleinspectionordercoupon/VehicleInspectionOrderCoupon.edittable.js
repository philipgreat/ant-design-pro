...
import styles from './VehicleInspectionOrderCoupon.edittable.less'
...
class VehicleInspectionOrderCouponEditTable extends PureComponent {
  constructor(props) {
    super(props)
    console.log(props)
    this.state = {
  ...
  render() {
    ...
    const deleteRecord = (e, record) => {
      ...
      const { data } = this.state
      const vehicleInspectionOrderCouponIds = [record.id]
    ...
    const updateRecord = (e, record) => {
      const { dispatch, owner } = this.props
      const { data } = this.state
      const { vehicleInspectionOrderCouponId } = record.id
      const parameters = { ...record, vehicleInspectionOrderCouponId }
      const newData = [...data]
      const row = newData.filter(item => item.id === record.id)[0]
      row.editable = !row.editable

      dispatch({
        type: `${owner.type}/updateVehicleInspectionOrderCoupon`,
        payload: {
          id: owner.id,
          type: 'vehicleInspectionOrderCoupon',
          parameters,
    ...
    const columns = [
      { title: 'ID', debugtype: 'string', dataIndex: 'id', width: '20' },
      {
        ...
        width: '10',
        render: (text, record) => renderStringEdit('couponTitle', text, record),
      },
      {
        title: '折扣金额',
      ...
      },
      {
        ...
        width: '7',
        render: (text, record) =>
          renderStringEdit('couponStatus', text, record),
      },
      {
        title: '客户',
        dataIndex: 'customer',
        render: (text, record) =>
          record.customer ? record.customer.id : '暂无',
      },
      {
        title: '主订单',
        dataIndex: 'mainOrder',
    ...
    const newRecord = () => {
      ...
      newData.push(newVehicleInspectionOrderCouponToAppend)
...
export default Form.create()(VehicleInspectionOrderCouponEditTable)
