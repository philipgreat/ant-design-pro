...
import styles from './TaskPage.edittable.less'
...
class TaskPageEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {taskPageId} = record.id
      const parameters = { ...record, taskPageId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateTaskPage`,
        payload: {
          id: owner.id,
          type: 'taskPage',
          parameters,
  ...
  { title: '标题', debugtype: 'string', dataIndex: 'title', width: '6', render: (text, record) => renderStringEdit('title',text, record)  },
  { title: '当前健值', debugtype: 'string', dataIndex: 'currentKey', width: '25', render: (text, record) => renderStringEdit('currentKey',text, record)  },
  { title: '社区', dataIndex: 'community', render: (text, record) => (record.community ? record.community.displayName : '暂无') },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newTaskPageToAppend  = {
      	'id':`+1`, 
				'title':'',
				'currentKey':'',
				'community':'',
      ...
      newData.push(newTaskPageToAppend);
...
export default Form.create()(TaskPageEditTable)
