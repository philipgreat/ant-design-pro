...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        const editDetailMatch = pathToRegexp('/taskPage/:id/editDetail').exec(pathname)
        if (editDetailMatch) {
          const id = editDetailMatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const viewDetailMatch = pathToRegexp('/taskPage/:id/viewDetail').exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp('/taskPage/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    *view({ payload }, { call, put }) { 
      const {TaskPageService} = GlobalComponents;
    ...
    *gotoUpdateForm({ payload }, { put }) {
      ...
      const location = { pathname: `/taskPage/${id}/list/${type}UpdateForm`, state }
      yield put(routerRedux.push(location))
    },
    *goback({ payload }, { put }) {
      const { id, type,listName } = payload
      yield put(routerRedux.push(`/taskPage/${id}/list/${type}List/${listName}`))
    },
    ...
    *addTaskFilter({ payload }, { call, put }) {
      ...
      const location = { pathname: `/taskPage/${id}/list/${type}List/任务过滤器列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateTaskFilter({ payload }, { call, put }) {
      ...
      const location = { pathname: `/taskPage/${id}/list/${type}List/任务过滤器列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *addTask({ payload }, { call, put }) {
      ...
      const partialList = true
      const newState = {...data, partialList}
      const location = { pathname: `/taskPage/${id}/list/${type}List/任务列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateTask({ payload }, { call, put }) {
      ...
      const location = { pathname: `/taskPage/${id}/list/${type}List/任务列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
