...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        const dashboardmatch = pathToRegexp('/taskPage/:id/dashboard').exec(
          pathname
        )
        if (dashboardmatch) {
        ...
        const editDetailMatch = pathToRegexp('/taskPage/:id/editDetail').exec(
          pathname
        )
        if (editDetailMatch) {
        ...
        const viewDetailMatch = pathToRegexp('/taskPage/:id/viewDetail').exec(
          pathname
        )
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp('/taskPage/:id/list/:listName').exec(
          pathname
        )
        if (!match) {
          return
  ...
  effects: {
    ...
    *gotoUpdateForm({ payload }, { put }) {
      ...
      const location = {
        pathname: `/taskPage/${id}/list/${type}UpdateForm`,
        state,
      }
    ...
    *addTaskFilter({ payload }, { call, put }) {
      ...
      const location = {
        pathname: `/taskPage/${id}/list/${type}List`,
        state: data,
      }
      yield put(routerRedux.push(location))
    },
    ...
    *removeTaskFilterList({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(
        TaskPageService.removeTaskFilterList,
        id,
        parameters
      )
      if (hasError(data)) {
