...
import styles from './TaskPage.createform.less'
...
const testValues = {
  title: '求助',
  currentKey: 'ADDITIONAL_BONUS_TASK',
  communityId: 'C000001',
}
*/
...
class TaskPageCreateForm extends Component {
  state = {
  ...
  executeCandidateCommunitySearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = TaskPageService.requestCandidateCommunity("community", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addTaskPage`,
          payload: { id: owner.id, type: 'taskPage', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'taskPage',listName:'任务页面列表' },
      })
    }
        ...
        <Card title="基础信息" className={styles.card} bordered={false}>
          <Form >
            <Row gutter={16}>
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.currentKey} {...formItemLayout}>
                  {getFieldDecorator('currentKey', {
                    rules: [{ required: true, message: '请输入当前健值' }],
                  })(
                    <Input placeholder="请输入当前健值" />
                  )}
                </Form.Item>
              </Col>
