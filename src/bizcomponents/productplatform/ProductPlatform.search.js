...
import styles from './ProductPlatform.search.less'
...
@Form.create()
export default class ProductPlatformSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const productPlatformIds = selectedRows.map((item) => { return item.id })
    console.log('productPlatformIds', productPlatformIds)
    const parameters = { productPlatformIds }
    dispatch({
      type: `${owner.type}/removeProductPlatformList`,
      payload: { id: owner.id, type: 'productPlatform', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'productPlatform' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'productPlatform', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {ProductPlatformTable} = GlobalComponents;
    const {ProductPlatformConfirmationTable} = GlobalComponents;
    const {ProductPlatformSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ProductPlatformConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 产品平台列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ProductPlatformSearchForm {...this.props} />
            </div>
