...
import styles from './AvailableProduct.updateform.less'
...
class AvailableProductUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const availableProductId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, availableProductId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextAvailableProductUpdateRow`,
        payload: {
          id: owner.id,
          type: 'availableProduct',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={
          '更新产品类型' + (currentUpdateIndex + 1) + '/' + selectedRows.length
        }
        content="更新产品类型"
...
}))(Form.create()(AvailableProductUpdateForm))
