...
import styles from './AvailableProduct.search.less'
...
class AvailableProductSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const availableProductIds = selectedRows.map((item) => { return item.id })
    console.log('availableProductIds', availableProductIds)
    const parameters = { availableProductIds }
    dispatch({
      type: `${owner.type}/removeAvailableProductList`,
      payload: { id: owner.id, type: 'availableProduct', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'availableProduct' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'availableProduct', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'产品类型列表' } })
  ...
  render() {
    ...
    const {AvailableProductTable} = GlobalComponents;
    const {AvailableProductConfirmationTable} = GlobalComponents;
    const {AvailableProductSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <AvailableProductConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 产品类型列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <AvailableProductSearchForm {...this.props} />
            </div>
...
export default Form.create()(AvailableProductSearch)
