...
import styles from './Ad.dashboard.less'
...
class AdDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, ad } = this.props;
    
    if(!ad){
    	return;
    }
    const {displayName} = ad;
    if(!displayName){
    	return;
    }
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.ad
    const cardsData = {cardsName:"广告",cardsFor: "ad",cardsSource: this.props.ad,
  		subItems: [
