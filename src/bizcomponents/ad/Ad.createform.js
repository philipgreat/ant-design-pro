...
import styles from './Ad.createform.less'
...
class AdCreateForm extends Component {
  ...
  executeCandidateStoreSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = AdService.requestCandidateStore("store", id, filterKey, pageNo);
    console.log(future);
  ...
  executeCandidateGamePlatformSearch = (filterKey) =>{
    ...
    const id = "";//not used for now
    const pageNo = 1;
    const future = AdService.requestCandidateGamePlatform("gamePlatform", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addAd`,
          payload: { id: owner.id, type: 'ad', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'ad',listName:'广告列表' },
      })
    }
