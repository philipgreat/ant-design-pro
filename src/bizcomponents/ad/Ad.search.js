...
import styles from './Ad.search.less'
...
class AdSearch extends PureComponent {
  ...
  handleDelete = () => {
    ...
    const adIds = selectedRows.map((item) => { return item.id })
    console.log('adIds', adIds)
    const parameters = { adIds }
    dispatch({
      type: `${owner.type}/removeAdList`,
      payload: { id: owner.id, type: 'ad', parameters },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'ad', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'广告列表' } })
  ...
  render() {
    ...
    const {AdTable} = GlobalComponents;
    const {AdConfirmationTable} = GlobalComponents;
    const {AdSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <AdConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 广告列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <AdSearchForm {...this.props} />
            </div>
...
export default Form.create()(AdSearch)
