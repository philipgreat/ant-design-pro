...
import styles from './Material.edittable.less'
...
class MaterialEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      ...
      const parameters = { ...record, materialId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateMaterial`,
        payload: {
          id: owner.id,
          type: 'material',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newMaterialToAppend);
...
export default Form.create()(MaterialEditTable)
