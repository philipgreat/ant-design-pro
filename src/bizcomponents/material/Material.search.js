...
import styles from './Material.search.less'
...
@Form.create()
export default class MaterialSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const materialIds = selectedRows.map((item) => { return item.id })
    console.log('materialIds', materialIds)
    const parameters = { materialIds }
    dispatch({
      type: `${owner.type}/removeMaterialList`,
      payload: { id: owner.id, type: 'material', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'material' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'material', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {MaterialTable} = GlobalComponents;
    const {MaterialConfirmationTable} = GlobalComponents;
    const {MaterialSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <MaterialConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 材料列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <MaterialSearchForm {...this.props} />
            </div>
