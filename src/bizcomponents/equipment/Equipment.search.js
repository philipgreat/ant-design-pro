...
import styles from './Equipment.search.less'
...
@Form.create()
export default class EquipmentSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const equipmentIds = selectedRows.map((item) => { return item.id })
    console.log('equipmentIds', equipmentIds)
    const parameters = { equipmentIds }
    dispatch({
      type: `${owner.type}/removeEquipmentList`,
      payload: { id: owner.id, type: 'equipment', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'equipment' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'equipment', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {EquipmentTable} = GlobalComponents;
    const {EquipmentConfirmationTable} = GlobalComponents;
    const {EquipmentSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EquipmentConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
        ...
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <EquipmentSearchForm {...this.props} />
            </div>
