...
import styles from './ThreadLike.search.less'
...
class ThreadLikeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const threadLikeIds = selectedRows.map((item) => { return item.id })
    console.log('threadLikeIds', threadLikeIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'threadLike', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'主贴点赞列表' } })
  ...
  render() {
    ...
    const {ThreadLikeTable} = GlobalComponents;
    const {ThreadLikeConfirmationTable} = GlobalComponents;
    const {ThreadLikeSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ThreadLikeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 主贴点赞列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ThreadLikeSearchForm {...this.props} />
            </div>
...
export default Form.create()(ThreadLikeSearch)
