...
class ThreadLikeUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const threadLikeId = values.id
        const imagesValues = this.mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, threadLikeId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextThreadLikeUpdateRow`,
        payload: {
          id: owner.id,
          type: 'threadLike',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={
          '更新主贴点赞' + (currentUpdateIndex + 1) + '/' + selectedRows.length
        }
        content="更新主贴点赞"
...
}))(Form.create()(ThreadLikeUpdateForm))
