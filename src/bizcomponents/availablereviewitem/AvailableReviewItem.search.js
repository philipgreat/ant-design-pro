...
import styles from './AvailableReviewItem.search.less'
...
class AvailableReviewItemSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const availableReviewItemIds = selectedRows.map((item) => { return item.id })
    console.log('availableReviewItemIds', availableReviewItemIds)
    const parameters = { availableReviewItemIds }
    dispatch({
      type: `${owner.type}/removeAvailableReviewItemList`,
      payload: { id: owner.id, type: 'availableReviewItem', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'availableReviewItem' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'availableReviewItem', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'评论列表' } })
  ...
  render() {
    ...
    const {AvailableReviewItemTable} = GlobalComponents;
    const {AvailableReviewItemConfirmationTable} = GlobalComponents;
    const {AvailableReviewItemSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <AvailableReviewItemConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 评论列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <AvailableReviewItemSearchForm {...this.props} />
            </div>
...
export default Form.create()(AvailableReviewItemSearch)
