...
import styles from './Contract.edittable.less'
...
class ContractEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      ...
      const parameters = { ...record, contractId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateContract`,
        payload: {
          id: owner.id,
          type: 'contract',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newContractToAppend);
...
export default Form.create()(ContractEditTable)
