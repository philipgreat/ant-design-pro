...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        const match = pathToRegexp('/contract/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *goback({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/contract/${id}/list/${type}List/${listName}`))
    },
    ...
    *addServicePrice({ payload }, { call, put }) {
      ...
      const data = yield call(ContractService.addServicePrice, id, parameters)
      if (hasError(data)) {
      ...
      const location = { pathname: `/contract/${id}/list/${type}List/合同价格列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateServicePrice({ payload }, { call, put }) {
      ...
      const location = { pathname: `/contract/${id}/list/${type}List/合同价格列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
