...
import styles from './Contract.search.less'
...
class ContractSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const contractIds = selectedRows.map((item) => { return item.id })
    console.log('contractIds', contractIds)
    const parameters = { contractIds }
    dispatch({
      type: `${owner.type}/removeContractList`,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'contract', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'合同列表' } })
  ...
  render() {
    ...
    const {ContractTable} = GlobalComponents;
    const {ContractConfirmationTable} = GlobalComponents;
    const {ContractSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ContractConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 合同列表`}>
        <Card bordered={false}>
...
export default Form.create()(ContractSearch)
