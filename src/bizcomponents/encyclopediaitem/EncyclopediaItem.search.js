...
class EncyclopediaItemSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const encyclopediaItemIds = selectedRows.map((item) => { return item.id })
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'encyclopediaItem', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'百科全书条目列表' } })
  ...
  render() {
    ...
    const {EncyclopediaItemTable} = GlobalComponents;
    const {EncyclopediaItemConfirmationTable} = GlobalComponents;
    const {EncyclopediaItemSearchForm} = GlobalComponents;
...
<Menu.Item key="community">关联社区</Menu.Item>
<Menu.Item key="homePage">关联主页</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EncyclopediaItemConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 百科全书条目列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <EncyclopediaItemSearchForm {...this.props} />
            </div>
...
export default Form.create()(EncyclopediaItemSearch)
