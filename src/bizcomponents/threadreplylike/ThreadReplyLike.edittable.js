...
import styles from './ThreadReplyLike.edittable.less'
...
class ThreadReplyLikeEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = record => {
      ...
      const threadReplyId = record.threadReply.id

      //const communityId = record.community.id;
      return { replierId, threadReplyId }
    }
    const deleteRecord = (e, record) => {
      const { dispatch, owner } = this.props
      const { data } = this.state
      const threadReplyLikeIds = [record.id]
    ...
    const updateRecord = (e, record) => {
      const { dispatch, owner } = this.props
      const { data } = this.state
      const { threadReplyLikeId } = record.id
      const parameters = { ...record, threadReplyLikeId }
      const newData = [...data]
      const row = newData.filter(item => item.id === record.id)[0]
      row.editable = !row.editable
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record) =>
          record.threadReply ? record.threadReply.displayName : '暂无',
      },
      {
        title: '操作',
    ...
    const newRecord = () => {
      const newThreadReplyLikeToAppend = {
        id: `+1`,
        likeTime: '',
        replier: '',
        threadReply: '',
      ...
      newData.push(newThreadReplyLikeToAppend)
...
export default Form.create()(ThreadReplyLikeEditTable)
