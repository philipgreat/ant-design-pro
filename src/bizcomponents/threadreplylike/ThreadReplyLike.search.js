...
import styles from './ThreadReplyLike.search.less'
...
class ThreadReplyLikeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const threadReplyLikeIds = selectedRows.map((item) => { return item.id })
    console.log('threadReplyLikeIds', threadReplyLikeIds)
    const parameters = { threadReplyLikeIds }
    dispatch({
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'threadReplyLike', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'跟帖回复点赞列表' } })
  ...
  render() {
    ...
    const {ThreadReplyLikeTable} = GlobalComponents;
    const {ThreadReplyLikeConfirmationTable} = GlobalComponents;
    const {ThreadReplyLikeSearchForm} = GlobalComponents;
...
<Menu.Item key="replier">关联应答者</Menu.Item>
<Menu.Item key="threadReply">关联跟帖回复</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ThreadReplyLikeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 跟帖回复点赞列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ThreadReplyLikeSearchForm {...this.props} />
            </div>
...
export default Form.create()(ThreadReplyLikeSearch)
