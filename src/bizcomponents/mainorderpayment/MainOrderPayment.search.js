...
class MainOrderPaymentSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const mainOrderPaymentIds = selectedRows.map((item) => { return item.id })
    console.log('mainOrderPaymentIds', mainOrderPaymentIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'mainOrderPayment' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'mainOrderPayment', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'主要订单付款列表' } })
  ...
  render() {
    ...
    const {MainOrderPaymentTable} = GlobalComponents;
    const {MainOrderPaymentConfirmationTable} = GlobalComponents;
    const {MainOrderPaymentSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <MainOrderPaymentConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 主要订单付款列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <MainOrderPaymentSearchForm {...this.props} />
            </div>
...
export default Form.create()(MainOrderPaymentSearch)
