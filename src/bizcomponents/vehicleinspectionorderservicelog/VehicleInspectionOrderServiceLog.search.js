...
class VehicleInspectionOrderServiceLogSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'年检订单执行日志列表' } })
...
<Menu.Item key="mainOrder">关联年检订单</Menu.Item>
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 年检订单执行日志列表`}>
        <Card bordered={false}>
...
export default Form.create()(VehicleInspectionOrderServiceLogSearch)
