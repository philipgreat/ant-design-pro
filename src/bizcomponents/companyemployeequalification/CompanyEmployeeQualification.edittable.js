...
import styles from './CompanyEmployeeQualification.edittable.less'
...
class CompanyEmployeeQualificationEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const companyEmployeeQualificationIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {companyEmployeeQualificationId} = record.id
      const parameters = { ...record, companyEmployeeQualificationId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateCompanyEmployeeQualification`,
        payload: {
          id: owner.id,
          type: 'companyEmployeeQualification',
          parameters,
  ...
  { title: '批注', debugtype: 'string', dataIndex: 'comment', width: '13', render: (text, record) => renderStringEdit('comment',text, record)  },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newCompanyEmployeeQualificationToAppend  = {
      	'id':`+1`, 
				'eventTime':'',
				'who':'',
				'comment':'',
      ...
      newData.push(newCompanyEmployeeQualificationToAppend);
...
export default Form.create()(CompanyEmployeeQualificationEditTable)
