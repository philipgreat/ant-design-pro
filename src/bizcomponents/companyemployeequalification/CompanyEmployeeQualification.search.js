...
import styles from './CompanyEmployeeQualification.search.less'
...
class CompanyEmployeeQualificationSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const companyEmployeeQualificationIds = selectedRows.map((item) => { return item.id })
    console.log('companyEmployeeQualificationIds', companyEmployeeQualificationIds)
    const parameters = { companyEmployeeQualificationIds }
    dispatch({
      type: `${owner.type}/removeCompanyEmployeeQualificationList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'companyEmployeeQualification' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'companyEmployeeQualification', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'商户员工资格审查列表' } })
  ...
  render() {
    ...
    const {CompanyEmployeeQualificationTable} = GlobalComponents;
    const {CompanyEmployeeQualificationConfirmationTable} = GlobalComponents;
    const {CompanyEmployeeQualificationSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CompanyEmployeeQualificationConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 商户员工资格审查列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CompanyEmployeeQualificationSearchForm {...this.props} />
            </div>
...
export default Form.create()(CompanyEmployeeQualificationSearch)
