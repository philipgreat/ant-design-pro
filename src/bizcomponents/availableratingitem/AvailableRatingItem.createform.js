...
import styles from './AvailableRatingItem.createform.less'
...
class AvailableRatingItemCreateForm extends Component {
  state = {
  ...
  executeCandidatePlatformSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = AvailableRatingItemService.requestCandidatePlatform("carInspectionPlatform", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addAvailableRatingItem`,
          payload: { id: owner.id, type: 'availableRatingItem', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'availableRatingItem',listName:'服务评分列表' },
      })
    }
...
}))(Form.create()(AvailableRatingItemCreateForm))
