...
const imageListOf = (availableRatingItem) =>{
...
const settingListOf = (availableRatingItem) =>{
...
const largeTextOf = (availableRatingItem) =>{
...
class AvailableRatingItemDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, availableRatingItem } = this.props;
    
    if(!availableRatingItem){
    	return;
    }
    const {displayName} = availableRatingItem;
    if(!displayName){
    	return;
    }
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.availableRatingItem
    const cardsData = {cardsName:"服务评分",cardsFor: "availableRatingItem",cardsSource: this.props.availableRatingItem,
  		subItems: [
