...
import styles from './AvailableRatingItem.search.less'
...
class AvailableRatingItemSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const availableRatingItemIds = selectedRows.map((item) => { return item.id })
    console.log('availableRatingItemIds', availableRatingItemIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'availableRatingItem' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'availableRatingItem', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'服务评分列表' } })
  ...
  render() {
    ...
    const {AvailableRatingItemTable} = GlobalComponents;
    const {AvailableRatingItemConfirmationTable} = GlobalComponents;
    const {AvailableRatingItemSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <AvailableRatingItemConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 服务评分列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <AvailableRatingItemSearchForm {...this.props} />
            </div>
...
export default Form.create()(AvailableRatingItemSearch)
