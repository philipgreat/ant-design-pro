...
@Form.create()
export default class DecorationCountryCenterSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const decorationCountryCenterIds = selectedRows.map((item) => { return item.id })
    console.log('decorationCountryCenterIds', decorationCountryCenterIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'decorationCountryCenter', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {DecorationCountryCenterTable} = GlobalComponents;
    const {DecorationCountryCenterConfirmationTable} = GlobalComponents;
    const {DecorationCountryCenterSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <DecorationCountryCenterConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 全国中心列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <DecorationCountryCenterSearchForm {...this.props} />
            </div>
