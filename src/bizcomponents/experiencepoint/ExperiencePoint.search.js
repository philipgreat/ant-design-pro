...
import ExperiencePointTable from './ExperiencePoint.table';
import ExperiencePointConfirmationTable from './ExperiencePoint.confirmmationtable';

import ExperiencePointSearchForm from './ExperiencePoint.searchform';

import PageHeaderLayout from '../../layouts/PageHeaderLayout';

import styles from './ExperiencePoint.search.less';
...
@Form.create()
export default class ExperiencePointSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    const params = {
      experiencePointList: 1,
      experiencePointListCurrentPage: pagination.current,
      experiencePointListRowsPerPage: pagination.pageSize,
      ...formValues,
      ...filters,
    };
    if (sorter.field) {
  ...
  handleDelete = () => {
    ...
    const experiencePointIds = selectedRows.map((item) => { return item.id; });
    console.log('experiencePointIds', experiencePointIds);
    const parameters = { experiencePointIds };
    dispatch({
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'experiencePoint', selectedRows, currentUpdateIndex },
    });
  }
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ExperiencePointConfirmationTable data={selectedRows} owner={owner} />
        </Modal>);
    };
    
    return (
      <PageHeaderLayout title="成长值列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ExperiencePointSearchForm {...this.props} />
            </div>
