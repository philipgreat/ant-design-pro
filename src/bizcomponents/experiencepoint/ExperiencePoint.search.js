...
class ExperiencePointSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'成长值列表' } })
  ...
  render() {
    ...
    const {ExperiencePointTable} = GlobalComponents;
    const {ExperiencePointConfirmationTable} = GlobalComponents;
    const {ExperiencePointSearchForm} = GlobalComponents;
...
<Menu.Item key="user">关联用户</Menu.Item>
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 成长值列表`}>
        <Card bordered={false}>
...
export default Form.create()(ExperiencePointSearch)
