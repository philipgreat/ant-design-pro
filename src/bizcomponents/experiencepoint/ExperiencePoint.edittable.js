...
import styles from './ExperiencePoint.edittable.less'
...
class ExperiencePointEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const experiencePointIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {experiencePointId} = record.id
      const parameters = { ...record, experiencePointId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateExperiencePoint`,
        payload: {
          id: owner.id,
          type: 'experiencePoint',
          parameters,
  ...
  { title: '点', debugtype: 'int', dataIndex: 'points', width: '7', render: (text, record) => renderStringEdit('points',text, record)  },
  { title: '用户', dataIndex: 'user', render: (text, record) => (record.user ? record.user.displayName : '暂无') },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newExperiencePointToAppend  = {
      	'id':`+1`, 
				'name':'',
				'obtainTime':'',
				'points':'',
      ...
      newData.push(newExperiencePointToAppend);
...
export default Form.create()(ExperiencePointEditTable)
