...
import styles from './ExperiencePoint.createform.less'
...
class ExperiencePointCreateForm extends Component {
  ...
  executeCandidateUserSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = ExperiencePointService.requestCandidateUser("communityUser", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'experiencePoint',listName:'成长值列表' },
      })
    }
