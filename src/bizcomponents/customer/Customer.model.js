...
export default {
  ...
  effects: {
    ...
    *addMemberCustomer({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(CustomerService.addMemberCustomer, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const partialList = true
      const newState = {...data, partialList}
      const location = { pathname: `/customer/${id}/list/${type}List/会员的客户列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateMemberCustomer({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext, selectedRows, currentUpdateIndex } = payload
      console.log('get form parameters', parameters)
      const data = yield call(CustomerService.updateMemberCustomer, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/customer/${id}/list/${type}List/会员的客户列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *removeMemberCustomerList({ payload }, { call, put }) {
      ...
      const { id, type, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(CustomerService.removeMemberCustomerList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
