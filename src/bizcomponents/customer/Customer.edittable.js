...
import styles from './Customer.edittable.less'
...
class CustomerEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const customerIds = [record.id];
      const parameters = { customerIds }
      dispatch({
        type: `${owner.type}/removeCustomerList`,
    ...
    const updateRecord = (e,record) =>{
      ...
      const parameters = { ...record, customerId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '尼克的名字', debugtype: 'string', dataIndex: 'nickName', width: '24', render: (text, record) => renderStringEdit('nickName',text, record)  },
  { title: '标志形象', dataIndex: 'logoImage', render: (text, record) => <ImagePreview imageLocation={record.logoImage} /> },
  { title: 'Weixin Openid', debugtype: 'string', dataIndex: 'weixinOpenid', width: '29', render: (text, record) => renderStringEdit('weixinOpenid',text, record)  },
  { title: 'Weixin Appid', debugtype: 'string', dataIndex: 'weixinAppid', width: '23', render: (text, record) => renderStringEdit('weixinAppid',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newCustomerToAppend);
...
export default Form.create()(CustomerEditTable)
