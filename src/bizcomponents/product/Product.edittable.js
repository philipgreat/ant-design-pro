...
import styles from './Product.edittable.less'
...
class ProductEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const productIds = [record.id];
      const parameters = { productIds }
      dispatch({
        type: `${owner.type}/removeProductList`,
        payload: { id: owner.id, type: 'product', parameters },
      })
    }
    ...
    const updateRecord = (e,record) =>{
      ...
      const parameters = { ...record, productId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newProductToAppend);
...
export default Form.create()(ProductEditTable)
