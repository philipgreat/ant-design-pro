...
import styles from './InspectionStation.search.less'
...
class InspectionStationSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const inspectionStationIds = selectedRows.map((item) => { return item.id })
    console.log('inspectionStationIds', inspectionStationIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'inspectionStation', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'检测站列表' } })
  ...
  render() {
    ...
    const {InspectionStationTable} = GlobalComponents;
    const {InspectionStationConfirmationTable} = GlobalComponents;
    const {InspectionStationSearchForm} = GlobalComponents;
...
<Menu.Item key="addressCity">关联所在城市</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <InspectionStationConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 检测站列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <InspectionStationSearchForm {...this.props} />
            </div>
...
export default Form.create()(InspectionStationSearch)
