...
const displayColumns = [
  {
    ...
    width: '20',
    render: (text, record) => (
      <Link to={`/inspectionStation/${text}/dashboard`}>{text}</Link>
    ),
  },
  ...
  },
  {
    ...
    render: (text, record) => (
      <ImagePreview
        imageTitle="计量资格认证"
        imageLocation={record.metrologyAccreditationImage}
      />
    ),
  },
]
...
const InspectionStationBase = { menuData, displayColumns, fieldLabels }
