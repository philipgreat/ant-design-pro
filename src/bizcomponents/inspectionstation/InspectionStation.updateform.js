...
import styles from './InspectionStation.updateform.less'
...
class InspectionStationUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const inspectionStationId = values.id
        const imagesValues = this.mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, inspectionStationId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextInspectionStationUpdateRow`,
        payload: {
          id: owner.id,
          type: 'inspectionStation',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新检测站"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新检测站"
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label={fieldLabels.contactMobile}>
                  {getFieldDecorator('contactMobile', {
                    ...
                    <Input placeholder="请输入请输入联系手机string_china_mobile_phone" />
                  )}
                </Form.Item>
              </Col>
...
}))(Form.create()(InspectionStationUpdateForm))
