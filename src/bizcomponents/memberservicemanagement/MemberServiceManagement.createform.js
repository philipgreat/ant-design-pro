...
import styles from './MemberServiceManagement.createform.less'
...
class MemberServiceManagementCreateForm extends Component {
  state = {
  ...
  executeCandidateBookSharingPlatformSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = MemberServiceManagementService.requestCandidateBookSharingPlatform("bookSharingPlatform", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addMemberServiceManagement`,
          payload: { id: owner.id, type: 'memberServiceManagement', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'memberServiceManagement',listName:'会员服务管理列表' },
      })
    }
...
}))(Form.create()(MemberServiceManagementCreateForm))
