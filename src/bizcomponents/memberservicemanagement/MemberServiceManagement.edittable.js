...
import styles from './MemberServiceManagement.edittable.less'
...
class MemberServiceManagementEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const memberServiceManagementIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addMemberServiceManagement`,
        payload: {
          id: owner.id,
          type: 'memberServiceManagement',
          parameters,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {memberServiceManagementId} = record.id
      const parameters = { ...record, memberServiceManagementId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateMemberServiceManagement`,
        payload: {
          id: owner.id,
          type: 'memberServiceManagement',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newMemberServiceManagementToAppend  = {
      	'id':`+1`, 
				'name':'',
				'bookSharingPlatform':'',
      ...
      newData.push(newMemberServiceManagementToAppend);
...
export default Form.create()(MemberServiceManagementEditTable)
