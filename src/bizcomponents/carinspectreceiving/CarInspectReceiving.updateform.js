...
import styles from './CarInspectReceiving.updateform.less';
...
class CarInspectReceivingUpdateForm extends Component {
 ...
 mapBackToImageValues(convertedImagesValues) {
  ...
  getSelectedRow(){
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
			receiveTime: moment(item.receiveTime).format('YYYY-MM-DD'),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
     validateFieldsAndScroll((error, values) => {
        ...
        const carInspectReceivingId = values.id;
        const imagesValues = this.mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values,carInspectReceivingId, ...imagesValues };

	    const newIndex= currentUpdateIndex+1;
        dispatch({
          type: owner.type+'/updateCarInspectReceiving',
    ...
    const skipToNext = () => {
      ...
      const newIndex= currentUpdateIndex+1;
      dispatch({
          type: owner.type+'/gotoNextCarInspectReceivingUpdateRow',
            payload: {
              id:owner.id,type:'carInspectReceiving', 
    ...
    return (
      <PageHeaderLayout
        ...
        title={"更新还车接收"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新还车接收"
...
}))(Form.create()(CarInspectReceivingUpdateForm));
