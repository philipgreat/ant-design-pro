...
const imageListOf = (fileInspectionPlateNumberPattern) =>{
...
const settingListOf = (fileInspectionPlateNumberPattern) =>{
...
const largeTextOf = (fileInspectionPlateNumberPattern) =>{
...
class FileInspectionPlateNumberPatternDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, fileInspectionPlateNumberPattern } = this.props;
    
    if(!fileInspectionPlateNumberPattern){
    	return;
    }
    const {displayName} = fileInspectionPlateNumberPattern;
    if(!displayName){
    	return;
    }
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.fileInspectionPlateNumberPattern
    const cardsData = {cardsName:"6年免检检测支持的车牌号码类别",cardsFor: "fileInspectionPlateNumberPattern",cardsSource: this.props.fileInspectionPlateNumberPattern,
  		subItems: [
