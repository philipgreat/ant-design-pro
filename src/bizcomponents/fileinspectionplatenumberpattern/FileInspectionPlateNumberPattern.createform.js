...
import styles from './FileInspectionPlateNumberPattern.createform.less'
...
class FileInspectionPlateNumberPatternCreateForm extends Component {
  state = {
  ...
  executeCandidateCompanySearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = FileInspectionPlateNumberPatternService.requestCandidateCompany("vehicleServiceCompany", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addFileInspectionPlateNumberPattern`,
          payload: { id: owner.id, type: 'fileInspectionPlateNumberPattern', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'fileInspectionPlateNumberPattern',listName:'6年免检检测支持的车牌号码类别列表' },
      })
    }
    ...
    return (
      <PageHeaderLayout
        title="新建一个6年免检检测支持的车牌号码类别"
        content="新建一个6年免检检测支持的车牌号码类别"
...
}))(Form.create()(FileInspectionPlateNumberPatternCreateForm))
