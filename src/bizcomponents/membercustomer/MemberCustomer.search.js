...
import styles from './MemberCustomer.search.less'
...
class MemberCustomerSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const memberCustomerIds = selectedRows.map((item) => { return item.id })
    console.log('memberCustomerIds', memberCustomerIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberCustomer' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'memberCustomer', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'会员的客户列表' } })
  ...
  render() {
    ...
    const {MemberCustomerTable} = GlobalComponents;
    const {MemberCustomerConfirmationTable} = GlobalComponents;
    const {MemberCustomerSearchForm} = GlobalComponents;
...
<Menu.Item key="memberServiceSku">关联会员服务Sku</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <MemberCustomerConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 会员的客户列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <MemberCustomerSearchForm {...this.props} />
            </div>
...
export default Form.create()(MemberCustomerSearch)
