...
import styles from './VehicleInspectionInsuranceOrder.search.less'
...
class VehicleInspectionInsuranceOrderSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const vehicleInspectionInsuranceOrderIds = selectedRows.map((item) => { return item.id })
    console.log('vehicleInspectionInsuranceOrderIds', vehicleInspectionInsuranceOrderIds)
    const parameters = { vehicleInspectionInsuranceOrderIds }
    dispatch({
      type: `${owner.type}/removeVehicleInspectionInsuranceOrderList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'vehicleInspectionInsuranceOrder' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'vehicleInspectionInsuranceOrder', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'车辆上线检测保险订单列表' } })
  ...
  render() {
    ...
    const {VehicleInspectionInsuranceOrderTable} = GlobalComponents;
    const {VehicleInspectionInsuranceOrderConfirmationTable} = GlobalComponents;
    const {VehicleInspectionInsuranceOrderSearchForm} = GlobalComponents;
...
<Menu.Item key="insurance">关联保险产品</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <VehicleInspectionInsuranceOrderConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 车辆上线检测保险订单列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <VehicleInspectionInsuranceOrderSearchForm {...this.props} />
            </div>
...
export default Form.create()(VehicleInspectionInsuranceOrderSearch)
