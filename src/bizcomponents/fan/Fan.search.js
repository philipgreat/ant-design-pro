...
import FanTable from './Fan.table';
import FanConfirmationTable from './Fan.confirmmationtable';

import FanSearchForm from './Fan.searchform';
...
@Form.create()
export default class FanSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    const params = {
      fanList: 1,
      fanListCurrentPage: pagination.current,
      fanListRowsPerPage: pagination.pageSize,
      ...formValues,
      ...filters,
    };
    if (sorter.field) {
  ...
  handleDelete = () => {
    ...
    const fanIds = selectedRows.map((item) => { return item.id; });
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'fan', selectedRows, currentUpdateIndex },
    });
  }
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <FanConfirmationTable data={selectedRows} owner={owner} />
        </Modal>);
    };
