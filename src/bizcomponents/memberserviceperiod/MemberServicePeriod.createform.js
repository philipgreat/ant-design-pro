...
import styles from './MemberServicePeriod.createform.less'
...
class MemberServicePeriodCreateForm extends Component {
  state = {
  ...
  executeCandidateMemberServiceManagementSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = MemberServicePeriodService.requestCandidateMemberServiceManagement("memberServiceManagement", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addMemberServicePeriod`,
          payload: { id: owner.id, type: 'memberServicePeriod', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'memberServicePeriod',listName:'会员服务时间列表' },
      })
    }
...
}))(Form.create()(MemberServicePeriodCreateForm))
