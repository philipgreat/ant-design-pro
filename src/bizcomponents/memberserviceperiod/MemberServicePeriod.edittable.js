...
import styles from './MemberServicePeriod.edittable.less'
...
class MemberServicePeriodEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
			const memberServiceManagementId = record.memberServiceManagement.id

      //const communityId = record.community.id;
      return {memberServiceManagementId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const memberServicePeriodIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addMemberServicePeriod`,
        payload: {
          id: owner.id,
          type: 'memberServicePeriod',
          parameters,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {memberServicePeriodId} = record.id
      const parameters = { ...record, memberServicePeriodId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateMemberServicePeriod`,
        payload: {
          id: owner.id,
          type: 'memberServicePeriod',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newMemberServicePeriodToAppend  = {
      	'id':`+1`, 
				'servicePeriodName':'',
				'servicePeriodDays':'',
      ...
      newData.push(newMemberServicePeriodToAppend);
...
export default Form.create()(MemberServicePeriodEditTable)
