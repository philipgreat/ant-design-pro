...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        const dashboardmatch = pathToRegexp('/role/:id/dashboard').exec(pathname)
        if (dashboardmatch) {
          const id = dashboardmatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const editDetailMatch = pathToRegexp('/role/:id/editDetail').exec(pathname)
        if (editDetailMatch) {
          const id = editDetailMatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const viewDetailMatch = pathToRegexp('/role/:id/viewDetail').exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp('/role/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    *view({ payload }, { call, put }) { 
      ...
      const data = yield call(RoleService.view, payload.id)
    ...
    *load({ payload }, { call, put }) { 
      const {RoleService} = GlobalComponents;
      yield put({ type: 'showLoading', payload })
      const data = yield call(RoleService.load, payload.id, payload.parameters)
    ...
    *gotoCreateForm({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/role/${id}/list/${type}CreateForm`))
    },
    *gotoUpdateForm({ payload }, { put }) {
      const { id, type, selectedRows, currentUpdateIndex } = payload
      const state = { id, type, selectedRows, currentUpdateIndex }
      const location = { pathname: `/role/${id}/list/${type}UpdateForm`, state }
    ...
    *removeEmployeeList({ payload }, { call, put }) {
      ...
      // const location = { pathname: `role/${id}/list/${type}List`, state: data}
      // yield put(routerRedux.push(location))
    },
