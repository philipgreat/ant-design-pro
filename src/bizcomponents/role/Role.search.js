...
import styles from './Role.search.less'
...
class RoleSearch extends PureComponent {
  ...
  handleDelete = () => {
    ...
    const roleIds = selectedRows.map((item) => { return item.id })
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'role', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'角色列表' } })
  ...
  render() {
    ...
    const {RoleTable} = GlobalComponents;
    const {RoleConfirmationTable} = GlobalComponents;
    const {RoleSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <RoleConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 角色列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <RoleSearchForm {...this.props} />
            </div>
