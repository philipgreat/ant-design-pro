...
import styles from './Role.edittable.less'
...
class RoleEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const parameters = { roleIds }
      dispatch({
        type: `${owner.type}/removeRoleList`,
    ...
    const updateRecord = (e,record) =>{
      ...
      const parameters = { ...record, roleId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '角色名', debugtype: 'string', dataIndex: 'roleName', width: '7', render: (text, record) => renderStringEdit('roleName',text, record)  },
  ...
  { title: '角色Property3', debugtype: 'int', dataIndex: 'roleProperty3', width: '7', render: (text, record) => renderStringEdit('roleProperty3',text, record)  },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newRoleToAppend  = {
      	'id':`+1`, 
				'roleName':'',
				'roleProperty1':'',
				'roleProperty2':'',
...
export default Form.create()(RoleEditTable)
