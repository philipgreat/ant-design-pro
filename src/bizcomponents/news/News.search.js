...
import styles from './News.search.less'
...
class NewsSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const newsIds = selectedRows.map((item) => { return item.id })
    console.log('newsIds', newsIds)
    const parameters = { newsIds }
    dispatch({
      type: `${owner.type}/removeNewsList`,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'news', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'新闻列表' } })
  ...
  render() {
    ...
    const {NewsTable} = GlobalComponents;
    const {NewsConfirmationTable} = GlobalComponents;
    const {NewsSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <NewsConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 新闻列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <NewsSearchForm {...this.props} />
            </div>
...
export default Form.create()(NewsSearch)
