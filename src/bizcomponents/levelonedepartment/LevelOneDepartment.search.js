...
@Form.create()
export default class LevelOneDepartmentSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const levelOneDepartmentIds = selectedRows.map((item) => { return item.id })
    console.log('levelOneDepartmentIds', levelOneDepartmentIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'levelOneDepartment', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {LevelOneDepartmentConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <LevelOneDepartmentConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 一级部门列表`}>
        <Card bordered={false}>
