...
import styles from './PlatformAccountDetails.updateform.less'
...
class PlatformAccountDetailsUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const platformAccountDetailsId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, platformAccountDetailsId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextPlatformAccountDetailsUpdateRow`,
        payload: {
          id: owner.id,
          type: 'platformAccountDetails',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新平台账户信息"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新平台账户信息"
...
}))(Form.create()(PlatformAccountDetailsUpdateForm))
