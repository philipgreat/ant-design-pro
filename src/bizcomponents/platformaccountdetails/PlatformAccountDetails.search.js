...
import styles from './PlatformAccountDetails.search.less'
...
class PlatformAccountDetailsSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const platformAccountDetailsIds = selectedRows.map((item) => { return item.id })
    console.log('platformAccountDetailsIds', platformAccountDetailsIds)
    const parameters = { platformAccountDetailsIds }
    dispatch({
      type: `${owner.type}/removePlatformAccountDetailsList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'platformAccountDetails' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'platformAccountDetails', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'平台账户信息列表' } })
  ...
  render() {
    ...
    const {PlatformAccountDetailsTable} = GlobalComponents;
    const {PlatformAccountDetailsConfirmationTable} = GlobalComponents;
    const {PlatformAccountDetailsSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <PlatformAccountDetailsConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 平台账户信息列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <PlatformAccountDetailsSearchForm {...this.props} />
            </div>
...
export default Form.create()(PlatformAccountDetailsSearch)
