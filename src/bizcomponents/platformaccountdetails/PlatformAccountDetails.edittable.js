...
import styles from './PlatformAccountDetails.edittable.less'
...
class PlatformAccountDetailsEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
			const platformAccountId = record.platformAccount.id
			const relatedMainOrderId = record.relatedMainOrder.id

      //const communityId = record.community.id;
      return {platformAccountId,relatedMainOrderId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const platformAccountDetailsIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {platformAccountDetailsId} = record.id
      const parameters = { ...record, platformAccountDetailsId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updatePlatformAccountDetails`,
        payload: {
          id: owner.id,
          type: 'platformAccountDetails',
          parameters,
  ...
  { title: '总结', debugtype: 'string', dataIndex: 'summary', width: '18', render: (text, record) => renderStringEdit('summary',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newPlatformAccountDetailsToAppend  = {
      	'id':`+1`, 
				'summary':'',
				'changeAmount':'',
				'changeType':'',
      ...
      newData.push(newPlatformAccountDetailsToAppend);
...
export default Form.create()(PlatformAccountDetailsEditTable)
