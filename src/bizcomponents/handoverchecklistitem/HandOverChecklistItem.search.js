...
import styles from './HandOverChecklistItem.search.less'
...
class HandOverChecklistItemSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const handOverChecklistItemIds = selectedRows.map((item) => { return item.id })
    console.log('handOverChecklistItemIds', handOverChecklistItemIds)
    const parameters = { handOverChecklistItemIds }
    dispatch({
      type: `${owner.type}/removeHandOverChecklistItemList`,
      payload: { id: owner.id, type: 'handOverChecklistItem', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'handOverChecklistItem' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'handOverChecklistItem', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'交接检查项列表' } })
  ...
  render() {
    ...
    const {HandOverChecklistItemTable} = GlobalComponents;
    const {HandOverChecklistItemConfirmationTable} = GlobalComponents;
    const {HandOverChecklistItemSearchForm} = GlobalComponents;
...
<Menu.Item key="question">关联检查问题</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <HandOverChecklistItemConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 交接检查项列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <HandOverChecklistItemSearchForm {...this.props} />
            </div>
...
export default Form.create()(HandOverChecklistItemSearch)
