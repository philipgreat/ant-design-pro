...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        const match = pathToRegexp('/serviceFileMovementC2m/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *goback({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/serviceFileMovementC2m/${id}/list/${type}List/${listName}`))
    },
    ...
    *addHandOverChecklistResult({ payload }, { call, put }) {
      ...
      const location = { pathname: `/serviceFileMovementC2m/${id}/list/${type}List/交接检查结果列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateHandOverChecklistResult({ payload }, { call, put }) {
      ...
      const location = { pathname: `/serviceFileMovementC2m/${id}/list/${type}List/交接检查结果列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
