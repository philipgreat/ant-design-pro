...
class ServiceFileMovementC2mSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'收件服务列表' } })
...
<Menu.Item key="merchant">关联商户</Menu.Item>
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 收件服务列表`}>
        <Card bordered={false}>
...
export default Form.create()(ServiceFileMovementC2mSearch)
