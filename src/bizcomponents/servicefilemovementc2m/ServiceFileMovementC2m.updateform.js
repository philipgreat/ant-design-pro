...
import styles from './ServiceFileMovementC2m.updateform.less'
...
class ServiceFileMovementC2mUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const serviceFileMovementC2mId = values.id
        const imagesValues = this.mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, serviceFileMovementC2mId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextServiceFileMovementC2mUpdateRow`,
        payload: {
          id: owner.id,
          type: 'serviceFileMovementC2m',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新收件服务"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新收件服务"
...
}))(Form.create()(ServiceFileMovementC2mUpdateForm))
