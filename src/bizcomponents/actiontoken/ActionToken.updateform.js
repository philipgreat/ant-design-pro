...
import styles from './ActionToken.updateform.less'
...
class ActionTokenUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const actionTokenId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, actionTokenId, ...imagesValues }

        // const newIndex= currentUpdateIndex + 1
        dispatch({
          type: `${owner.type}/updateActionToken`,
          payload: {
            id: owner.id,
            type: 'actionToken',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextActionTokenUpdateRow`,
        payload: {
          id: owner.id,
          type: 'actionToken',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新行动令牌"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新行动令牌"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.holderType} {...formItemLayout}>
                  {getFieldDecorator('holderType', {
                    initialValue: selectedRow.holderType,
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.holderId} {...formItemLayout}>
                  {getFieldDecorator('holderId', {
                    initialValue: selectedRow.holderId,
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.tokenCode} {...formItemLayout}>
                  {getFieldDecorator('tokenCode', {
                    initialValue: selectedRow.tokenCode,
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.tokenQuantity} {...formItemLayout}>
                  {getFieldDecorator('tokenQuantity', {
                    initialValue: selectedRow.tokenQuantity,
...
}))(Form.create()(ActionTokenUpdateForm))
