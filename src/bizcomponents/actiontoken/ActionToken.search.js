...
import styles from './ActionToken.search.less'
...
class ActionTokenSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const actionTokenIds = selectedRows.map((item) => { return item.id })
    console.log('actionTokenIds', actionTokenIds)
    const parameters = { actionTokenIds }
    dispatch({
      type: `${owner.type}/removeActionTokenList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'actionToken' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'actionToken', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'行动令牌列表' } })
  ...
  render() {
    ...
    const {ActionTokenTable} = GlobalComponents;
    const {ActionTokenConfirmationTable} = GlobalComponents;
    const {ActionTokenSearchForm} = GlobalComponents;
...
<Menu.Item key="userDomain">关联用户域</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ActionTokenConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 行动令牌列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ActionTokenSearchForm {...this.props} />
            </div>
...
export default Form.create()(ActionTokenSearch)
