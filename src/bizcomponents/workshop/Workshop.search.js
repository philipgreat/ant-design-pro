...
import styles from './Workshop.search.less'
...
class WorkshopSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const workshopIds = selectedRows.map((item) => { return item.id })
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'workshop' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'workshop', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'车间列表' } })
  ...
  render() {
    ...
    const {WorkshopTable} = GlobalComponents;
    const {WorkshopConfirmationTable} = GlobalComponents;
    const {WorkshopSearchForm} = GlobalComponents;
...
<Menu.Item key="publishStore">关联发布商店</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <WorkshopConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 车间列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <WorkshopSearchForm {...this.props} />
            </div>
...
export default Form.create()(WorkshopSearch)
