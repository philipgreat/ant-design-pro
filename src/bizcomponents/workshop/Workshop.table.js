...
const columns = [
  { title: 'ID', debugtype: 'string', dataIndex: 'id',  render: (text, record)=>(<Link to={`/workshop/${text}/dashboard`}>{text}</Link>) },
  { title: '工厂的名字', debugtype: 'string', dataIndex: 'workshopName',},
  { title: '研讨会内容', debugtype: 'string', dataIndex: 'workshopContent',},
  { title: '车间图片', dataIndex: 'workshopImage', render: (text, record) => <ImagePreview imageTitle="车间图片" imageLocation={record.workshopImage} /> },
  { title: '车间的地位', debugtype: 'string', dataIndex: 'workshopStatus',},
  ...
  { title: '可用的寄存器数量', debugtype: 'int', dataIndex: 'availableRegisterQuantity',},
