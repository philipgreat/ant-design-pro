...
const columns = [
  ...
  { title: '车间活动日期时间', dataIndex: 'workshopEventDatetime', render: (text, record) => moment(record.workshopEventDatetime).format('YYYY-MM-DD') },
  { title: '可用的寄存器Datetime', dataIndex: 'availableRegisterDatetime', render: (text, record) => moment(record.availableRegisterDatetime).format('YYYY-MM-DD') },
  { title: '可用的寄存器数量', debugtype: 'int', dataIndex: 'availableRegisterQuantity', width: '6' },
  { title: '发布日期时间', dataIndex: 'publishDatetime', render: (text, record) => moment(record.publishDatetime).format('YYYY-MM-DD') },
...
class WorkshopConfirmationTable extends PureComponent {
  render() {
...
export default WorkshopConfirmationTable
