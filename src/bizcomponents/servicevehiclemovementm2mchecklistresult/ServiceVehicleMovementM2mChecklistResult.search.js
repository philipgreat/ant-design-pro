...
import styles from './ServiceVehicleMovementM2mChecklistResult.search.less'
...
@Form.create()
export default class ServiceVehicleMovementM2mChecklistResultSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const serviceVehicleMovementM2mChecklistResultIds = selectedRows.map((item) => { return item.id })
    console.log('serviceVehicleMovementM2mChecklistResultIds', serviceVehicleMovementM2mChecklistResultIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'serviceVehicleMovementM2mChecklistResult' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'serviceVehicleMovementM2mChecklistResult', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {ServiceVehicleMovementM2mChecklistResultTable} = GlobalComponents;
    const {ServiceVehicleMovementM2mChecklistResultConfirmationTable} = GlobalComponents;
    const {ServiceVehicleMovementM2mChecklistResultSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ServiceVehicleMovementM2mChecklistResultConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="送审服务检查结果列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ServiceVehicleMovementM2mChecklistResultSearchForm {...this.props} />
            </div>
