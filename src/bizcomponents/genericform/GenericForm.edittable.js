...
import styles from './GenericForm.edittable.less'
...
class GenericFormEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const genericFormIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {genericFormId} = record.id
      const parameters = { ...record, genericFormId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '头衔', debugtype: 'string', dataIndex: 'title', width: '9', render: (text, record) => renderStringEdit('title',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newGenericFormToAppend);
...
export default Form.create()(GenericFormEditTable)
