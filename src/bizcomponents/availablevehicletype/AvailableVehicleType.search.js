...
import styles from './AvailableVehicleType.search.less'
...
class AvailableVehicleTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const availableVehicleTypeIds = selectedRows.map((item) => { return item.id })
    console.log('availableVehicleTypeIds', availableVehicleTypeIds)
    const parameters = { availableVehicleTypeIds }
    dispatch({
      type: `${owner.type}/removeAvailableVehicleTypeList`,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'availableVehicleType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'车辆类型列表' } })
  ...
  render() {
    ...
    const {AvailableVehicleTypeTable} = GlobalComponents;
    const {AvailableVehicleTypeConfirmationTable} = GlobalComponents;
    const {AvailableVehicleTypeSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <AvailableVehicleTypeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 车辆类型列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <AvailableVehicleTypeSearchForm {...this.props} />
            </div>
...
export default Form.create()(AvailableVehicleTypeSearch)
