...
const imageListOf = (availableVehicleType) =>{
...
const settingListOf = (availableVehicleType) =>{

	const optionList = [ 
	  {"title":'可下单',"value":availableVehicleType.canPlaceOrder},
  {"title":'可6年免检',"value":availableVehicleType.canDoExempt},
]
...
const largeTextOf = (availableVehicleType) =>{
...
class AvailableVehicleTypeDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, availableVehicleType } = this.props;
    
    if(!availableVehicleType){
    	return;
    }
    const {displayName} = availableVehicleType;
    if(!displayName){
    	return;
    }
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.availableVehicleType
    const cardsData = {cardsName:"车辆类型",cardsFor: "availableVehicleType",cardsSource: this.props.availableVehicleType,
  		subItems: [
