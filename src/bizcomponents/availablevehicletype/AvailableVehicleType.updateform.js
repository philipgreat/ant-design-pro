...
import styles from './AvailableVehicleType.updateform.less'
...
class AvailableVehicleTypeUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const availableVehicleTypeId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, availableVehicleTypeId, ...imagesValues }

        // const newIndex= currentUpdateIndex + 1
        dispatch({
          type: `${owner.type}/updateAvailableVehicleType`,
          payload: {
            id: owner.id,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextAvailableVehicleTypeUpdateRow`,
        payload: {
          id: owner.id,
          type: 'availableVehicleType',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新车辆类型"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新车辆类型"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.vehicleTypeAlias} {...formItemLayout}>
                  {getFieldDecorator('vehicleTypeAlias', {
                    initialValue: selectedRow.vehicleTypeAlias,
                    rules: [{ required: false, message: '请输入车辆类型别名' }],
                  })(
              ...
              <Col lg={8} md={12} sm={24}>
                <Form.Item label={fieldLabels.canPlaceOrder} {...switchFormItemLayout}>
                  {getFieldDecorator('canPlaceOrder', {
                    initialValue: selectedRow.canPlaceOrder,
                    rules: [{ required: true, message: '请输入可下单' }],
                    valuePropName: 'checked'
                  })(
                    <Switch checkedChildren="是" unCheckedChildren="否"  placeholder="请输入可下单bool" />
                  )}
                </Form.Item>
              </Col>
...
}))(Form.create()(AvailableVehicleTypeUpdateForm))
