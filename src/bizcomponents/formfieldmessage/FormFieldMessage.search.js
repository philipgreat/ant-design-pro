...
@Form.create()
export default class FormFieldMessageSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const formFieldMessageIds = selectedRows.map((item) => { return item.id })
    console.log('formFieldMessageIds', formFieldMessageIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'formFieldMessage' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'formFieldMessage', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'表单字段的信息列表' } })
...
<Menu.Item key="form">关联形式</Menu.Item>
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 表单字段的信息列表`}>
        <Card bordered={false}>
