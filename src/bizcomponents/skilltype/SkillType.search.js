...
@Form.create()
export default class SkillTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const skillTypeIds = selectedRows.map((item) => { return item.id })
    console.log('skillTypeIds', skillTypeIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'skillType' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'skillType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {SkillTypeConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <SkillTypeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 技能类型列表`}>
        <Card bordered={false}>
