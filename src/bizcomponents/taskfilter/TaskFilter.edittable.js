...
import styles from './TaskFilter.edittable.less'
...
class TaskFilterEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {taskFilterId} = record.id
      const parameters = { ...record, taskFilterId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '过滤器健值', debugtype: 'string', dataIndex: 'filterKey', width: '25', render: (text, record) => renderStringEdit('filterKey',text, record)  },
  { title: '链接网址', debugtype: 'string', dataIndex: 'linkUrl', width: '40', render: (text, record) => renderStringEdit('linkUrl',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newTaskFilterToAppend);
...
export default Form.create()(TaskFilterEditTable)
