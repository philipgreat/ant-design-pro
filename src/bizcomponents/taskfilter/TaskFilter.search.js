...
class TaskFilterSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const taskFilterIds = selectedRows.map((item) => { return item.id })
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'taskFilter', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'任务过滤器列表' } })
  ...
  render() {
    ...
    const {TaskFilterTable} = GlobalComponents;
    const {TaskFilterConfirmationTable} = GlobalComponents;
    const {TaskFilterSearchForm} = GlobalComponents;
...
<Menu.Item key="taskPage">关联任务页面</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TaskFilterConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 任务过滤器列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TaskFilterSearchForm {...this.props} />
            </div>
...
export default Form.create()(TaskFilterSearch)
