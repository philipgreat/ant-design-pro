...
@Form.create()
export default class InterviewTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const interviewTypeIds = selectedRows.map((item) => { return item.id })
    console.log('interviewTypeIds', interviewTypeIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'interviewType' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'interviewType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {InterviewTypeConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <InterviewTypeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 面试类型列表`}>
        <Card bordered={false}>
