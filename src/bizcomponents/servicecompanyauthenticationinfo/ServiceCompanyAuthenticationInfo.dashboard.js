...
const imageListOf = (serviceCompanyAuthenticationInfo) =>{

  const imageList = [
	   {"title":'营业执照照片',"imageLocation":serviceCompanyAuthenticationInfo.businessLicenseImg},
]
...
const settingListOf = (serviceCompanyAuthenticationInfo) =>{
...
const largeTextOf = (serviceCompanyAuthenticationInfo) =>{
...
class ServiceCompanyAuthenticationInfoDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, serviceCompanyAuthenticationInfo } = this.props;
    
    if(!serviceCompanyAuthenticationInfo){
    	return;
    }
    const {displayName} = serviceCompanyAuthenticationInfo;
    if(!displayName){
    	return;
    }
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.serviceCompanyAuthenticationInfo
    const cardsData = {cardsName:"商户认证信息",cardsFor: "serviceCompanyAuthenticationInfo",cardsSource: this.props.serviceCompanyAuthenticationInfo,
  		subItems: [
