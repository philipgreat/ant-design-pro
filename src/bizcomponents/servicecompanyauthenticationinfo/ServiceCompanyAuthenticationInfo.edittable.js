...
import styles from './ServiceCompanyAuthenticationInfo.edittable.less'
...
class ServiceCompanyAuthenticationInfoEditTable extends PureComponent {
  constructor(props) {
    super(props)
    console.log(props)
    this.state = {
  ...
  render() {
    ...
    const deleteRecord = (e, record) => {
      ...
      const { data } = this.state
      const serviceCompanyAuthenticationInfoIds = [record.id]
    ...
    const updateRecord = (e, record) => {
      const { dispatch, owner } = this.props
      const { data } = this.state
      const { serviceCompanyAuthenticationInfoId } = record.id
      const parameters = { ...record, serviceCompanyAuthenticationInfoId }
      const newData = [...data]
      const row = newData.filter(item => item.id === record.id)[0]
      row.editable = !row.editable
    ...
    const columns = [
      ...
      },
      {
        ...
        width: '20',
        render: (text, record) =>
          renderStringEdit('businessLicenseCode', text, record),
      },
      {
        title: '商户',
    ...
    const newRecord = () => {
      const newServiceCompanyAuthenticationInfoToAppend = {
        id: `+1`,
        businessLicenseImg: '',
        businessLicenseCode: '',
        serviceCompany: '',
      ...
      newData.push(newServiceCompanyAuthenticationInfoToAppend)
...
export default Form.create()(ServiceCompanyAuthenticationInfoEditTable)
