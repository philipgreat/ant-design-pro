...
import styles from './VehicleInspectionPlateNumberPattern.createform.less'
...
class VehicleInspectionPlateNumberPatternCreateForm extends Component {
  state = {
  ...
  executeCandidateCompanySearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = VehicleInspectionPlateNumberPatternService.requestCandidateCompany("vehicleServiceCompany", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addVehicleInspectionPlateNumberPattern`,
          payload: { id: owner.id, type: 'vehicleInspectionPlateNumberPattern', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'vehicleInspectionPlateNumberPattern',listName:'上线检测支持的车牌号码类别列表' },
      })
    }
...
}))(Form.create()(VehicleInspectionPlateNumberPatternCreateForm))
