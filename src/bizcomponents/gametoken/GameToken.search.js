...
import styles from './GameToken.search.less'
...
class GameTokenSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const gameTokenIds = selectedRows.map((item) => { return item.id })
    console.log('gameTokenIds', gameTokenIds)
    const parameters = { gameTokenIds }
    dispatch({
      type: `${owner.type}/removeGameTokenList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'gameToken' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'gameToken', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'游戏Token列表' } })
  ...
  render() {
    ...
    const {GameTokenTable} = GlobalComponents;
    const {GameTokenConfirmationTable} = GlobalComponents;
    const {GameTokenSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <GameTokenConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 游戏Token列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <GameTokenSearchForm {...this.props} />
            </div>
...
export default Form.create()(GameTokenSearch)
