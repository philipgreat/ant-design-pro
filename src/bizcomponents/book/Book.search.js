...
import styles from './Book.search.less'
...
class BookSearch extends PureComponent {
  ...
  handleDelete = () => {
    ...
    const bookIds = selectedRows.map((item) => { return item.id })
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'book', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'书列表' } })
  ...
  render() {
    ...
    const {BookTable} = GlobalComponents;
    const {BookConfirmationTable} = GlobalComponents;
    const {BookSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 书列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookSearch)
