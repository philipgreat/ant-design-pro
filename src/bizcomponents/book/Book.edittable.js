...
import styles from './Book.edittable.less'
...
class BookEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
      ...
      return {bookManagementId,platformId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const bookIds = [record.id];
      const parameters = { bookIds }
      dispatch({
        type: `${owner.type}/removeBookList`,
    ...
    const updateRecord = (e,record) =>{
      ...
      const parameters = { ...record, bookId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '书的名字', debugtype: 'string', dataIndex: 'bookName', width: '62', render: (text, record) => renderStringEdit('bookName',text, record)  },
  { title: '书的封面', dataIndex: 'bookCover', render: (text, record) => <ImagePreview imageLocation={record.bookCover} /> },
  { title: '书的作者', debugtype: 'string', dataIndex: 'bookAuthor', width: '67', render: (text, record) => renderStringEdit('bookAuthor',text, record)  },
  ...
  { title: '书Isbn13', debugtype: 'string', dataIndex: 'bookIsbn13', width: '21', render: (text, record) => renderStringEdit('bookIsbn13',text, record)  },
  { title: '书Isbn10', debugtype: 'string', dataIndex: 'bookIsbn10', width: '16', render: (text, record) => renderStringEdit('bookIsbn10',text, record)  },
...
export default Form.create()(BookEditTable)
