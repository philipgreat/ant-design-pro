...
@Form.create()
export default class PublicHolidaySearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const publicHolidayIds = selectedRows.map((item) => { return item.id })
    console.log('publicHolidayIds', publicHolidayIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'publicHoliday' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'publicHoliday', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {PublicHolidayConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <PublicHolidayConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 公共假日列表`}>
        <Card bordered={false}>
