...
import styles from './UserDomain.edittable.less'
...
class UserDomainEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {userDomainId} = record.id
      const parameters = { ...record, userDomainId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
...
export default Form.create()(UserDomainEditTable)
