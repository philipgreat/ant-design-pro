...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        const editDetailMatch = pathToRegexp('/userDomain/:id/editDetail').exec(pathname)
        if (editDetailMatch) {
          const id = editDetailMatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const viewDetailMatch = pathToRegexp('/userDomain/:id/viewDetail').exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp('/userDomain/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *gotoUpdateForm({ payload }, { put }) {
      ...
      const location = { pathname: `/userDomain/${id}/list/${type}UpdateForm`, state }
    ...
    *addSecUser({ payload }, { call, put }) {
      ...
      const partialList = true
      const newState = {...data, partialList}
      const location = { pathname: `/userDomain/${id}/list/${type}List/SEC的用户列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateSecUser({ payload }, { call, put }) {
      ...
      const location = { pathname: `/userDomain/${id}/list/${type}List/SEC的用户列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
