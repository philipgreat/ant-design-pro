...
class InvitationCodeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'invitationCode', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'邀请码列表' } })
  ...
  render() {
    ...
    const {InvitationCodeTable} = GlobalComponents;
    const {InvitationCodeConfirmationTable} = GlobalComponents;
    const {InvitationCodeSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <InvitationCodeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 邀请码列表`}>
        <Card bordered={false}>
...
export default Form.create()(InvitationCodeSearch)
