...
@Form.create()
export default class LevelTwoDepartmentSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const levelTwoDepartmentIds = selectedRows.map((item) => { return item.id })
    console.log('levelTwoDepartmentIds', levelTwoDepartmentIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'levelTwoDepartment' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'levelTwoDepartment', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {LevelTwoDepartmentConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <LevelTwoDepartmentConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 二级部门列表`}>
        <Card bordered={false}>
