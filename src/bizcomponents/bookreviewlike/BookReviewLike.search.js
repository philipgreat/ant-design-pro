...
import styles from './BookReviewLike.search.less'
...
class BookReviewLikeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookReviewLikeIds = selectedRows.map((item) => { return item.id })
    console.log('bookReviewLikeIds', bookReviewLikeIds)
    const parameters = { bookReviewLikeIds }
    dispatch({
      type: `${owner.type}/removeBookReviewLikeList`,
      payload: { id: owner.id, type: 'bookReviewLike', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookReviewLike' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookReviewLike', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'这样的书评列表' } })
  ...
  render() {
    ...
    const {BookReviewLikeTable} = GlobalComponents;
    const {BookReviewLikeConfirmationTable} = GlobalComponents;
    const {BookReviewLikeSearchForm} = GlobalComponents;
...
<Menu.Item key="replier">关联回答者</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookReviewLikeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 这样的书评列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookReviewLikeSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookReviewLikeSearch)
