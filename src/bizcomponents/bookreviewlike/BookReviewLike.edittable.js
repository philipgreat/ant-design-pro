...
import styles from './BookReviewLike.edittable.less'
...
class BookReviewLikeEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const bookReviewLikeIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addBookReviewLike`,
        payload: {
          id: owner.id,
          type: 'bookReviewLike',
          parameters,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {bookReviewLikeId} = record.id
      const parameters = { ...record, bookReviewLikeId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateBookReviewLike`,
        payload: {
          id: owner.id,
          type: 'bookReviewLike',
          parameters,
  ...
  { title: '喜欢的类型', debugtype: 'string', dataIndex: 'likeType', width: '6', render: (text, record) => renderStringEdit('likeType',text, record)  },
  { title: '当前状态', debugtype: 'string', dataIndex: 'currentStatus', width: '17', render: (text, record) => renderStringEdit('currentStatus',text, record)  },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newBookReviewLikeToAppend  = {
      	'id':`+1`, 
				'bookReview':'',
				'replier':'',
				'likePublishDatetime':'',
      ...
      newData.push(newBookReviewLikeToAppend);
...
export default Form.create()(BookReviewLikeEditTable)
