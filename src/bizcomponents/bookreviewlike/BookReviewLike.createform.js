...
import styles from './BookReviewLike.createform.less'
...
class BookReviewLikeCreateForm extends Component {
  state = {
  ...
  executeCandidateReplierSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = BookReviewLikeService.requestCandidateReplier("customer", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addBookReviewLike`,
          payload: { id: owner.id, type: 'bookReviewLike', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'bookReviewLike',listName:'这样的书评列表' },
      })
    }
...
}))(Form.create()(BookReviewLikeCreateForm))
