...
import styles from './Instructor.edittable.less'
...
class InstructorEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const instructorIds = [record.id];
      const parameters = { instructorIds }
      dispatch({
        type: `${owner.type}/removeInstructorList`,
        payload: { id: owner.id, type: 'instructor', parameters },
      })
    }
    ...
    const updateRecord = (e,record) =>{
      ...
      const parameters = { ...record, instructorId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '电子邮件', debugtype: 'string_email', dataIndex: 'email', width: '24', render: (text, record) => renderStringEdit('email',text, record)  },
  { title: '公司', dataIndex: 'company', render: (text, record) => (record.company ? record.company.displayName : '暂无') },
  { title: '介绍', debugtype: 'string', dataIndex: 'introduction', width: '19', render: (text, record) => renderStringEdit('introduction',text, record)  },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newInstructorToAppend  = {
      	'id':`+1`, 
				'title':'',
				'familyName':'',
				'givenName':'',
      ...
      newData.push(newInstructorToAppend);
...
export default Form.create()(InstructorEditTable)
