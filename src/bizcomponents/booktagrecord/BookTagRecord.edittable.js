...
import styles from './BookTagRecord.edittable.less'
...
class BookTagRecordEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const bookTagRecordIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {bookTagRecordId} = record.id
      const parameters = { ...record, bookTagRecordId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateBookTagRecord`,
        payload: {
          id: owner.id,
          type: 'bookTagRecord',
          parameters,
  ...
  { title: '标签名', debugtype: 'string', dataIndex: 'tagName', width: '7', render: (text, record) => renderStringEdit('tagName',text, record)  },
  ...
  { title: '标签的客户', debugtype: 'string', dataIndex: 'tagCustomer', width: '17', render: (text, record) => renderStringEdit('tagCustomer',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newBookTagRecordToAppend  = {
      	'id':`+1`, 
				'tagName':'',
				'book':'',
				'bookCopy':'',
      ...
      newData.push(newBookTagRecordToAppend);
...
export default Form.create()(BookTagRecordEditTable)
