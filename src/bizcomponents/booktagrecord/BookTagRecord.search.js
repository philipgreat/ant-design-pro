...
import styles from './BookTagRecord.search.less'
...
class BookTagRecordSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookTagRecordIds = selectedRows.map((item) => { return item.id })
    console.log('bookTagRecordIds', bookTagRecordIds)
    const parameters = { bookTagRecordIds }
    dispatch({
      type: `${owner.type}/removeBookTagRecordList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookTagRecord' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookTagRecord', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'书标签记录列表' } })
  ...
  render() {
    ...
    const {BookTagRecordTable} = GlobalComponents;
    const {BookTagRecordConfirmationTable} = GlobalComponents;
    const {BookTagRecordSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookTagRecordConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 书标签记录列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookTagRecordSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookTagRecordSearch)
