...
import styles from './BorrowingExpiredSku.edittable.less'
...
class BorrowingExpiredSkuEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
			...
			const borrowingHistoryId = record.borrowingHistory.id

      //const communityId = record.community.id;
      return {borrowerId,bookCopyId,bookId,lendingStoreId,returnStoreId,borrowingHistoryId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const borrowingExpiredSkuIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {borrowingExpiredSkuId} = record.id
      const parameters = { ...record, borrowingExpiredSkuId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '天到期', debugtype: 'int', dataIndex: 'expiredDays', width: '5', render: (text, record) => renderStringEdit('expiredDays',text, record)  },
  { title: '过期的费用', dataIndex: 'expiredFee', className:'money', render: (text, record) => ('￥'+text.toFixed(2)) },
  { title: '成本付款状态', debugtype: 'string', dataIndex: 'costPaymentStatus', width: '7', render: (text, record) => renderStringEdit('costPaymentStatus',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newBorrowingExpiredSkuToAppend  = {
      	'id':`+1`, 
				'borrower':'',
				'bookCopy':'',
				'book':'',
      ...
      newData.push(newBorrowingExpiredSkuToAppend);
...
export default Form.create()(BorrowingExpiredSkuEditTable)
