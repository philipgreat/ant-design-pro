...
import styles from './Coupon.search.less'
...
class CouponSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const couponIds = selectedRows.map((item) => { return item.id })
    console.log('couponIds', couponIds)
    const parameters = { couponIds }
    dispatch({
      type: `${owner.type}/removeCouponList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'coupon' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'coupon', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {CouponTable} = GlobalComponents;
    const {CouponConfirmationTable} = GlobalComponents;
    const {CouponSearchForm} = GlobalComponents;
...
<Menu.Item key="offlineStoreOrder">关联线下门店订单</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CouponConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
...
export default Form.create()(CouponSearch)
