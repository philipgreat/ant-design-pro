...
import styles from './VehicleInspectionOrderCharge.edittable.less'
...
class VehicleInspectionOrderChargeEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const vehicleInspectionOrderChargeIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addVehicleInspectionOrderCharge`,
        payload: {
          id: owner.id,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {vehicleInspectionOrderChargeId} = record.id
      const parameters = { ...record, vehicleInspectionOrderChargeId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '代码', debugtype: 'string', dataIndex: 'code', width: '14', render: (text, record) => renderStringEdit('code',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newVehicleInspectionOrderChargeToAppend  = {
      	'id':`+1`, 
				'title':'',
				'code':'',
				'amount':'',
      ...
      newData.push(newVehicleInspectionOrderChargeToAppend);
...
export default Form.create()(VehicleInspectionOrderChargeEditTable)
