...
import styles from './VehicleInspectionOrderCharge.createform.less'
...
class VehicleInspectionOrderChargeCreateForm extends Component {
  state = {
  ...
  executeCandidateMainOrderSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = VehicleInspectionOrderChargeService.requestCandidateMainOrder("vehicleInspectionOrder", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addVehicleInspectionOrderCharge`,
          payload: { id: owner.id, type: 'vehicleInspectionOrderCharge', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'vehicleInspectionOrderCharge',listName:'车辆检验订单费用列表' },
      })
    }
...
}))(Form.create()(VehicleInspectionOrderChargeCreateForm))
