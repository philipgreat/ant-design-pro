...
import styles from './BookManagement.createform.less'
...
class BookManagementCreateForm extends Component {
  state = {
  ...
  executeCandidateBookSharingPlatformSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = BookManagementService.requestCandidateBookSharingPlatform("bookSharingPlatform", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addBookManagement`,
          payload: { id: owner.id, type: 'bookManagement', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'bookManagement',listName:'图书管理列表' },
      })
    }
          ...
          <Form >
            <Row gutter={16}>
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.bookManagementName} {...formItemLayout}>
                  {getFieldDecorator('bookManagementName', {
                    rules: [{ required: true, message: '请输入书管理的名字' }],
                  })(
                    <Input placeholder="请输入书管理的名字" />
                  )}
                </Form.Item>
              </Col>
...
}))(Form.create()(BookManagementCreateForm))
