...
const addBookBorrowConfiguration = (targetObjectId, parameters) => {
  const url = `${PREFIX}bookManagementManager/addBookBorrowConfiguration/bookManagementId/maxBorrowQuantity/freeLendingDays/lendingPayRate/tokensExpr/`
  const requestParameters = { ...parameters, tokensExpr: 'none' }
...
const updateBookBorrowConfiguration = (targetObjectId, parameters) => {
  const url = `${PREFIX}bookManagementManager/updateBookBorrowConfigurationProperties/bookManagementId/id/maxBorrowQuantity/freeLendingDays/lendingPayRate/tokensExpr/`
  const bookManagementId = targetObjectId
  const requestParameters = { ...parameters, bookManagementId, tokensExpr: 'none' }
  const headers = { 'Content-Type': 'application/x-www-form-urlencoded' }
  return post({
