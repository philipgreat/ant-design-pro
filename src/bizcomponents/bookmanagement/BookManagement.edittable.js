...
import styles from './BookManagement.edittable.less'
...
class BookManagementEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const bookManagementIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addBookManagement`,
        payload: {
          id: owner.id,
          type: 'bookManagement',
          parameters,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {bookManagementId} = record.id
      const parameters = { ...record, bookManagementId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateBookManagement`,
        payload: {
          id: owner.id,
          type: 'bookManagement',
          parameters,
  ...
  { title: '书管理的名字', debugtype: 'string', dataIndex: 'bookManagementName', width: '10', render: (text, record) => renderStringEdit('bookManagementName',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newBookManagementToAppend  = {
      	'id':`+1`, 
				'bookManagementName':'',
				'bookSharingPlatform':'',
      ...
      newData.push(newBookManagementToAppend);
...
export default Form.create()(BookManagementEditTable)
