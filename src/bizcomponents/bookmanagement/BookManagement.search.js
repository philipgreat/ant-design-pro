...
import styles from './BookManagement.search.less'
...
class BookManagementSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookManagementIds = selectedRows.map((item) => { return item.id })
    console.log('bookManagementIds', bookManagementIds)
    const parameters = { bookManagementIds }
    dispatch({
      type: `${owner.type}/removeBookManagementList`,
      payload: { id: owner.id, type: 'bookManagement', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookManagement' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookManagement', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'图书管理列表' } })
  ...
  render() {
    ...
    const {BookManagementTable} = GlobalComponents;
    const {BookManagementConfirmationTable} = GlobalComponents;
    const {BookManagementSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookManagementConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 图书管理列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookManagementSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookManagementSearch)
