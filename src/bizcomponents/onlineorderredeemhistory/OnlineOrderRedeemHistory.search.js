...
import styles from './OnlineOrderRedeemHistory.search.less'
...
class OnlineOrderRedeemHistorySearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const onlineOrderRedeemHistoryIds = selectedRows.map((item) => { return item.id })
    console.log('onlineOrderRedeemHistoryIds', onlineOrderRedeemHistoryIds)
    const parameters = { onlineOrderRedeemHistoryIds }
    dispatch({
      type: `${owner.type}/removeOnlineOrderRedeemHistoryList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'onlineOrderRedeemHistory' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'onlineOrderRedeemHistory', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'线上订单兑换记录列表' } })
  ...
  render() {
    ...
    const {OnlineOrderRedeemHistoryTable} = GlobalComponents;
    const {OnlineOrderRedeemHistoryConfirmationTable} = GlobalComponents;
    const {OnlineOrderRedeemHistorySearchForm} = GlobalComponents;
...
<Menu.Item key="onlineOrder">关联线上订单号</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <OnlineOrderRedeemHistoryConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 线上订单兑换记录列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <OnlineOrderRedeemHistorySearchForm {...this.props} />
            </div>
...
export default Form.create()(OnlineOrderRedeemHistorySearch)
