...
import styles from './HomePage.edittable.less'
...
class HomePageEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {homePageId} = record.id
      const parameters = { ...record, homePageId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newHomePageToAppend);
...
export default Form.create()(HomePageEditTable)
