...
class HomePageCreateForm extends Component {
  state = {
  ...
  executeCandidateCommunitySearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = HomePageService.requestCandidateCommunity("community", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addHomePage`,
          payload: { id: owner.id, type: 'homePage', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'homePage',listName:'主页列表' },
      })
    }
