...
import styles from './TaskLike.search.less'
...
class TaskLikeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const taskLikeIds = selectedRows.map((item) => { return item.id })
    console.log('taskLikeIds', taskLikeIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'taskLike', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'任务点赞列表' } })
  ...
  render() {
    ...
    const {TaskLikeTable} = GlobalComponents;
    const {TaskLikeConfirmationTable} = GlobalComponents;
    const {TaskLikeSearchForm} = GlobalComponents;
...
<Menu.Item key="task">关联任务</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TaskLikeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 任务点赞列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TaskLikeSearchForm {...this.props} />
            </div>
...
export default Form.create()(TaskLikeSearch)
