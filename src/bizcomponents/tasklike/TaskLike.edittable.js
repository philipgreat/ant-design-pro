...
import styles from './TaskLike.edittable.less'
...
class TaskLikeEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e, record) => {
      const { dispatch, owner } = this.props
      const { data } = this.state
      const { taskLikeId } = record.id
      const parameters = { ...record, taskLikeId }
      const newData = [...data]
      const row = newData.filter(item => item.id === record.id)[0]
      row.editable = !row.editable
    ...
    const newRecord = () => {
      ...
      newData.push(newTaskLikeToAppend)
...
export default Form.create()(TaskLikeEditTable)
