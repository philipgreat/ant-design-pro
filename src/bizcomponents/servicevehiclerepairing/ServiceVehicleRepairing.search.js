...
import styles from './ServiceVehicleRepairing.search.less'
...
class ServiceVehicleRepairingSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const serviceVehicleRepairingIds = selectedRows.map((item) => { return item.id })
    console.log('serviceVehicleRepairingIds', serviceVehicleRepairingIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'serviceVehicleRepairing', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'维修服务列表' } })
  ...
  render() {
    ...
    const {ServiceVehicleRepairingTable} = GlobalComponents;
    const {ServiceVehicleRepairingConfirmationTable} = GlobalComponents;
    const {ServiceVehicleRepairingSearchForm} = GlobalComponents;
...
<Menu.Item key="serviceVehicleInspection">关联车辆上线检测</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ServiceVehicleRepairingConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 维修服务列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ServiceVehicleRepairingSearchForm {...this.props} />
            </div>
...
export default Form.create()(ServiceVehicleRepairingSearch)
