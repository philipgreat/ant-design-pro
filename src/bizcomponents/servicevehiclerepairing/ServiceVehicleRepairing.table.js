...
const columns = [
  ...
  { title: '车辆上线检测', dataIndex: 'serviceVehicleInspection', render: (text, record) => (record.serviceVehicleInspection ? (<Link to={`/serviceVehicleInspection/${record.serviceVehicleInspection.id}/dashboard`}>{record.serviceVehicleInspection.displayName}</Link>) : '暂无') },
