...
import styles from './PreorderPromotion.search.less'
...
class PreorderPromotionSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const preorderPromotionIds = selectedRows.map((item) => { return item.id })
    console.log('preorderPromotionIds', preorderPromotionIds)
    const parameters = { preorderPromotionIds }
    dispatch({
      type: `${owner.type}/removePreorderPromotionList`,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'preorderPromotion', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'提前下单优惠列表' } })
  ...
  render() {
    ...
    const {PreorderPromotionTable} = GlobalComponents;
    const {PreorderPromotionConfirmationTable} = GlobalComponents;
    const {PreorderPromotionSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <PreorderPromotionConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 提前下单优惠列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <PreorderPromotionSearchForm {...this.props} />
            </div>
...
export default Form.create()(PreorderPromotionSearch)
