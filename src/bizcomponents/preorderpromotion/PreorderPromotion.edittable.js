...
import styles from './PreorderPromotion.edittable.less'
...
class PreorderPromotionEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const preorderPromotionIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {preorderPromotionId} = record.id
      const parameters = { ...record, preorderPromotionId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '促销信息', debugtype: 'string', dataIndex: 'promotionMessage', width: '15', render: (text, record) => renderStringEdit('promotionMessage',text, record)  },
  { title: '预订的日子', debugtype: 'int', dataIndex: 'preorderDays', width: '6', render: (text, record) => renderStringEdit('preorderDays',text, record)  },
  { title: '折扣金额', dataIndex: 'discountAmount', className:'money', render: (text, record) => ('￥'+text.toFixed(2)) },
  { title: '生效日期', dataIndex: 'startDate', render: (text, record) => moment(record.startDate).format('YYYY-MM-DD HH:mm:ss') },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newPreorderPromotionToAppend  = {
      	'id':`+1`, 
				'promotionMessage':'',
				'preorderDays':'',
				'discountAmount':'',
      ...
      newData.push(newPreorderPromotionToAppend);
...
export default Form.create()(PreorderPromotionEditTable)
