...
import styles from './OrderDiscountCoupon.search.less'
...
class OrderDiscountCouponSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const orderDiscountCouponIds = selectedRows.map((item) => { return item.id })
    console.log('orderDiscountCouponIds', orderDiscountCouponIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'orderDiscountCoupon' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'orderDiscountCoupon', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'优惠券列表' } })
  ...
  render() {
    ...
    const {OrderDiscountCouponTable} = GlobalComponents;
    const {OrderDiscountCouponConfirmationTable} = GlobalComponents;
    const {OrderDiscountCouponSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <OrderDiscountCouponConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 优惠券列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <OrderDiscountCouponSearchForm {...this.props} />
            </div>
...
export default Form.create()(OrderDiscountCouponSearch)
