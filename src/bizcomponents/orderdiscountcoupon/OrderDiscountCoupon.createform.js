...
class OrderDiscountCouponCreateForm extends Component {
  ...
  executeCandidateCustomerSearch = filterKey => {
    ...
    const pageNo = 1
    const future = OrderDiscountCouponService.requestCandidateCustomer(
      'customer',
      id,
      filterKey,
      pageNo
  ...
  executeCandidateMainOrderSearch = filterKey => {
    ...
    const id = '' //not used for now
    const pageNo = 1
    const future = OrderDiscountCouponService.requestCandidateMainOrder(
  ...
  executeCandidatePlatformSearch = filterKey => {
    ...
    const id = '' //not used for now
    const pageNo = 1
    const future = OrderDiscountCouponService.requestCandidatePlatform(
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label={fieldLabels.shareCode}>
                  {getFieldDecorator('shareCode', {
                    rules: [
                      { required: true, message: '请输入优惠券分享随机码' },
                    ],
                  })(
