...
import styles from './ProductPrice.edittable.less'
...
class ProductPriceEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const productPriceIds = [record.id];
      const parameters = { productPriceIds }
      dispatch({
        type: `${owner.type}/removeProductPriceList`,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {productPriceId} = record.id
      const parameters = { ...record, productPriceId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '车辆类型', debugtype: 'string', dataIndex: 'vehicleType', width: '6', render: (text, record) => renderStringEdit('vehicleType',text, record)  },
  ...
  { title: '折扣价格代理服务', dataIndex: 'discountAgentServicePrice', className:'money', render: (text, record) => ('￥'+text.toFixed(2)) },
  { title: '描述', debugtype: 'string', dataIndex: 'description', width: '36', render: (text, record) => renderStringEdit('description',text, record)  },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newProductPriceToAppend  = {
      	'id':`+1`, 
				'product':'',
				'city':'',
				'vehicleType':'',
				'greenVehicle':'',
				'inspectionPrice':'',
				'secondLevelMaintenancePrice':'',
				'gradeEstimationPrice':'',
				'agentServicePrice':'',
				'discountAgentServicePrice':'',
				'description':'',


      };
      const newData = data ? [...data]:[];
      newData.push(newProductPriceToAppend);
...
export default Form.create()(ProductPriceEditTable)
