...
import styles from './EquipmentFile.search.less'
...
@Form.create()
export default class EquipmentFileSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const equipmentFileIds = selectedRows.map((item) => { return item.id })
    console.log('equipmentFileIds', equipmentFileIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'equipmentFile' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'equipmentFile', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {EquipmentFileTable} = GlobalComponents;
    const {EquipmentFileConfirmationTable} = GlobalComponents;
    const {EquipmentFileSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EquipmentFileConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="设备文件列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <EquipmentFileSearchForm {...this.props} />
            </div>
