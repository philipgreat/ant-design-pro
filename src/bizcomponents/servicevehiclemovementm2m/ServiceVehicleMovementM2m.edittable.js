...
import styles from './ServiceVehicleMovementM2m.edittable.less'
...
class ServiceVehicleMovementM2mEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
      ...
      return {responsibleWorkerId,mainOrderId,driverId,receiverId,merchantId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const serviceVehicleMovementM2mIds = [record.id];
      const parameters = { serviceVehicleMovementM2mIds }
      dispatch({
        type: `${owner.type}/removeServiceVehicleMovementM2mList`,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {serviceVehicleMovementM2mId} = record.id
      const parameters = { ...record, serviceVehicleMovementM2mId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '服务类型', debugtype: 'string', dataIndex: 'movementPurpose', width: '30', render: (text, record) => renderStringEdit('movementPurpose',text, record)  },
  ...
  { title: '通知的评论', debugtype: 'string', dataIndex: 'notifyComment', width: '14', render: (text, record) => renderStringEdit('notifyComment',text, record)  },
  ...
  { title: '交接检查备注', debugtype: 'string', dataIndex: 'handoverResultComment', width: '22', render: (text, record) => renderStringEdit('handoverResultComment',text, record)  },
  { title: '商户', dataIndex: 'merchant', render: (text, record) => (record.merchant ? record.merchant.id : '暂无') },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newServiceVehicleMovementM2mToAppend  = {
      	'id':`+1`, 
				'serviceStatus':'',
				'responsibleWorker':'',
				'serviceSummary':'',
      ...
      newData.push(newServiceVehicleMovementM2mToAppend);
...
export default Form.create()(ServiceVehicleMovementM2mEditTable)
