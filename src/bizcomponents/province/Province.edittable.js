...
import styles from './Province.edittable.less'
...
class ProvinceEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {provinceId} = record.id
      const parameters = { ...record, provinceId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateProvince`,
        payload: {
          id: owner.id,
          type: 'province',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newProvinceToAppend);
...
export default Form.create()(ProvinceEditTable)
