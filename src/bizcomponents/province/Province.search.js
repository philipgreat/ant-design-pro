...
import styles from './Province.search.less'
...
class ProvinceSearch extends PureComponent {
  ...
  handleDelete = () => {
    ...
    const provinceIds = selectedRows.map((item) => { return item.id })
    console.log('provinceIds', provinceIds)
    const parameters = { provinceIds }
    dispatch({
      type: `${owner.type}/removeProvinceList`,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'province', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'省列表' } })
  ...
  render() {
    ...
    const {ProvinceTable} = GlobalComponents;
    const {ProvinceConfirmationTable} = GlobalComponents;
    const {ProvinceSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ProvinceConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 省列表`}>
        <Card bordered={false}>
