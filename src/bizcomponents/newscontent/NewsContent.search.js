...
import styles from './NewsContent.search.less'
...
class NewsContentSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const newsContentIds = selectedRows.map((item) => { return item.id })
    console.log('newsContentIds', newsContentIds)
    const parameters = { newsContentIds }
    dispatch({
      type: `${owner.type}/removeNewsContentList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'newsContent' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'newsContent', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'新闻内容列表' } })
  ...
  render() {
    ...
    const {NewsContentTable} = GlobalComponents;
    const {NewsContentConfirmationTable} = GlobalComponents;
    const {NewsContentSearchForm} = GlobalComponents;
...
<Menu.Item key="news">关联新闻</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <NewsContentConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 新闻内容列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <NewsContentSearchForm {...this.props} />
            </div>
...
export default Form.create()(NewsContentSearch)
