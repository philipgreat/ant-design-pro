...
import styles from './GameSessionRecord.search.less'
...
class GameSessionRecordSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const gameSessionRecordIds = selectedRows.map((item) => { return item.id })
    console.log('gameSessionRecordIds', gameSessionRecordIds)
    const parameters = { gameSessionRecordIds }
    dispatch({
      type: `${owner.type}/removeGameSessionRecordList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'gameSessionRecord' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'gameSessionRecord', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'游戏场次记录列表' } })
  ...
  render() {
    ...
    const {GameSessionRecordTable} = GlobalComponents;
    const {GameSessionRecordConfirmationTable} = GlobalComponents;
    const {GameSessionRecordSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <GameSessionRecordConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 游戏场次记录列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <GameSessionRecordSearchForm {...this.props} />
            </div>
...
export default Form.create()(GameSessionRecordSearch)
