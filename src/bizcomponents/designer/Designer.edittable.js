...
import styles from './Designer.edittable.less'
...
class DesignerEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {designerId} = record.id
      const parameters = { ...record, designerId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateDesigner`,
        payload: {
          id: owner.id,
          type: 'designer',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newDesignerToAppend);
...
export default Form.create()(DesignerEditTable)
