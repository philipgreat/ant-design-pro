...
import styles from './Designer.updateform.less'
...
class DesignerUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const designerId = values.id
        const imagesValues = this.mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, designerId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextDesignerUpdateRow`,
        payload: {
          id: owner.id,
          type: 'designer',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新设计师"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新设计师"
...
}))(Form.create()(DesignerUpdateForm))
