...
import styles from './Designer.search.less'
...
@Form.create()
export default class DesignerSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const designerIds = selectedRows.map((item) => { return item.id })
    console.log('designerIds', designerIds)
    const parameters = { designerIds }
    dispatch({
      type: `${owner.type}/removeDesignerList`,
      payload: { id: owner.id, type: 'designer', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'designer' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'designer', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {DesignerTable} = GlobalComponents;
    const {DesignerConfirmationTable} = GlobalComponents;
    const {DesignerSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <DesignerConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="设计师列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <DesignerSearchForm {...this.props} />
            </div>
