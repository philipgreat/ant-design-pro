...
const summaryOf = identityCard => {
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="身份证正面照片">
        <ImagePreview
          imageTitle="身份证正面照片"
          imageLocation={identityCard.frontImage}
        />
      </Description>
      <Description term="身份证背面照片">
        <ImagePreview
          imageTitle="身份证背面照片"
          imageLocation={identityCard.backImage}
        />
      </Description>
      <Description term="有效期至">
        {moment(identityCard.expirationDate).format('YYYY-MM-DD')}
      </Description>
    </DescriptionList>
  )
}
...
export default class IdentityCardDashboard extends Component {
  ...
  render() {
    ...
    const cardsData = {
      cardsName: '身份证',
      cardsFor: 'identityCard',
      cardsSource: this.props.identityCard,
      subItems: [],
    }
