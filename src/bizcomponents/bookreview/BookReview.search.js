...
import styles from './BookReview.search.less'
...
class BookReviewSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookReviewIds = selectedRows.map((item) => { return item.id })
    console.log('bookReviewIds', bookReviewIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookReview' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookReview', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'书评列表' } })
  ...
  render() {
    ...
    const {BookReviewTable} = GlobalComponents;
    const {BookReviewConfirmationTable} = GlobalComponents;
    const {BookReviewSearchForm} = GlobalComponents;
...
<Menu.Item key="bookInfo">关联书的信息</Menu.Item>
<Menu.Item key="bookCopy">关联书副本</Menu.Item>
<Menu.Item key="bookReviewType">关联书评类型</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookReviewConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 书评列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookReviewSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookReviewSearch)
