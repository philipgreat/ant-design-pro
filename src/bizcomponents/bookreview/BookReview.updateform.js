...
import styles from './BookReview.updateform.less'
...
class BookReviewUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const bookReviewId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, bookReviewId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextBookReviewUpdateRow`,
        payload: {
          id: owner.id,
          type: 'bookReview',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新书评"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新书评"
...
}))(Form.create()(BookReviewUpdateForm))
