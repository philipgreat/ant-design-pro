...
@Form.create()
export default class HrInterviewSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const hrInterviewIds = selectedRows.map((item) => { return item.id })
    console.log('hrInterviewIds', hrInterviewIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'hrInterview' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'hrInterview', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {HrInterviewConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <HrInterviewConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 人力资源部面试列表`}>
        <Card bordered={false}>
