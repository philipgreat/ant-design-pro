...
const columns = [
  ...
  { title: '躲藏', dataIndex: 'hiding', render: (text, record) => (record.hiding ? (<Link to={`/taskHiding/${record.hiding.id}/dashboard`}>{record.hiding.displayName}</Link>) : '暂无') },
  { title: '解决', dataIndex: 'resolving', render: (text, record) => (record.resolving ? (<Link to={`/taskResolving/${record.resolving.id}/dashboard`}>{record.resolving.displayName}</Link>) : '暂无') },
  { title: '悬赏', dataIndex: 'reward', render: (text, record) => (record.reward ? (<Link to={`/taskReward/${record.reward.id}/dashboard`}>{record.reward.displayName}</Link>) : '暂无') },
