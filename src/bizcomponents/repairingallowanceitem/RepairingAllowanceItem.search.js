...
import styles from './RepairingAllowanceItem.search.less'
...
class RepairingAllowanceItemSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const repairingAllowanceItemIds = selectedRows.map((item) => { return item.id })
    console.log('repairingAllowanceItemIds', repairingAllowanceItemIds)
    const parameters = { repairingAllowanceItemIds }
    dispatch({
      type: `${owner.type}/removeRepairingAllowanceItemList`,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'repairingAllowanceItem', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'车辆维修补贴项列表' } })
  ...
  render() {
    ...
    const {RepairingAllowanceItemTable} = GlobalComponents;
    const {RepairingAllowanceItemConfirmationTable} = GlobalComponents;
    const {RepairingAllowanceItemSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <RepairingAllowanceItemConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 车辆维修补贴项列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <RepairingAllowanceItemSearchForm {...this.props} />
            </div>
...
export default Form.create()(RepairingAllowanceItemSearch)
