...
import styles from './RepairingAllowanceItem.edittable.less'
...
class RepairingAllowanceItemEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const repairingAllowanceItemIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {repairingAllowanceItemId} = record.id
      const parameters = { ...record, repairingAllowanceItemId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newRepairingAllowanceItemToAppend  = {
      	'id':`+1`, 
				'allowanceTitle':'',
				'allowanceCode':'',
				'allowanceAmount':'',
      ...
      newData.push(newRepairingAllowanceItemToAppend);
...
export default Form.create()(RepairingAllowanceItemEditTable)
