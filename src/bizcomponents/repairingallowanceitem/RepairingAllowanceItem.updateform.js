...
import styles from './RepairingAllowanceItem.updateform.less'
...
class RepairingAllowanceItemUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const repairingAllowanceItemId = values.id
        const imagesValues = this.mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, repairingAllowanceItemId, ...imagesValues }
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextRepairingAllowanceItemUpdateRow`,
        payload: {
          id: owner.id,
          type: 'repairingAllowanceItem',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新修补贴项目"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新修补贴项目"
...
}))(Form.create()(RepairingAllowanceItemUpdateForm))
