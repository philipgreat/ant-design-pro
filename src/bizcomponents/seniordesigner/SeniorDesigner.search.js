...
import styles from './SeniorDesigner.search.less'
...
@Form.create()
export default class SeniorDesignerSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const seniorDesignerIds = selectedRows.map((item) => { return item.id })
    console.log('seniorDesignerIds', seniorDesignerIds)
    const parameters = { seniorDesignerIds }
    dispatch({
      type: `${owner.type}/removeSeniorDesignerList`,
      payload: { id: owner.id, type: 'seniorDesigner', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'seniorDesigner' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'seniorDesigner', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {SeniorDesignerTable} = GlobalComponents;
    const {SeniorDesignerConfirmationTable} = GlobalComponents;
    const {SeniorDesignerSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <SeniorDesignerConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="高级设计师列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <SeniorDesignerSearchForm {...this.props} />
            </div>
