...
import styles from './SeniorDesigner.edittable.less'
...
class SeniorDesignerEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const seniorDesignerIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addSeniorDesigner`,
        payload: {
          id: owner.id,
          type: 'seniorDesigner',
          parameters,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {seniorDesignerId} = record.id
      const parameters = { ...record, seniorDesignerId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateSeniorDesigner`,
        payload: {
          id: owner.id,
          type: 'seniorDesigner',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newSeniorDesignerToAppend  = {
      	'id':`+1`, 
				'name':'',
				'description':'',
				'owner':'',
      ...
      newData.push(newSeniorDesignerToAppend);
...
export default Form.create()(SeniorDesignerEditTable)
