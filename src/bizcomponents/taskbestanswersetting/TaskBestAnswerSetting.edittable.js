...
import styles from './TaskBestAnswerSetting.edittable.less'
...
class TaskBestAnswerSettingEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const taskBestAnswerSettingIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addTaskBestAnswerSetting`,
        payload: {
          id: owner.id,
          type: 'taskBestAnswerSetting',
          parameters,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {taskBestAnswerSettingId} = record.id
      const parameters = { ...record, taskBestAnswerSettingId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateTaskBestAnswerSetting`,
        payload: {
          id: owner.id,
          type: 'taskBestAnswerSetting',
          parameters,
  ...
  { title: '谁', debugtype: 'string_current_user_name', dataIndex: 'who', width: '21', render: (text, record) => renderStringEdit('who',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newTaskBestAnswerSettingToAppend  = {
      	'id':`+1`, 
				'who':'',
				'setTime':'',
				'comment':'',
      ...
      newData.push(newTaskBestAnswerSettingToAppend);
...
export default Form.create()(TaskBestAnswerSettingEditTable)
