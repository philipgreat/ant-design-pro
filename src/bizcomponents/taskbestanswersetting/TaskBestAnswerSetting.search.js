...
import TaskBestAnswerSettingTable from './TaskBestAnswerSetting.table';
import TaskBestAnswerSettingConfirmationTable from './TaskBestAnswerSetting.confirmmationtable';

import TaskBestAnswerSettingSearchForm from './TaskBestAnswerSetting.searchform';

import PageHeaderLayout from '../../layouts/PageHeaderLayout';

import styles from './TaskBestAnswerSetting.search.less';
...
@Form.create()
export default class TaskBestAnswerSettingSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    const params = {
      taskBestAnswerSettingList: 1,
      taskBestAnswerSettingListCurrentPage: pagination.current,
      taskBestAnswerSettingListRowsPerPage: pagination.pageSize,
      ...formValues,
      ...filters,
    };
    if (sorter.field) {
  ...
  handleDelete = () => {
    ...
    const taskBestAnswerSettingIds = selectedRows.map((item) => { return item.id; });
    console.log('taskBestAnswerSettingIds', taskBestAnswerSettingIds);
    const parameters = { taskBestAnswerSettingIds };
    dispatch({
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'taskBestAnswerSetting', selectedRows, currentUpdateIndex },
    });
  }
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TaskBestAnswerSettingConfirmationTable data={selectedRows} owner={owner} />
        </Modal>);
    };
    
    return (
      <PageHeaderLayout title="任务最佳答案设置列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <TaskBestAnswerSettingSearchForm {...this.props} />
            </div>
