...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        const editDetailMatch = pathToRegexp('/taskBestAnswerSetting/:id/editDetail').exec(pathname)
        if (editDetailMatch) {
          const id = editDetailMatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const viewDetailMatch = pathToRegexp('/taskBestAnswerSetting/:id/viewDetail').exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp('/taskBestAnswerSetting/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    *view({ payload }, { call, put }) { 
      const {TaskBestAnswerSettingService} = GlobalComponents;
    ...
    *gotoUpdateForm({ payload }, { put }) {
      ...
      const location = { pathname: `/taskBestAnswerSetting/${id}/list/${type}UpdateForm`, state }
      yield put(routerRedux.push(location))
    },
    *goback({ payload }, { put }) {
      const { id, type,listName } = payload
      yield put(routerRedux.push(`/taskBestAnswerSetting/${id}/list/${type}List/${listName}`))
    },
    ...
    *addTaskReply({ payload }, { call, put }) {
      ...
      const location = { pathname: `/taskBestAnswerSetting/${id}/list/${type}List/回复任务列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateTaskReply({ payload }, { call, put }) {
      ...
      const location = { pathname: `/taskBestAnswerSetting/${id}/list/${type}List/回复任务列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
