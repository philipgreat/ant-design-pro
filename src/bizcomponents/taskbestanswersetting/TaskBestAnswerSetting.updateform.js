...
import styles from './TaskBestAnswerSetting.updateform.less';
...
class TaskBestAnswerSettingUpdateForm extends PureComponent {
 ...
 mapBackToImageValues(convertedImagesValues) {
  ...
  getSelectedRow(){
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
			setTime: moment(item.setTime).format('YYYY-MM-DD'),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
     validateFieldsAndScroll((error, values) => {
        ...
        const taskBestAnswerSettingId = values.id;
        const imagesValues = this.mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values,taskBestAnswerSettingId, ...imagesValues };
    ...
    const skipToNext = () => {
      ...
      const newIndex= currentUpdateIndex+1;
      dispatch({
          type: owner.type+'/gotoNextTaskBestAnswerSettingUpdateRow',
            payload: {
              id:owner.id,type:'taskBestAnswerSetting', 
    ...
    return (
      <PageHeaderLayout
        ...
        title={"更新任务最佳答案设置"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新任务最佳答案设置"
...
}))(Form.create()(TaskBestAnswerSettingUpdateForm));
