...
import styles from './OutputInterface.search.less'
...
@Form.create()
export default class OutputInterfaceSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const outputInterfaceIds = selectedRows.map((item) => { return item.id })
    console.log('outputInterfaceIds', outputInterfaceIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'outputInterface' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'outputInterface', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {OutputInterfaceTable} = GlobalComponents;
    const {OutputInterfaceConfirmationTable} = GlobalComponents;
    const {OutputInterfaceSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <OutputInterfaceConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="输出接口列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <OutputInterfaceSearchForm {...this.props} />
            </div>
