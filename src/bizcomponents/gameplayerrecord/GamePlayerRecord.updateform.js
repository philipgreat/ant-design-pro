...
import styles from './GamePlayerRecord.updateform.less'
...
class GamePlayerRecordUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const gamePlayerRecordId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, gamePlayerRecordId, ...imagesValues }

        // const newIndex= currentUpdateIndex + 1
        dispatch({
          type: `${owner.type}/updateGamePlayerRecord`,
          payload: {
            id: owner.id,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextGamePlayerRecordUpdateRow`,
        payload: {
          id: owner.id,
          type: 'gamePlayerRecord',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新游戏玩家记录"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新游戏玩家记录"
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.playerNickName} {...formItemLayout}>
                  {getFieldDecorator('playerNickName', {
                    initialValue: selectedRow.playerNickName,
...
}))(Form.create()(GamePlayerRecordUpdateForm))
