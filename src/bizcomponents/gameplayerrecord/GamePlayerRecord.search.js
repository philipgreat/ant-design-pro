...
import styles from './GamePlayerRecord.search.less'
...
class GamePlayerRecordSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const gamePlayerRecordIds = selectedRows.map((item) => { return item.id })
    console.log('gamePlayerRecordIds', gamePlayerRecordIds)
    const parameters = { gamePlayerRecordIds }
    dispatch({
      type: `${owner.type}/removeGamePlayerRecordList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'gamePlayerRecord' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'gamePlayerRecord', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'游戏玩家记录列表' } })
  ...
  render() {
    ...
    const {GamePlayerRecordTable} = GlobalComponents;
    const {GamePlayerRecordConfirmationTable} = GlobalComponents;
    const {GamePlayerRecordSearchForm} = GlobalComponents;
...
<Menu.Item key="player">关联玩家</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <GamePlayerRecordConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 游戏玩家记录列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <GamePlayerRecordSearchForm {...this.props} />
            </div>
...
export default Form.create()(GamePlayerRecordSearch)
