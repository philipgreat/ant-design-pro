...
import styles from './BookReviewType.search.less'
...
class BookReviewTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bookReviewTypeIds = selectedRows.map((item) => { return item.id })
    console.log('bookReviewTypeIds', bookReviewTypeIds)
    const parameters = { bookReviewTypeIds }
    dispatch({
      type: `${owner.type}/removeBookReviewTypeList`,
      payload: { id: owner.id, type: 'bookReviewType', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookReviewType' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'bookReviewType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'书评类型列表' } })
  ...
  render() {
    ...
    const {BookReviewTypeTable} = GlobalComponents;
    const {BookReviewTypeConfirmationTable} = GlobalComponents;
    const {BookReviewTypeSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BookReviewTypeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 书评类型列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <BookReviewTypeSearchForm {...this.props} />
            </div>
...
export default Form.create()(BookReviewTypeSearch)
