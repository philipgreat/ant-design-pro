...
const columns = [
  { title: 'ID', debugtype: 'string', dataIndex: 'id',  render: (text, record)=>(<Link to={`/bookReviewType/${text}/dashboard`}>{text}</Link>) },
  { title: '书评类型名称', debugtype: 'string', dataIndex: 'bookReviewTypeTitle',},
  { title: '书评类型描述', debugtype: 'string', dataIndex: 'bookReviewTypeDescription',},
  { title: '图书管理', dataIndex: 'bookManagement', render: (text, record) => (record.bookManagement ? record.bookManagement.displayName : '暂无') },


]

class BookReviewTypeTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
...
export default BookReviewTypeTable
