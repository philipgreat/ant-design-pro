...
import styles from './VehicleServiceCompanyBusinessScope.createform.less'
...
const fieldLabels = {
  ...
  serviceAvaliability: '可用状态',
}
const testValues = {};
/*
const testValues = {
  serviceAvaliability: 'true',
  companyId: 'VSC000001',
...
class VehicleServiceCompanyBusinessScopeCreateForm extends Component {
  state = {
  ...
  executeCandidateCompanySearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = VehicleServiceCompanyBusinessScopeService.requestCandidateCompany("vehicleServiceCompany", id, filterKey, pageNo);
    console.log(future);
  ...
  executeCandidateAvailableServiceSearch = (filterKey) =>{
    ...
    const id = "";//not used for now
    const pageNo = 1;
    const future = VehicleServiceCompanyBusinessScopeService.requestCandidateAvailableService("availableService", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addVehicleServiceCompanyBusinessScope`,
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addVehicleServiceCompanyBusinessScope`,
          payload: { id: owner.id, type: 'vehicleServiceCompanyBusinessScope', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'vehicleServiceCompanyBusinessScope' },
      })
    }
          ...
          <Form layout="vertical" hideRequiredMark>
            <Row gutter={16}>
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label={fieldLabels.serviceAvaliability}>
                  {getFieldDecorator('serviceAvaliability', {
                    rules: [{ required: true, message: '请输入可用状态' }],
                  })(
                    <Input placeholder="请输入请输入可用状态bool" />
                  )}
                </Form.Item>
              </Col>
              ...
              <Col lg={6} md={12} sm={24}>
                <Form.Item label={fieldLabels.serviceAvaliability}>
                  {getFieldDecorator('serviceAvaliability', {
                    ...
                    <Switch checkedChildren="是" unCheckedChildren="否"  placeholder="请输入可用状态bool" />
                  )}
                </Form.Item>
              </Col>
...
}))(Form.create()(VehicleServiceCompanyBusinessScopeCreateForm))
