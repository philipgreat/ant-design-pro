...
class VehicleServiceCompanyBusinessScopeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'商户服务范围列表' } })
...
<Menu.Item key="availableService">关联服务范围</Menu.Item>
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 商户服务范围列表`}>
        <Card bordered={false}>
...
export default Form.create()(VehicleServiceCompanyBusinessScopeSearch)
