...
import styles from './AccountManagement.edittable.less'
...
class AccountManagementEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const accountManagementIds = [record.id];
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addAccountManagement`,
        payload: {
          id: owner.id,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {accountManagementId} = record.id
      const parameters = { ...record, accountManagementId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateAccountManagement`,
        payload: {
          id: owner.id,
          type: 'accountManagement',
          parameters,
  ...
  { title: '总结', debugtype: 'string', dataIndex: 'summary', width: '10', render: (text, record) => renderStringEdit('summary',text, record)  },
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newAccountManagementToAppend  = {
      	'id':`+1`, 
				'summary':'',
				'bookSharingPlatform':'',
      ...
      newData.push(newAccountManagementToAppend);
...
export default Form.create()(AccountManagementEditTable)
