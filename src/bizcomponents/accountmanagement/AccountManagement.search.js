...
import styles from './AccountManagement.search.less'
...
class AccountManagementSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const accountManagementIds = selectedRows.map((item) => { return item.id })
    console.log('accountManagementIds', accountManagementIds)
    const parameters = { accountManagementIds }
    dispatch({
      type: `${owner.type}/removeAccountManagementList`,
      payload: { id: owner.id, type: 'accountManagement', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'accountManagement' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'accountManagement', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'账户管理列表' } })
  ...
  render() {
    ...
    const {AccountManagementTable} = GlobalComponents;
    const {AccountManagementConfirmationTable} = GlobalComponents;
    const {AccountManagementSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <AccountManagementConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 账户管理列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <AccountManagementSearchForm {...this.props} />
            </div>
...
export default Form.create()(AccountManagementSearch)
