...
import styles from './AccountManagement.createform.less'
...
class AccountManagementCreateForm extends Component {
  state = {
  ...
  executeCandidateBookSharingPlatformSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = AccountManagementService.requestCandidateBookSharingPlatform("bookSharingPlatform", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addAccountManagement`,
          payload: { id: owner.id, type: 'accountManagement', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'accountManagement',listName:'账户管理列表' },
      })
    }
...
}))(Form.create()(AccountManagementCreateForm))
