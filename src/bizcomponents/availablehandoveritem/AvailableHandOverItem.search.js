...
import styles from './AvailableHandOverItem.search.less'
...
class AvailableHandOverItemSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const availableHandOverItemIds = selectedRows.map((item) => { return item.id })
    console.log('availableHandOverItemIds', availableHandOverItemIds)
    const parameters = { availableHandOverItemIds }
    dispatch({
      type: `${owner.type}/removeAvailableHandOverItemList`,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'availableHandOverItem', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {AvailableHandOverItemTable} = GlobalComponents;
    const {AvailableHandOverItemConfirmationTable} = GlobalComponents;
    const {AvailableHandOverItemSearchForm} = GlobalComponents;
...
<Menu.Item key="product">关联产品名称</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <AvailableHandOverItemConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
        ...
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <AvailableHandOverItemSearchForm {...this.props} />
            </div>
...
export default Form.create()(AvailableHandOverItemSearch)
