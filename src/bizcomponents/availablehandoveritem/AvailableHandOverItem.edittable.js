...
import styles from './AvailableHandOverItem.edittable.less'
...
class AvailableHandOverItemEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const availableHandOverItemIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {availableHandOverItemId} = record.id
      const parameters = { ...record, availableHandOverItemId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
  ...
  { title: '检查项目描述', debugtype: 'string_longtext', dataIndex: 'checkItemDescription', width: '10', render: (text, record) => renderStringEdit('checkItemDescription',text, record)  },
  { title: '产品名称', dataIndex: 'product', render: (text, record) => (record.product ? record.product.displayName : '暂无') },
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
   
    const newRecord =()=>{
      const newAvailableHandOverItemToAppend  = {
      	'id':`+1`, 
				'checkItemName':'',
      ...
      newData.push(newAvailableHandOverItemToAppend);
...
export default Form.create()(AvailableHandOverItemEditTable)
