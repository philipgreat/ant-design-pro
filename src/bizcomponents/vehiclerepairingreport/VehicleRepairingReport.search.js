...
import styles from './VehicleRepairingReport.search.less'
...
@Form.create()
export default class VehicleRepairingReportSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const vehicleRepairingReportIds = selectedRows.map((item) => { return item.id })
    console.log('vehicleRepairingReportIds', vehicleRepairingReportIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'vehicleRepairingReport', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {VehicleRepairingReportTable} = GlobalComponents;
    const {VehicleRepairingReportConfirmationTable} = GlobalComponents;
    const {VehicleRepairingReportSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <VehicleRepairingReportConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="车辆维修报告列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <VehicleRepairingReportSearchForm {...this.props} />
            </div>
