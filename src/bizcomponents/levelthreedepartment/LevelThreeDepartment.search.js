...
@Form.create()
export default class LevelThreeDepartmentSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const levelThreeDepartmentIds = selectedRows.map((item) => { return item.id })
    console.log('levelThreeDepartmentIds', levelThreeDepartmentIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'levelThreeDepartment' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'levelThreeDepartment', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {LevelThreeDepartmentConfirmationTable} = GlobalComponents;
...
<Menu.Item key="belongsTo">关联属于</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <LevelThreeDepartmentConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 三级部门列表`}>
        <Card bordered={false}>
