...
class FundationAccountCreateForm extends Component {
  ...
  executeCandidateAccountManagementSearch = (filterKey) =>{
    ...
    const id = "";//not used for now
    const pageNo = 1;
    const future = FundationAccountService.requestCandidateAccountManagement("accountManagement", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'fundationAccount',listName:'基金账户列表' },
      })
    }
