...
import styles from './FundationAccount.search.less'
...
class FundationAccountSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const fundationAccountIds = selectedRows.map((item) => { return item.id })
    console.log('fundationAccountIds', fundationAccountIds)
    const parameters = { fundationAccountIds }
    dispatch({
      type: `${owner.type}/removeFundationAccountList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'fundationAccount' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'fundationAccount', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'基金账户列表' } })
  ...
  render() {
    ...
    const {FundationAccountTable} = GlobalComponents;
    const {FundationAccountConfirmationTable} = GlobalComponents;
    const {FundationAccountSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <FundationAccountConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 基金账户列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <FundationAccountSearchForm {...this.props} />
            </div>
...
export default Form.create()(FundationAccountSearch)
