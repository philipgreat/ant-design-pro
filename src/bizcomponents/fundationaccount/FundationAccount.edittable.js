...
import styles from './FundationAccount.edittable.less'
...
class FundationAccountEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const fundationAccountIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {fundationAccountId} = record.id
      const parameters = { ...record, fundationAccountId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateFundationAccount`,
        payload: {
          id: owner.id,
          type: 'fundationAccount',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newFundationAccountToAppend  = {
      	'id':`+1`, 
				'name':'',
				'amount':'',
				'platform':'',
      ...
      newData.push(newFundationAccountToAppend);
...
export default Form.create()(FundationAccountEditTable)
