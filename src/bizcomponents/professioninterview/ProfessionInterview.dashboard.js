...
export default class ProfessionInterviewDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, professionInterview } = this.props;
    
    if(!professionInterview){
    	return;
    }
    const {displayName} = professionInterview;
    if(!displayName){
    	return;
    }
        ...
        <div>
          <Row gutter={24}>
            ...
            <Card title={`员工(${numeral(employeeCount).format('员工0,0')})`}  style={{ width: 180 }}>
              ...
              <p><Link to={`/professionInterview/${id}/list/employeeList/员工列表`}><FontAwesome name="gear"  />&nbsp;管理</Link></p>
              <p><Link to={`/professionInterview/${id}/list/employeeCreateForm`}><FontAwesome name="plus"  />&nbsp;新增</Link></p>
