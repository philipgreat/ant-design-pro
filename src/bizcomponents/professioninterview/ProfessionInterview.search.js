...
@Form.create()
export default class ProfessionInterviewSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const professionInterviewIds = selectedRows.map((item) => { return item.id })
    console.log('professionInterviewIds', professionInterviewIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'professionInterview' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'professionInterview', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {ProfessionInterviewConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ProfessionInterviewConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 专业面试列表`}>
        <Card bordered={false}>
