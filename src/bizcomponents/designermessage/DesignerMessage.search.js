...
import styles from './DesignerMessage.search.less'
...
@Form.create()
export default class DesignerMessageSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const designerMessageIds = selectedRows.map((item) => { return item.id })
    console.log('designerMessageIds', designerMessageIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'designerMessage' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'designerMessage', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {DesignerMessageTable} = GlobalComponents;
    const {DesignerMessageConfirmationTable} = GlobalComponents;
    const {DesignerMessageSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <DesignerMessageConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="设计师信息列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <DesignerMessageSearchForm {...this.props} />
            </div>
