...
import styles from './StoreAccount.search.less'
...
class StoreAccountSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const storeAccountIds = selectedRows.map((item) => { return item.id })
    console.log('storeAccountIds', storeAccountIds)
    const parameters = { storeAccountIds }
    dispatch({
      type: `${owner.type}/removeStoreAccountList`,
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'storeAccount' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'storeAccount', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'存储账户列表' } })
  ...
  render() {
    ...
    const {StoreAccountTable} = GlobalComponents;
    const {StoreAccountConfirmationTable} = GlobalComponents;
    const {StoreAccountSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <StoreAccountConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 存储账户列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <StoreAccountSearchForm {...this.props} />
            </div>
...
export default Form.create()(StoreAccountSearch)
