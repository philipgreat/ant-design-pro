...
class StoreAccountCreateForm extends Component {
  ...
  executeCandidateAccountManagementSearch = (filterKey) =>{
    ...
    const id = "";//not used for now
    const pageNo = 1;
    const future = StoreAccountService.requestCandidateAccountManagement("accountManagement", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'storeAccount',listName:'存储账户列表' },
      })
    }
              ...
              <Col lg={12} md={12} sm={24}>
                <Form.Item label={fieldLabels.store} {...formItemLayout}>
                  {getFieldDecorator('storeId', {
                    ...
                    rules: [{ required: true, message: '请输入商店' }],
                  })(
