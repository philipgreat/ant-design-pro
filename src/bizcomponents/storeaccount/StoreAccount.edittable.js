...
import styles from './StoreAccount.edittable.less'
...
class StoreAccountEditTable extends PureComponent {
  ...
  render() {
    ...
    const remapReference = (record) => {
      ...
      return {storeId,accountManagementId,};
    }
    const deleteRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const storeAccountIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {storeAccountId} = record.id
      const parameters = { ...record, storeAccountId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateStoreAccount`,
        payload: {
          id: owner.id,
          type: 'storeAccount',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newStoreAccountToAppend);
...
export default Form.create()(StoreAccountEditTable)
