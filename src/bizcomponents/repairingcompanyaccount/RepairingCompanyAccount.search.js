...
class RepairingCompanyAccountSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const repairingCompanyAccountIds = selectedRows.map((item) => { return item.id })
    console.log('repairingCompanyAccountIds', repairingCompanyAccountIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'repairingCompanyAccount', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'修理厂对账单列表' } })
  ...
  render() {
    ...
    const {RepairingCompanyAccountTable} = GlobalComponents;
    const {RepairingCompanyAccountConfirmationTable} = GlobalComponents;
    const {RepairingCompanyAccountSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <RepairingCompanyAccountConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 修理厂对账单列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <RepairingCompanyAccountSearchForm {...this.props} />
            </div>
...
export default Form.create()(RepairingCompanyAccountSearch)
