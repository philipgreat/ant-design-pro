...
import styles from './Banner.search.less'
...
class BannerSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const bannerIds = selectedRows.map((item) => { return item.id })
    console.log('bannerIds', bannerIds)
    const parameters = { bannerIds }
    dispatch({
      type: `${owner.type}/removeBannerList`,
      payload: { id: owner.id, type: 'banner', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'banner' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'banner', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'横幅列表' } })
  ...
  render() {
    ...
    const {BannerTable} = GlobalComponents;
    const {BannerConfirmationTable} = GlobalComponents;
    const {BannerSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <BannerConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 横幅列表`}>
        <Card bordered={false}>
...
export default Form.create()(BannerSearch)
