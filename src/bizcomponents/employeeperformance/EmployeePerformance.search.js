...
@Form.create()
export default class EmployeePerformanceSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const employeePerformanceIds = selectedRows.map((item) => { return item.id })
    console.log('employeePerformanceIds', employeePerformanceIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employeePerformance' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employeePerformance', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {EmployeePerformanceConfirmationTable} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EmployeePerformanceConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 员工绩效列表`}>
        <Card bordered={false}>
