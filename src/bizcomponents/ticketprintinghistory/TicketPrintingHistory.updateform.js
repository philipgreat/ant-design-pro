...
import styles from './TicketPrintingHistory.updateform.less'
...
class TicketPrintingHistoryUpdateForm extends Component {
  state = {
  ...
  getSelectedRow() {
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...
        printDatetime: moment(item.printDatetime),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const ticketPrintingHistoryId = values.id
        const imagesValues = mapBackToImageValues(convertedImagesValues)
        const parameters = { ...values, ticketPrintingHistoryId, ...imagesValues }

        // const newIndex= currentUpdateIndex + 1
        dispatch({
          type: `${owner.type}/updateTicketPrintingHistory`,
          payload: {
            id: owner.id,
            type: 'ticketPrintingHistory',
            parameters,
            selectedRows,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextTicketPrintingHistoryUpdateRow`,
        payload: {
          id: owner.id,
          type: 'ticketPrintingHistory',
          selectedRows,
    ...
    return (
      <PageHeaderLayout
        title={"更新门票打印记录"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新门票打印记录"
...
}))(Form.create()(TicketPrintingHistoryUpdateForm))
