...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        const viewDetailMatch = pathToRegexp('/threadCompletion/:id/viewDetail').exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp('/threadCompletion/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *goback({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/threadCompletion/${id}/list/${type}List/${listName}`))
    },
    ...
    *addThread({ payload }, { call, put }) {
      ...
      const location = { pathname: `/threadCompletion/${id}/list/${type}List/主贴列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateThread({ payload }, { call, put }) {
      ...
      const location = { pathname: `/threadCompletion/${id}/list/${type}List/主贴列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
