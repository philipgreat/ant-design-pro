...
export default {
    ...
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        const editDetailMatch = pathToRegexp(
          '/threadCompletion/:id/editDetail'
        ).exec(pathname)
        if (editDetailMatch) {
        ...
        const viewDetailMatch = pathToRegexp(
          '/threadCompletion/:id/viewDetail'
        ).exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp('/threadCompletion/:id/list/:listName').exec(
          pathname
        )
        if (!match) {
          return
  ...
  effects: {
    *view({ payload }, { call, put }) {
      const { ThreadCompletionService } = GlobalComponents
    ...
    *gotoCreateForm({ payload }, { put }) {
      ...
      yield put(
        routerRedux.push(`/threadCompletion/${id}/list/${type}CreateForm`)
      )
    },
    *gotoUpdateForm({ payload }, { put }) {
      ...
      const location = {
        pathname: `/threadCompletion/${id}/list/${type}UpdateForm`,
        state,
      }
    ...
    *addThread({ payload }, { call, put }) {
      ...
      const location = {
        pathname: `/threadCompletion/${id}/list/${type}List`,
        state: data,
      }
      yield put(routerRedux.push(location))
    },
    *updateThread({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(
        ThreadCompletionService.updateThread,
        id,
        parameters
      )
      if (hasError(data)) {
    ...
    *removeThreadList({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(
        ThreadCompletionService.removeThreadList,
        id,
        parameters
      )
      if (hasError(data)) {
