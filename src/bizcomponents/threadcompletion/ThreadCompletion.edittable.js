...
import styles from './ThreadCompletion.edittable.less'
...
class ThreadCompletionEditTable extends PureComponent {
  ...
  render() {
    ...
    const addRecord = (e,record) =>{
      ...
      dispatch({
        type: `${owner.type}/addThreadCompletion`,
        payload: {
          id: owner.id,
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {threadCompletionId} = record.id
      const parameters = { ...record, threadCompletionId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateThreadCompletion`,
        payload: {
          id: owner.id,
          type: 'threadCompletion',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newThreadCompletionToAppend  = {
      	'id':`+1`, 
				'who':'',
				'actionTime':'',
				'comment':'',
      ...
      newData.push(newThreadCompletionToAppend);
...
export default Form.create()(ThreadCompletionEditTable)
