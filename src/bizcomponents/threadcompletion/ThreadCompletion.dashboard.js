...
class ThreadCompletionDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, threadCompletion } = this.props;
    
    if(!threadCompletion){
    	return;
    }
    const {displayName} = threadCompletion;
    if(!displayName){
    	return;
    }
  ...
  render() {
    ...
    const { id,displayName, threadCount } = this.props.threadCompletion
    const cardsData = {cardsName:"线程完成",cardsFor: "threadCompletion",cardsSource: this.props.threadCompletion,
  		subItems: [
