...
class ThreadCompletionSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'线程完成列表' } })
  ...
  render() {
    ...
    const {ThreadCompletionTable} = GlobalComponents;
    const {ThreadCompletionConfirmationTable} = GlobalComponents;
    const {ThreadCompletionSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 线程完成列表`}>
        <Card bordered={false}>
...
export default Form.create()(ThreadCompletionSearch)
