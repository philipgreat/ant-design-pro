...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        const editDetailMatch = pathToRegexp('/threadApproval/:id/editDetail').exec(pathname)
        if (editDetailMatch) {
          const id = editDetailMatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const viewDetailMatch = pathToRegexp('/threadApproval/:id/viewDetail').exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp('/threadApproval/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    *view({ payload }, { call, put }) { 
      const {ThreadApprovalService} = GlobalComponents;
    ...
    *goback({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/threadApproval/${id}/list/${type}List/${listName}`))
    },
    ...
    *addThread({ payload }, { call, put }) {
      ...
      const location = { pathname: `/threadApproval/${id}/list/${type}List/主贴列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateThread({ payload }, { call, put }) {
      ...
      const location = { pathname: `/threadApproval/${id}/list/${type}List/主贴列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
