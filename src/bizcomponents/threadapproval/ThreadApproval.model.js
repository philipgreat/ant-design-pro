...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        const match = pathToRegexp('/threadApproval/:id/list/:listName').exec(
          pathname
        )
        if (!match) {
          return
  ...
  effects: {
    ...
    *gotoCreateForm({ payload }, { put }) {
      ...
      yield put(
        routerRedux.push(`/threadApproval/${id}/list/${type}CreateForm`)
      )
    },
    *gotoUpdateForm({ payload }, { put }) {
      ...
      const location = {
        pathname: `/threadApproval/${id}/list/${type}UpdateForm`,
        state,
      }
    ...
    *addThread({ payload }, { call, put }) {
      ...
      const location = {
        pathname: `/threadApproval/${id}/list/${type}List`,
        state: data,
      }
      yield put(routerRedux.push(location))
    },
    *updateThread({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(
        ThreadApprovalService.updateThread,
        id,
        parameters
      )
      if (hasError(data)) {
    ...
    *removeThreadList({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(
        ThreadApprovalService.removeThreadList,
        id,
        parameters
      )
      if (hasError(data)) {
