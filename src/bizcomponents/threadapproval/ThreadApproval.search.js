...
import ThreadApprovalTable from './ThreadApproval.table';
import ThreadApprovalConfirmationTable from './ThreadApproval.confirmmationtable';

import ThreadApprovalSearchForm from './ThreadApproval.searchform';

import PageHeaderLayout from '../../layouts/PageHeaderLayout';

import styles from './ThreadApproval.search.less';
...
@Form.create()
export default class ThreadApprovalSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    const params = {
      threadApprovalList: 1,
      threadApprovalListCurrentPage: pagination.current,
      threadApprovalListRowsPerPage: pagination.pageSize,
      ...formValues,
      ...filters,
    };
    if (sorter.field) {
  ...
  handleDelete = () => {
    ...
    const threadApprovalIds = selectedRows.map((item) => { return item.id; });
    console.log('threadApprovalIds', threadApprovalIds);
    const parameters = { threadApprovalIds };
    dispatch({
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'threadApproval', selectedRows, currentUpdateIndex },
    });
  }
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ThreadApprovalConfirmationTable data={selectedRows} owner={owner} />
        </Modal>);
    };
    
    return (
      <PageHeaderLayout title="线审批列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ThreadApprovalSearchForm {...this.props} />
            </div>
