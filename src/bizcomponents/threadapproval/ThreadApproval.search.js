...
class ThreadApprovalSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'线审批列表' } })
  ...
  render() {
    ...
    const {ThreadApprovalTable} = GlobalComponents;
    const {ThreadApprovalConfirmationTable} = GlobalComponents;
    const {ThreadApprovalSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 线审批列表`}>
        <Card bordered={false}>
...
export default Form.create()(ThreadApprovalSearch)
