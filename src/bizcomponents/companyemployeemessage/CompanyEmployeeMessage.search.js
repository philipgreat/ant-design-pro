...
@Form.create()
export default class CompanyEmployeeMessageSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleStandardTableChange = (pagination, filtersArg, sorter) => {
    ...
    const params = {
      companyEmployeeMessageList: 1,
      companyEmployeeMessageListCurrentPage: pagination.current,
      companyEmployeeMessageListRowsPerPage: pagination.pageSize,
      ...formValues,
      ...filters,
    }
    if (sorter.field) {
  ...
  handleDelete = () => {
    ...
    const companyEmployeeMessageIds = selectedRows.map((item) => { return item.id })
    console.log('companyEmployeeMessageIds', companyEmployeeMessageIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'companyEmployeeMessage', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {CompanyEmployeeMessageTable} = GlobalComponents;
    const {CompanyEmployeeMessageConfirmationTable} = GlobalComponents;
    const {CompanyEmployeeMessageSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CompanyEmployeeMessageConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="消息管理列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CompanyEmployeeMessageSearchForm {...this.props} />
            </div>
