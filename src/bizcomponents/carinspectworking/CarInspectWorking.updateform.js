...
import styles from './CarInspectWorking.updateform.less';
...
class CarInspectWorkingUpdateForm extends Component {
 ...
 mapBackToImageValues(convertedImagesValues) {
  ...
  getSelectedRow(){
    ...
    const convertiedValues = selectedRows.map((item) => {
      return {
        ...item,
			workTime: moment(item.workTime).format('YYYY-MM-DD'),
  ...
  render() {
    ...
    const submitUpdateForm = () => {
     validateFieldsAndScroll((error, values) => {
        ...
        const carInspectWorkingId = values.id;
        const imagesValues = this.mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values,carInspectWorkingId, ...imagesValues };

	    const newIndex= currentUpdateIndex+1;
        dispatch({
          type: owner.type+'/updateCarInspectWorking',
    ...
    const skipToNext = () => {
      ...
      const newIndex= currentUpdateIndex+1;
      dispatch({
          type: owner.type+'/gotoNextCarInspectWorkingUpdateRow',
            payload: {
              id:owner.id,type:'carInspectWorking', 
    ...
    return (
      <PageHeaderLayout
        ...
        title={"更新正在审车"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新正在审车"
...
}))(Form.create()(CarInspectWorkingUpdateForm));
