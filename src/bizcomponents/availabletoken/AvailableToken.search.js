...
import styles from './AvailableToken.search.less'
...
class AvailableTokenSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const availableTokenIds = selectedRows.map((item) => { return item.id })
    console.log('availableTokenIds', availableTokenIds)
    const parameters = { availableTokenIds }
    dispatch({
      type: `${owner.type}/removeAvailableTokenList`,
      payload: { id: owner.id, type: 'availableToken', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'availableToken' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'availableToken', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'使用令牌列表' } })
  ...
  render() {
    ...
    const {AvailableTokenTable} = GlobalComponents;
    const {AvailableTokenConfirmationTable} = GlobalComponents;
    const {AvailableTokenSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <AvailableTokenConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 使用令牌列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <AvailableTokenSearchForm {...this.props} />
            </div>
...
export default Form.create()(AvailableTokenSearch)
