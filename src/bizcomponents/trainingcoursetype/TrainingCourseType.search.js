...
@Form.create()
export default class TrainingCourseTypeSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const trainingCourseTypeIds = selectedRows.map((item) => { return item.id })
    console.log('trainingCourseTypeIds', trainingCourseTypeIds)
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'trainingCourseType' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'trainingCourseType', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {TrainingCourseTypeConfirmationTable} = GlobalComponents;
...
<Menu.Item key="company">关联公司</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <TrainingCourseTypeConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 培训课程类型列表`}>
        <Card bordered={false}>
