...
@Form.create()
export default class EmployeeInterviewSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const employeeInterviewIds = selectedRows.map((item) => { return item.id })
    console.log('employeeInterviewIds', employeeInterviewIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'employeeInterview', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {EmployeeInterviewConfirmationTable} = GlobalComponents;
...
<Menu.Item key="interviewType">关联面试类型</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <EmployeeInterviewConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 员工面试列表`}>
        <Card bordered={false}>
