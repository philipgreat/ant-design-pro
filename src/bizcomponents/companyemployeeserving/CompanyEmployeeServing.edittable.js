...
import styles from './CompanyEmployeeServing.edittable.less'
...
class CompanyEmployeeServingEditTable extends PureComponent {
  ...
  render() {
    ...
    const deleteRecord = (e,record) =>{
      ...
      const {data} = this.state
      const companyEmployeeServingIds = [record.id];
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {companyEmployeeServingId} = record.id
      const parameters = { ...record, companyEmployeeServingId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 

      dispatch({
        type: `${owner.type}/updateCompanyEmployeeServing`,
        payload: {
          id: owner.id,
          type: 'companyEmployeeServing',
          parameters,
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      const newCompanyEmployeeServingToAppend  = {
      	'id':`+1`, 
				'eventTime':'',
				'who':'',
				'comment':'',
      ...
      newData.push(newCompanyEmployeeServingToAppend);
...
export default Form.create()(CompanyEmployeeServingEditTable)
