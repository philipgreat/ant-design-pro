...
import styles from './GameCategory.search.less'
...
class GameCategorySearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const gameCategoryIds = selectedRows.map((item) => { return item.id })
    console.log('gameCategoryIds', gameCategoryIds)
    const parameters = { gameCategoryIds }
    dispatch({
      type: `${owner.type}/removeGameCategoryList`,
      payload: { id: owner.id, type: 'gameCategory', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'gameCategory' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'gameCategory', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'游戏类别列表' } })
  ...
  render() {
    ...
    const {GameCategoryTable} = GlobalComponents;
    const {GameCategoryConfirmationTable} = GlobalComponents;
    const {GameCategorySearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <GameCategoryConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 游戏类别列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <GameCategorySearchForm {...this.props} />
            </div>
...
export default Form.create()(GameCategorySearch)
