...
import styles from './GameCategory.createform.less'
...
const testValues = {
  name: '密室逃脱',
  description: '封闭的房间、独特的设计将你从现实抽离，这里有不同的困境和谜题。时间所剩不多了，运用你的智慧，你必须要逃出生天！',
  storeId: 'S000001',
}
*/
...
class GameCategoryCreateForm extends Component {
  state = {
  ...
  executeCandidateStoreSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = GameCategoryService.requestCandidateStore("store", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addGameCategory`,
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addGameCategory`,
          payload: { id: owner.id, type: 'gameCategory', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'gameCategory',listName:'游戏类别列表' },
      })
    }
...
}))(Form.create()(GameCategoryCreateForm))
