...
import styles from './CarReceiveWorking.updateform.less';
...
class CarReceiveWorkingUpdateForm extends Component {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
     validateFieldsAndScroll((error, values) => {
        ...
        const carReceiveWorkingId = values.id;
        const imagesValues = this.mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values,carReceiveWorkingId, ...imagesValues };

	    const newIndex= currentUpdateIndex+1;
        dispatch({
          type: owner.type+'/updateCarReceiveWorking',
    ...
    const skipToNext = () => {
      ...
      const newIndex= currentUpdateIndex+1;
      dispatch({
          type: owner.type+'/gotoNextCarReceiveWorkingUpdateRow',
            payload: {
              id:owner.id,type:'carReceiveWorking', 
    ...
    return (
      <PageHeaderLayout
        ...
        title={"更新收车检查"+(currentUpdateIndex+1)+"/"+selectedRows.length}
        content="更新收车检查"
...
}))(Form.create()(CarReceiveWorkingUpdateForm));
