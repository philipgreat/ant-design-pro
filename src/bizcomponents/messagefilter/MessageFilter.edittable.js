...
import styles from './MessageFilter.edittable.less'
...
class MessageFilterEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e, record) => {
      const { dispatch, owner } = this.props
      const { data } = this.state
      const { messageFilterId } = record.id
      const parameters = { ...record, messageFilterId }
      const newData = [...data]
      const row = newData.filter(item => item.id === record.id)[0]
      row.editable = !row.editable
    ...
    const columns = [
      ...
      },
      {
        ...
        width: '9',
        render: (text, record) =>
          renderStringEdit('messageCount', text, record),
      },
      {
        title: '过滤器健值',
        debugtype: 'string',
    ...
    const newRecord = () => {
      const newMessageFilterToAppend = {
        id: `+1`,
        name: '',
        messageCount: '',
        filterKey: '',
      ...
      newData.push(newMessageFilterToAppend)
...
export default Form.create()(MessageFilterEditTable)
