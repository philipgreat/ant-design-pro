...
const imageListOf = (messageFilter) =>{
...
const settingListOf = (messageFilter) =>{
...
const largeTextOf = (messageFilter) =>{
...
class MessageFilterDashboard extends Component {
  ...
  /*
    ...
    const { dispatch, location, messageFilter } = this.props;
    
    if(!messageFilter){
    	return;
    }
    const {displayName} = messageFilter;
    if(!displayName){
    	return;
    }
  ...
  render() {
    ...
    const { id,displayName,  } = this.props.messageFilter
    const cardsData = {cardsName:"消息过滤",cardsFor: "messageFilter",cardsSource: this.props.messageFilter,
  		subItems: [
