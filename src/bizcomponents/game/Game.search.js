...
import styles from './Game.search.less'
...
class GameSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const gameIds = selectedRows.map((item) => { return item.id })
    console.log('gameIds', gameIds)
    const parameters = { gameIds }
    dispatch({
      type: `${owner.type}/removeGameList`,
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'game', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'游戏列表' } })
  ...
  render() {
    ...
    const {GameTable} = GlobalComponents;
    const {GameConfirmationTable} = GlobalComponents;
    const {GameSearchForm} = GlobalComponents;
...
<Menu.Item key="gameCategory">关联游戏类别</Menu.Item>
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <GameConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 游戏列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <GameSearchForm {...this.props} />
            </div>
...
export default Form.create()(GameSearch)
