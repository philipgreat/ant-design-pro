...
class ThreadReplySearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'跟帖回复列表' } })
  ...
  render() {
    ...
    const {ThreadReplyTable} = GlobalComponents;
    const {ThreadReplyConfirmationTable} = GlobalComponents;
    const {ThreadReplySearchForm} = GlobalComponents;
...
<Menu.Item key="thread">关联主贴</Menu.Item>
    ...
    return (
      <PageHeaderLayout title={`${displayName}: 跟帖回复列表`}>
        <Card bordered={false}>
...
export default Form.create()(ThreadReplySearch)
