...
import styles from './ThreadReply.edittable.less'
...
class ThreadReplyEditTable extends PureComponent {
  ...
  render() {
    ...
    const updateRecord = (e,record) =>{
      const {dispatch, owner} = this.props
      const {data} = this.state
      const {threadReplyId} = record.id
      const parameters = { ...record, threadReplyId }
      const newData = [...data];
      const row =  newData.filter(item => item.id === record.id)[0];
      row.editable = !row.editable 
...
{ title: '操作',
   render: (text, record) => renderActions(text, record)}]
    ...
    const newRecord =()=>{
      ...
      newData.push(newThreadReplyToAppend);
...
export default Form.create()(ThreadReplyEditTable)
