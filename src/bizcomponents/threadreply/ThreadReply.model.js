...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        const dashboardmatch = pathToRegexp('/threadReply/:id/dashboard').exec(
          pathname
        )
        if (dashboardmatch) {
        ...
        const match = pathToRegexp('/threadReply/:id/list/:listName').exec(
          pathname
        )
        if (!match) {
          return
  ...
  effects: {
    ...
    *gotoUpdateForm({ payload }, { put }) {
      ...
      const location = {
        pathname: `/threadReply/${id}/list/${type}UpdateForm`,
        state,
      }
    ...
    *addThreadReplyLike({ payload }, { call, put }) {
      ...
      const data = yield call(
        ThreadReplyService.addThreadReplyLike,
        id,
        parameters
      )
      if (hasError(data)) {
      ...
      const location = {
        pathname: `/threadReply/${id}/list/${type}List`,
        state: data,
      }
      yield put(routerRedux.push(location))
    },
    *updateThreadReplyLike({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(
        ThreadReplyService.updateThreadReplyLike,
        id,
        parameters
      )
      if (hasError(data)) {
    ...
    *removeThreadReplyLikeList({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(
        ThreadReplyService.removeThreadReplyLikeList,
        id,
        parameters
      )
      if (hasError(data)) {
