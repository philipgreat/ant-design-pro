...
export default {
  ...
  subscriptions: {
    ...
    setup({ dispatch, history }) { 
      history.listen((location) => {
        ...
        const editDetailMatch = pathToRegexp('/threadReply/:id/editDetail').exec(pathname)
        if (editDetailMatch) {
          const id = editDetailMatch[1]
          dispatch({ type: 'view', payload: { id,pathname } })
          return
        }
        const viewDetailMatch = pathToRegexp('/threadReply/:id/viewDetail').exec(pathname)
        if (viewDetailMatch) {
        ...
        const match = pathToRegexp('/threadReply/:id/list/:listName/:listDisplayName').exec(pathname)
        if (!match) {
          return
  ...
  effects: {
    ...
    *goback({ payload }, { put }) {
      ...
      yield put(routerRedux.push(`/threadReply/${id}/list/${type}List/${listName}`))
    },
    ...
    *addThreadReplyLike({ payload }, { call, put }) {
      ...
      const location = { pathname: `/threadReply/${id}/list/${type}List/跟帖回复点赞列表`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateThreadReplyLike({ payload }, { call, put }) {
      ...
      const location = { pathname: `/threadReply/${id}/list/${type}List/跟帖回复点赞列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
