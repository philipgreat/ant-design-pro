...
class CommunityUserSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const communityUserIds = selectedRows.map((item) => { return item.id })
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'communityUser', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'社区用户列表' } })
  ...
  render() {
    ...
    const {CommunityUserTable} = GlobalComponents;
    const {CommunityUserConfirmationTable} = GlobalComponents;
    const {CommunityUserSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <CommunityUserConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 社区用户列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <CommunityUserSearchForm {...this.props} />
            </div>
...
export default Form.create()(CommunityUserSearch)
