  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        dataIndex: 'nickName',
        width: '6',
        render: (text, record) => renderStringEdit('nickName', text, record),
      },
      {
        title: '性别',
      ...
      },
      {
        ...
        dataIndex: 'userType',
        width: '8',
        render: (text, record) => renderStringEdit('userType', text, record),
      },
      {
        title: '头像',
        dataIndex: 'avatar',
        render: (text, record) => (
          <ImagePreview imageLocation={record.avatar} />
        ),
      },
      {
        title: '生日',
        dataIndex: 'birthday',
        render: (text, record) => moment(record.birthday).format('YYYY-MM-DD'),
      },
      {
        title: '成长值',
        debugtype: 'int',
        dataIndex: 'experiencePoint',
        width: '9',
        render: (text, record) =>
          renderStringEdit('experiencePoint', text, record),
      },
      {
        title: '积分',
        debugtype: 'int',
        ...
        width: '11',
        render: (text, record) => renderStringEdit('bonusPoint', text, record),
      },
      {
        title: '城市',
        debugtype: 'string',
      ...
      },
      {
        title: '隐藏的信息',
        dataIndex: 'hideInfo',
        render: (text, record) => (record.hideInfo ? '是' : '否'),
      },
      {
        title: '管理员',
        dataIndex: 'administrator',
        render: (text, record) => (record.administrator ? '是' : '否'),
      },
      {
        title: '社区',
        dataIndex: 'community',
      ...
      },
      {
        title: '点经验限制',
        debugtype: 'int',
        dataIndex: 'experiencePointLimit',
        width: '7',
        render: (text, record) =>
          renderStringEdit('experiencePointLimit', text, record),
      },
      {
        title: '经验点仍',
        debugtype: 'int',
        dataIndex: 'experiencePointRemain',
        width: '7',
        render: (text, record) =>
          renderStringEdit('experiencePointRemain', text, record),
      },
      {
        title: '经验点过去的日子',
        dataIndex: 'experiencePointLastDate',
        render: (text, record) =>
          moment(record.experiencePointLastDate).format('YYYY-MM-DD'),
      },
      {
        title: '操作',
