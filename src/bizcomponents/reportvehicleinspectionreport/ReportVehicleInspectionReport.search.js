...
import styles from './ReportVehicleInspectionReport.search.less'
...
@Form.create()
export default class ReportVehicleInspectionReportSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const reportVehicleInspectionReportIds = selectedRows.map((item) => { return item.id })
    console.log('reportVehicleInspectionReportIds', reportVehicleInspectionReportIds)
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'reportVehicleInspectionReport', selectedRows, currentUpdateIndex },
    })
  }
  ...
  render() {
    ...
    const {ReportVehicleInspectionReportTable} = GlobalComponents;
    const {ReportVehicleInspectionReportConfirmationTable} = GlobalComponents;
    const {ReportVehicleInspectionReportSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <ReportVehicleInspectionReportConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title="车辆上线年检报告列表">
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ReportVehicleInspectionReportSearchForm {...this.props} />
            </div>
