...
import styles from './Printer.createform.less'
...
const testValues = {
  name: '新希望大厦1号标签打印机',
  location: '成都市高新区天府3街新希望大厦A座1704',
...
class PrinterCreateForm extends Component {
  state = {
  ...
  executeCandidatePlatformSearch = (filterKey) =>{
    ...
    const pageNo = 1;
    const future = PrinterService.requestCandidatePlatform("bookSharingPlatform", id, filterKey, pageNo);
    console.log(future);
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/addPrinter`,
          payload: { id: owner.id, type: 'printer', parameters, continueNext: true },
        })
      })
    }
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'printer',listName:'打印机列表' },
      })
    }
