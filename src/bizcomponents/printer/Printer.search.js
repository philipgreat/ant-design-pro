...
import styles from './Printer.search.less'
...
class PrinterSearch extends PureComponent {
  state = {
    addInputValue: '',
    modalVisible: false,
    // expandForm: false,
  ...
  handleDelete = () => {
    ...
    const printerIds = selectedRows.map((item) => { return item.id })
    console.log('printerIds', printerIds)
    const parameters = { printerIds }
    dispatch({
      type: `${owner.type}/removePrinterList`,
      payload: { id: owner.id, type: 'printer', parameters },
    })
  }
  ...
  handleCreate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'printer' },
    })
  }
  ...
  handleUpdate = () => {
    ...
    dispatch({
      ...
      payload: { id: owner.id, type: 'printer', selectedRows, currentUpdateIndex },
    })
  }
  ...
  handleCloseAlert = () => {
      ...
      dispatch({ type: `${owner.type}/view`, payload: { id: owner.id,pathname,displayName:'打印机列表' } })
  ...
  render() {
    ...
    const {PrinterTable} = GlobalComponents;
    const {PrinterConfirmationTable} = GlobalComponents;
    const {PrinterSearchForm} = GlobalComponents;
    ...
    const modalContent = (data, owner) => {
      ...
      return (
        <Modal
          ...
          style={{ top: 40 }}
        >
          <PrinterConfirmationTable data={selectedRows} owner={owner} />
        </Modal>)
    }
    
    return (
      <PageHeaderLayout title={`${displayName}: 打印机列表`}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <PrinterSearchForm {...this.props} />
            </div>
...
export default Form.create()(PrinterSearch)
