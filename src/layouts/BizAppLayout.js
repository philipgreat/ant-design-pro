...
import styles from './BizAppLayout.less';
...
class BizAppLayout extends React.PureComponent {
  ...
  getPageTitle() {
    ...
    getRouteData('BizAppLayout').forEach((item) => {
      ...
      <Layout>
        ...
        <Layout>
          ...
          <Content style={{ margin: '24px 24px 0', height: '100%' }}>
            <Switch>
              {
                getRouteData('BizAppLayout').map(item =>
                  (
                    <Route
                      exact={item.exact}
                      key={item.path}
